...
C. de Dinechin (descubes@earthlink.net):
Fixed resizing, added function key mapping, X-compatible colors
*/
...
{
  NSHomeFunctionKey,            0xFF50,
  NSLeftArrowFunctionKey,       0xFF51,
  NSUpArrowFunctionKey,         0xFF52,
  NSRightArrowFunctionKey,      0xFF53,
  NSDownArrowFunctionKey,       0xFF54,
  NSPageUpFunctionKey,          0xFF55,
  NSPageDownFunctionKey,        0xFF56,
  NSEndFunctionKey,             0xFF57,
  NSBeginFunctionKey,           0xFF58,
  NSSelectFunctionKey,          0xFF60,
  NSPrintFunctionKey,           0xFF61,
  NSExecuteFunctionKey,         0xFF62,
  NSInsertFunctionKey,          0xFF63,
  NSUndoFunctionKey,            0xFF65,
  NSRedoFunctionKey,            0xFF66,
  NSMenuFunctionKey,            0xFF67,
  NSFindFunctionKey,            0xFF68,
  NSHelpFunctionKey,            0xFF6A,
  NSBreakFunctionKey,           0xFF6B,

  NSF1FunctionKey,              0xFFBE,
  NSF2FunctionKey,              0xFFBF,
  NSF3FunctionKey,              0xFFC0,
  NSF4FunctionKey,              0xFFC1,
  NSF5FunctionKey,              0xFFC2,
  NSF6FunctionKey,              0xFFC3,
  NSF7FunctionKey,              0xFFC4,
  NSF8FunctionKey,              0xFFC5,
  NSF9FunctionKey,              0xFFC6,
  NSF10FunctionKey,             0xFFC7,
  NSF11FunctionKey,             0xFFC8,
  NSF12FunctionKey,             0xFFC9,
  NSF13FunctionKey,             0xFFCA,
  NSF14FunctionKey,             0xFFCB,
  NSF15FunctionKey,             0xFFCC,

  NSDeleteFunctionKey,          0xFFFF
};
...
void debug_log_dps_call(const char *msg)
{
#if 0
  static FILE *f = NULL;
  if (!f)
    f = fopen("/tmp/DPS-in-emacs.log", "a");
  fprintf(f, "%s\n", msg);
  fflush(f);
#endif
}
...
static void keymap_init(void)
   ...
   debug_log_dps_call("keymap_init: [NSDPSServerContext setDeadKeyProcessingEnabled]");
...
void pixel_to_glyph_coords (struct frame *f, int px, int py,
   ...
   *y=(f->output_data.ns->line_height<=0)
   ...
   if (bounds)
     {
       fprintf(stderr,
               "Don't know what to do with bounds (%s line %d)\n",
               __FILE__, __LINE__);
         abort();
     }
...
int ns_get_color (const char *name, NSColor **col)
   ...
   // Parse #RRGGBB as well, since frequently used in X Window
   if (name[0]=='#')
      {
      for(t1=t2=0,c=name+1;1;c++)
         {
...
static void ns_cursor_to(int row, int col)
   {
...
static void ns_reset_clip(id view)
   ...
   PSnewpath();
   PSgstate();
   debug_log_dps_call("ns_reset_clip: DPSDefineUserObject");
   ...
   if (FONT_WIDTH(f->output_data.ns->font)<=0)
     cols = f->width;
   else
      {
        ...
        cols=floor((r.size.width - vbextra)/FONT_WIDTH(f->output_data.ns->font)+0.5);
      }

   if (f->output_data.ns->line_height<=0)
     rows = f->height;
   else
     rows=floor(r.size.height/f->output_data.ns->line_height+0.5);
...
static void ns_clear_frame(void)
   {
   struct frame *f=(updating_frame ? updating_frame : selected_frame);
   id view=f->output_data.ns->view;
   int i;
...
static void ns_clear_end_of_line(int first_unused)
   {
   struct frame *f=(updating_frame ? updating_frame : selected_frame);
   id view=f->output_data.ns->view;
   NSRect r;

   if (curs_y < 0 || curs_y >= f->height) return;
   if (first_unused <= 0) return;
...
static void ns_ins_del_lines(int vpos, int n)
   {
   struct frame *f=(updating_frame ? updating_frame : selected_frame);
   id view=f->output_data.ns->view;
   int height;
...
static void ns_change_line_highlight(int new_highlight, int vpos,
...
static void ns_insert_glyphs(GLYPH *start, int len)
   {
   abort();
   }

static void ns_write_glyphs(GLYPH *start, int len)
   {
   struct frame *f=(updating_frame ? updating_frame : selected_frame);
...
static void ns_delete_glyphs(int len)
   {
   abort();
   }

static void ns_ring_bell(void)
   {
   NSBeep();
   }

static void ns_reset_terminal_modes(void)
   {
   }

static void ns_set_terminal_modes(void)
   {
   }
...
static void ns_set_terminal_window(int n)
   {
   struct frame *f=(updating_frame ? updating_frame : selected_frame);
...
static void ns_reassert_line_highlight(int new, int vpos)
   {
   highlight = new;
   }
...
- (void)keyDown:(NSEvent *)theEvent 
   ...
   // descubes: Mapping of NS-keys to X-compatible function key numbers
   // Note: an array would be better here, but not as easy to read
   // Ultimately, we want to have an 'ns' based table in keyboard.c
   for (keysym = 0; keysym < last_keysym; keysym += 2)
     if (code == convert_ns_to_X_keysym[keysym])
       {
         code = convert_ns_to_X_keysym[keysym+1];
         mapped_function_key = YES;
         break;
       }
   ...
   if (!NILP(ns_alternate_is_meta) && (flags & NSAlternateKeyMask))
      {
      events->modifiers |= meta_modifier;
      flags&=~NSAlternateKeyMask;
      // descubes: removed code = char_from_key([theEvent keyCode],flags);
      }
   ...
   if ((code<0x20)&&(([theEvent modifierFlags]&NSControlKeyMask)==0))
     ...
     code |= (1<<28)|(3<<16);
   else if (mapped_function_key == NO)
...
- (void)windowDidResize:(NSNotification *)notification
   ...
   NSWindow *theWindow = [notification object];
    ...
    // descubes: Added the following so that resizing shows on Rhapsody CR1
    if (cols && rows)
