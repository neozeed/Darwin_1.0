...
static Lock *_dnsLock = NULL;
static Lock *_logLock = NULL;
...
dns_log_msg(dns_handle_t *dns, int priority, char *message, ...)
{
	...
	if (_logLock == NULL) _logLock = [[Lock alloc] init];
	[_logLock lock];
...
_dns_lock(void)
{
	if (_dnsLock == NULL) _dnsLock = [[Lock alloc] init];
	[_dnsLock lock];
}
...
dns_log_open_file(dns_handle_t *dns, char *title, char *name, char *mode)
{
	...
	if (title != NULL) dns->log_title = strdup(title);
}
...
_dns_ni_init(char *dom, u_int32_t timeout)
{
	...
	{
		/* Find /domains and use the value of "domain" property */
	...
	/*
	 * If there is no "domain" property (i.e. if this came from /domains/<dom>
	 * then change "name" to "domain".
	 */
	where = ni_proplist_match(*p, "domain", NULL);
	if (where == NI_INDEX_NULL) 
	{
		...
		free(p->ni_proplist_val[where].nip_name);
		p->ni_proplist_val[where].nip_name = strdup("domain");
	}
...
_dns_file_init(char *dom)
{
	...
	if ((dp[0].nip_val.ni_namelist_len == 0) || (dp[1].nip_val.ni_namelist_len == 0))
	{
		ni_proplist_free(p);
		free(p);
		return NULL;
...
dns_open_lock(char *dom, u_int32_t lockit)
{
	...
	if (where != NI_INDEX_NULL)
	{
		...
		dns->search = (char **)malloc(dns->search_count * sizeof(char *));
	...
	u_int16_t *ql, u_int16_t *xid)
{
	...
	len = DNS_HEADER_SIZE + _dns_cname_length(dnsq->name) + 4;
	...
	char **r, u_int16_t *rlen)
{
	...
	*r = malloc(*rlen);
	...
	{
		free(*r);
		dns_log_msg(dns, LOG_ERR, "dns_read_reply - reply for wrong query");
	...
	char **r, u_int16_t *rlen)
{
	...
	while (try <= dns->server_retries)
	{
		...
		if ((status == DNS_STATUS_WRONG_SERVER) ||
			...
			(status == DNS_STATUS_WRONG_QUESTION))
		{
			dosend = 0;
			continue;
		}
