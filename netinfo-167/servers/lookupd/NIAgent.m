...
- (NIAgent *)init
{
	...
	threadLock = [[Lock alloc] initThreadLock];
	...
	{
		sprintf(str, "NIAgent/%s", [self categoryPathname:(LUCategory)i]);
		config = [controller configurationForAgent:str];
...
- (LUDictionary *)item:(void *)ident method:(SEL)sel category:(LUCategory)cat
{
	...
	for (i = 0; i < len; i++)
	{
		...
		item = [d perform:sel with:(id)ident];
