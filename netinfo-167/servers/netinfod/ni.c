 ...
 */
bool_t
ni_db_reopen(void *handle)
{
	return(file_db_reopen(FH(handle)));
}
...
ni_idlist_hasid(ni_idlist *idlist, ni_index id)
{
	...
	{
		if (idlist->niil_val[i] == id) return 1;
	}
...
ni_lookup(void *handle, ni_id *id, ni_name_const pname, ni_name_const pval, ni_idlist *children_p)
{
	...
	if (im_has_indexed_dir(IMH(handle), id->nii_object, pname, pval, &dirs, &ndirs))
	{
		...
		MM_BCOPY(dirs, res.niil_val, res.niil_len * sizeof(dirs[0]));
	}
	else
	{
		res.niil_len = 0;
	...
	obj_free(obj);
	if (res.niil_len == 0) return NI_NODIR;
...
ni_lookupread(void *handle, ni_id *id, ni_name_const pname, ni_name_const pval, ni_proplist *props)
{
	...
	else
	{
		...
		found = 0;
		for (i = 0; (i < obj->nio_children.niil_len && (please_index || !found)); i++)
		{
			tmpid.nii_object = obj->nio_children.niil_val[i];
			status = obj_lookup(handle, &tmpid, NIOP_READ, &tmp);
			if (status != NI_OK)
		...
		for (i = 0; i < obj->nio_children.niil_len; i++)
		{
			...
			for (j = 0; j < pl->nipl_len; j++)
			{
				...
				if (ni_name_match(pl->nipl_val[j].nip_name, pname) && nl->ninl_len > 0)
				{
					...
					(*res.niel_val[i].names = ni_namelist_dup(*nl));
					break;
				}
			}
			obj_free(tmp);
