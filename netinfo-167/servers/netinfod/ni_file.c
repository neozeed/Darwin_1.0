...
dstoni(dsrecord *r)
{
	...
	{
		p = &(o->nio_props.ni_proplist_val[i]);
...
nitods(ni_object *n)
{
	...
	for (pn = 0; pn < n->nio_props.ni_proplist_len; pn++)
	{
		...
		a->count = len;
		a->value = (dsdata **)malloc(len * sizeof(dsdata *));
...
_convert_read(FILE *f)
{
	...
	if (n > 0)
	{
		...
		r->meta_attribute = (dsattribute **)malloc(n * sizeof(dsattribute *));
	}
...
file_init(char *rootdir, void **hdl)
{
	...
	*hdl = s;
	...
	if (doconvert == 1)
	{
		...
		sys_msg(debug, LOG_NOTICE, "upgrading database format (%d records)...",
			nrecords);
		...
		for (i = 0; i < nrecords; i++)
		{
			...
			if (tenth == 1) sys_msg(debug, LOG_NOTICE, "record %d", i);
			else if ((i % tenth) == 0)
			{
				if (pcnt > 0) sys_msg(debug, LOG_NOTICE, "%d%% complete (%d records)", pcnt, i);
				pcnt += 10;
			}
		}
		fclose(cf);
		
		if ((tenth > 1) && (pcnt <= 100)) sys_msg(debug, LOG_NOTICE, "100%%");
		sys_msg(debug, LOG_NOTICE, "finished database upgrade");
...
file_db_reopen(void *hdl)
{
	...
	status = dsstore_open((dsstore **)(&hdl), file_dirname(hdl), flags);
	if (status == DSStatusOK) return TRUE;
	return FALSE;
}
