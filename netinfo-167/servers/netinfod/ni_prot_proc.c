	...
	struct in_addr **addrs, ni_rparent_stuff *stuff, unsigned int *naddrs)
{
	...
	sys_msg(debug, LOG_DEBUG, "binding list added   %s/%s   (%s/%s)", astr, st, cstr, ct);

	MM_GROW_ARRAY(*addrs, *naddrs);
	(*addrs)[*naddrs].s_addr = sa.s_addr;
	MM_GROW_ARRAY(stuff->bindings, *naddrs);
	stuff->bindings[*naddrs].client_tag = ct;
	/*
...
#ifdef _DHCP_BINDING_
{
	...
	if (status != KERN_SUCCESS || active == FALSE) return;
	...
	for (i = 0; i < l->count; i++)
	{
		...
		status = ipconfig_get_option(server, l->interface[i].name, DHCP_NETINFO_SERVER_ADDRESS, (void *)&dhcp_ni_addr, &len);

		if (status != KERN_SUCCESS)
		{
			sys_msg(debug, LOG_INFO,
					"-DHCP- ipconfig get netinfo server address"
					" option failed, %s", mach_error_string(status));
			continue;
		}
		...
		dhcp_ni_tag[len] = '\0';
		add_binding_entry(dhcp_ni_addr, ni_name_dup(dhcp_ni_tag), l->interface[i].addr, client_tag, addrs, stuff, naddrs);
		sys_msg(debug, LOG_INFO,
				"-DHCP- bind lookup returned %s/%s",
				inet_ntoa(dhcp_ni_addr), dhcp_ni_tag);
	}
	...
	struct in_addr **addrs, ni_rparent_stuff *stuff, unsigned int *naddrs)
{
	...
	sys_msg(debug, LOG_INFO, "-BOOTPARAMS- bind lookup returned %s/%s",
 ...
 */
static int
hardwired(void *ni, ni_rparent_res *res)
{
	...
	{
		/* There's no /machines dir - must be standalone! */
		old_res.status = NI_NETROOT;
		latestParentStatus = NI_NETROOT;
		return (0);
	}
	...
	{
		server_addr.s_addr = servesdotdot(entries.niel_val[i], &server_tag);
		if (server_addr.s_addr == 0) continue;
		...
		for (j = lastn; j < naddrs; j++)
		{
			...
			if (ni_ping(addrs[j].s_addr, stuff.bindings[j].server_tag))
			{
				...
				sys_msg(debug, LOG_INFO, "Anonymous binding to %s/%s",
	      ...
	      )
{
	 ...
	 */
	if (alert_aborted() || sys_is_standalone()) {
...
waitforparent(void)
{
	...
	for (;;) {
		a++;
		sys_msg(debug, LOG_DEBUG, "waitforparent attempt %d", a);
		res = _ni_rparent_2_svc(NULL, (struct svc_req *)(~0));
		if (res->status != NI_NORESPONSE)
		{
			sys_msg(debug, LOG_DEBUG, "waitforparent exiting for status %d", res->status);
			break;
		}
		if (a == n)
		{
			sys_msg(debug, LOG_DEBUG, "waitforparent aborted after %d attempts");
			break;
		}

		for (i = 0; i < PARENT_NINTERVALS; i++) {
			if (alert_aborted()) {
				sys_msg(debug, LOG_DEBUG, "waitforparent aborted (alert_aborted)");
				alert_enable(0);
				return;
			}
...
readall_cleanup(void)
{
    ...
    while (TRUE)
	{
		p = wait4(0, (_WAIT_TYPE_ *)&wait_stat, WNOHANG, NULL);
		switch (p)
		{
