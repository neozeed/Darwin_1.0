 ...
 */
static void
freeclonelist(clone_list clist)
{
	clone_list l;
 ...
 */
static unsigned
servesdot(ni_entry entry, ni_name *name, ni_name *tag)
{
	ni_name sep;
	ni_name slashname;	
	unsigned addr;
	...
	for (i = 0; i < entry.names->ninl_len; i++)
	{
		...
		if (!ni_name_match_seg(NAME_DOT, slashname, sep - slashname)) continue;
...
notify_start(void)
{
	...
	notify_syslock = syslock_new(0);
	subthread_syslock = syslock_new(0);
		...
		while (notifications != NULL)
		{
			if (notifications->proc == _NI_RESYNC)
			{
				...
				*/
				...
				if (notify_waited)
				{
					sys_msg(debug, LOG_DEBUG, "resync continuing");
				}
