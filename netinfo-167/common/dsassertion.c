...
#include "dsassertion.h"
#include <stdlib.h>
#include <string.h>

dsassertion *dsassertion_new(int32_t a, int32_t m, dsdata *k, dsdata *v)
{
	dsassertion *x;
...
dsassertion *dsassertion_retain(dsassertion *a)
{
...
void dsassertion_release(dsassertion *a)
{
	if (a == NULL) return;
...
Logic3 dsassertion_test(dsassertion *t, dsrecord *r)
{
	...
	switch (t->assertion)
	{
		...
		case DSA_LESS:
			for (i = 0; i < a->count; i++)
			{
				if ((ttype != DataTypeAny) && (a->value[i]->type != ttype))
					continue;
				if (dsdata_compare(a->value[i], t->value) < 0) return L3True;
			}
			return L3False;
		...
		case DSA_HAS_KEY:
			if (dsdata_compare(a->key, t->key) == 0) return L3True;
			return L3False;
