...
#define ReservedFileNameLength 16
 ...
 */
static void
dsstore_index_insert(dsstore *s, store_index_entry_t *x)
{
...
dsstore_write_index(dsstore *s)
{
	...
	f = dsstore_fopen(s, IndexFileName, "w");
	...
	for (i = 0; i < s->index_count; i++)
	{
		...
		if (fwrite(&x, INDEX_ENTRY_SIZE, 1, f) != 1)
			return DSStatusWriteFailed;
	}
	fclose(f);
...
static int
dsstore_server_master(dsstore *s)
{
	if ((s->flags & DSSTORE_FLAGS_SERVER_MASK) == DSSTORE_FLAGS_SERVER_MASTER)
		return 1;
	return 0;
}
 ...
 */
static dsstatus
index_recovery_read(dsstore *s, char *store)
{
	FILE *f;
	char *p;
	u_int32_t size;
	...
	if (s == NULL) return DSStatusInvalidStore;
	if (store == NULL) return DSStatusOK;

	p = strrchr(store, '.');
	if (p == NULL) return DSStatusOK;
	size = atoi(p+1);
	...
	if (is_empty) return dsstore_unlink(s, store);
...
index_recovery(dsstore *s)
{
	...
	while ((d = readdir(dp)))
	{
		...
		{
			status = index_recovery_read(s, d->d_name);
...
tree_check_child(dsstore *s, dsrecord *parent, dsrecord *child, int write_allowed)
{
	...
	if (r == NULL)
	{
		...
		{
#ifdef DEBUG
			fprintf(stderr, "Directory %u adopting orphan %u\n",
				parent->dsid, child->dsid);
#endif
			return TreeCheckOrphanedChild;
		}

#ifdef DEBUG
		fprintf(stderr, "Directory %u failed to adopt orphan %u\n",
			parent->dsid, child->dsid);
#endif
...
#ifdef DEBUG
	if (status == DSStatusOK)
		fprintf(stderr, "Directory %u released kidnapped child %u\n",
			parent->dsid, child->dsid);
	else
		fprintf(stderr, "Directory %u failed to release kidnapped child %u\n",
			parent->dsid, child->dsid);
#endif
...
#ifdef DEBUG
	if (status1 == DSStatusOK)
		fprintf(stderr, "Directory %u reunited with child %u\n",
			r->dsid, child->dsid);
	else
		fprintf(stderr, "Directory %u failed to reunite with child %u\n",
			r->dsid, child->dsid);
#endif
...
tree_check_record(dsstore *s, dsrecord *r, int write_allowed)
{
	...
	for (i = 0; i < r->sub_count; i++)
	{
		...
		if (c == NULL)
		{
			...
			{
#ifdef DEBUG
				fprintf(stderr, "Directory %u exorcised ghost child %u\n",
				r->dsid, r->sub[i]);
#endif
...
#ifdef DEBUG
			fprintf(stderr, "Directory %u failed to exorcise ghost child %u\n",
				r->dsid, r->sub[i]);
#endif
			return TreeCheckUpdateFailed;
		}
...
tree_check(dsstore *s, int write_allowed)
{
...
#ifdef DEBUG
	fprintf(stderr, "Checking directory tree");
#endif
	...
	while (tc != TreeCheckOK)
	{
		...
		if (tc == TreeCheckUpdateFailed)
		{
#ifdef DEBUG
			fprintf(stderr, "Tree update failed!\n");
#endif
...
connection_check(dsstore *s)
{
	...
	free(test);
#ifdef DEBUG
	fprintf(stderr, "Connectivity check OK\n");
#endif
	return x;
}
 ...
 * - re-creates the Index file if necessary
...
dsstore_init(dsstore *s)
{
	...
	/* If Clean file or Index file is missing, store is dirty */
	...
	{
		/* Store is clean.  Read nichecksum from the Clean file */
		f = dsstore_fopen(s, CleanFileName, "r");
		i = fread(&(s->nichecksum), sizeof(u_int32_t), 1, f);
		fclose(f);
		if (i != 1) return DSStatusInvalidStore;
		...
		if (ifp == NULL) return DSStatusInvalidStore;
...
dsstore_close(dsstore *s)
{
	 ...
	 */
	dsstore_write_index(s);

	if (dsstore_access_readwrite(s))
	{
		/* Write the Clean file */
		f = dsstore_fopen(s, CleanFileName, "w");
		i = fwrite(&(s->nichecksum), sizeof(u_int32_t), 1, f);
		fclose(f);
		if (i != 1) return DSStatusWriteFailed;
	}
...
dsstore_create_dsid(dsstore *s)
{
	...
	if (!dsstore_server_master(s)) return IndexNull;
...
dsstore_save(dsstore *s, dsrecord *r)
{
	...
	is_master = dsstore_server_master(s);

	/* Set record ID if it is a new record */
	if (r->dsid == IndexNull)
	{
		if (!is_master) return DSStatusInvalidUpdate;
	...
	{
		if (r->vers > s->max_vers) s->max_vers = r->vers;
	}
