...
static inline void *allocate_pages(unsigned bytes) {
    void *address;
    if (vm_allocate(mach_task_self(), (vm_address_t *)&address, bytes, TRUE)) address = 0;
    return (void *)address;
}
...
void stack_logging_log_stack(unsigned type, unsigned arg1, unsigned arg2, unsigned arg3, unsigned result, unsigned num_hot_to_skip) {
    ...
    if (stack_logging_the_record_list->num_records && (type == stack_logging_type_dealloc) && arg1 && ((rec-1)->type == stack_logging_type_alloc) && (arg1 == STACK_LOGGING_DISGUISE((rec-1)->address))) {
...
kern_return_t stack_logging_get_frames(task_t task, memory_reader_t reader, vm_address_t address, vm_address_t *stack_frames_buffer, unsigned max_stack_frames, unsigned *num_frames) {
    ...
    *num_frames = 0;
    // printf("In stack_logging_print_last_record_stack 0x%x\n", address);
    ...
    err = reader(task, *remote_records_address_ref, sizeof(stack_logging_record_list_t), (void **)&records);
    if (err) return err;
    // printf("stack_logging: overall num bytes = %d\n", records->overall_num_bytes);
    err = reader(task, *remote_records_address_ref, records->overall_num_bytes, (void **)&records);
    if (err) return err;
    ...
    index = records->num_records;
    while (index--) {
        stack_logging_record_t	*record = records->records + index;
        if (record->address == disguised) {
            unsigned	uniqued_stack = record->uniqued_stack;
            unsigned	uniquing_table_num_pages = records->uniquing_table_num_pages;
            while (max_stack_frames && (uniqued_stack != -1)) {
                unsigned	thisPC;
                if ((uniqued_stack * 2 + 1) * sizeof(unsigned) >= uniquing_table_num_pages * vm_page_size) {
