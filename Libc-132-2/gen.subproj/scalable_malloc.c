...
#define LARGE_THRESHOLD			(vm_page_size / 2) // at or above this use "large"

#define	SHIFT_QUANTUM			3
#define	QUANTUM				(1 << SHIFT_QUANTUM) // allocation quantum
#define MIN_BLOCK			2	// minimum size, in QUANTUM multiples
...
#define REGION_MIN_NUM_PAGES		8	// too small regions mean more regions, so 8 is a reasonable minimum
#define REGION_MAX_NUM_PAGES		(1 << (15 + SHIFT_QUANTUM - vm_page_shift)) // since we only have 16 bits for msize_t, and 1 bit is taken by THIS_FREE

#define INITIAL_NUM_REGIONS		32 // must always be at least 2 to always have 1 slot empty

#define PROTECT_SMALL			0 // 1 is too slow for normal use
...
typedef compact_range_t region_t;

typedef compact_range_t large_entry_t;

typedef vm_range_t huge_entry_t;

#define LARGE_CACHE_SIZE	10	// define hysterisis of large chunks
...
#define REGION_ADDRESS(region)		(((region).address_and_num_pages >> vm_page_shift) << vm_page_shift)
#define REGION_NUM_PAGES(region)	((region).address_and_num_pages & ((1 << vm_page_shift) - 1))
#define REGION_SIZE(region)		(REGION_NUM_PAGES(region) << vm_page_shift)
#define REGION_END(region)		(REGION_ADDRESS(region)+REGION_SIZE(region))
...
static void szone_error(szone_t *szone, const char *msg, const void *ptr) {
    ...
    if (ptr) {
        malloc_printf("*** Malloc: error for object %p: %s\n", ptr, msg);
#if DEBUG
        szone_print(szone, 1);
#endif
    } else {
        malloc_printf("*** Malloc: error: %s\n", msg);
    }
#if DEBUG
...
static void protect(szone_t *szone, vm_address_t address, vm_size_t size, unsigned protection, unsigned debug_flags) {
    ...
    if (!(debug_flags & SCALABLE_MALLOC_DONT_PROTECT_PRELUDE)) {
        ...
        if (err) malloc_printf("*** malloc: Can't protect(%x) region for prelude guard page at 0x%x\n", protection, address - vm_page_size);
    }
    if (!(debug_flags & SCALABLE_MALLOC_DONT_PROTECT_POSTLUDE)) {
        ...
        if (err) malloc_printf("*** malloc: Can't protect(%x) region for postlude guard page at 0x%x\n", protection, address + size);
    }
}

static vm_address_t allocate_pages(szone_t *szone, size_t size, unsigned debug_flags) {
    kern_return_t	err;
    vm_address_t	addr;
...
#if DEBUG
    if (ptr->checksum != (((unsigned)ptr->previous) ^ ((unsigned)ptr->next))) {
        malloc_printf("*** malloc: Incorrect check sum for freed object; object %p was probably modified after beeing freed\n", ptr);
    }
#endif
}
...
#if DEBUG
    ptr->checksum = ((unsigned)ptr->previous) ^ ((unsigned)ptr->next);
#endif
}
...
static boolean_t free_list_check(szone_t *szone, grain_t grain) {
    ...
    while (ptr) {
        ...
        if ((grain != MAX_GRAIN-1) && (msize_and_free != (msize_for_grain(szone, grain) | THIS_FREE))) {
            malloc_printf("*** Incorrect msize for grain=%d  count=%d ptr=%p msize=%d\n", grain, count, ptr, msize_and_free);
            return 0;
        }
        previous = ptr;
        ptr = ptr->next;
...
static INLINE region_t *region_for_ptr_no_lock(szone_t *szone, const void *ptr) {
    ...
    if ((unsigned)ptr - (unsigned)REGION_ADDRESS(this) < (unsigned)REGION_SIZE(this)) {
        return region;
    } else {
...
static INLINE void small_free_no_lock(szone_t *szone, region_t *region, void *ptr, msize_t msize_and_free) {
...
#if DEBUG
        if (LOG(szone,previous)) malloc_printf("In small_free_no_lock(), coalesced backwards foir %p previous=%p, msize=%d\n", ptr, previous, previous_msize);
    ...
    // next line can be enabled to help debug
...
static unsigned round_region_num_pages(size_t region_size) {
    unsigned	num_pages = (region_size + vm_page_size - 1) / vm_page_size;
    if (num_pages < REGION_MIN_NUM_PAGES) return REGION_MIN_NUM_PAGES;
    if (num_pages > REGION_MAX_NUM_PAGES) return REGION_MAX_NUM_PAGES;
    return num_pages;
}
...
static void *small_malloc_from_region_no_lock(szone_t *szone, msize_t msize) {
    ...
    if (msize_and_free != PREV_FREE && msize_and_free != 0) {
        malloc_printf("*** msize_and_free = %d\n", msize_and_free);
    ...
    num_pages = round_region_num_pages(REGION_SIZE(*last_region) * 2);
    new_address = allocate_pages(szone, num_pages << vm_page_shift, 0);
    if (!new_address) {
        // out of memory!
        return NULL;
    }
    ...
    last_region[1].address_and_num_pages = new_address | num_pages;
    szone->num_regions++;
    szone->num_bytes_free_in_last_region = (num_pages << vm_page_shift) - QUANTUM + PTR_HEADER_SIZE - (msize << SHIFT_QUANTUM);
    ...
    if (szone->num_regions >= region_capacity) {
        ...
        memmove(new_regions, szone->regions, szone->num_regions * sizeof(region_t));
...
static boolean_t szone_check_region(szone_t *szone, region_t *region) {
    ...
    msize_t	prev_free = 0;
    while ((vm_address_t)ptr < REGION_END(*region)) {
        msize_t		msize_and_free = MSIZE_FLAGS_FOR_PTR(ptr);
        if (!(msize_and_free & THIS_FREE)) {
            msize_t	msize = msize_and_free & ~PREV_FREE;
            if ((msize_and_free & PREV_FREE) != prev_free) {
                malloc_printf("*** malloc: invariant broken for %p (prev_free=%d) this msize=%d\n", ptr, prev_free, msize_and_free);
                return 0;
            }
            if (!msize) {
                int	extra = (region == szone->regions + szone->num_regions - 1) ? szone->num_bytes_free_in_last_region : QUANTUM;
                if (((unsigned)(ptr + extra)) < REGION_END(*region)) {
                    malloc_printf("*** malloc: invariant broken at region end: ptr=%p extra=%d index=%d num_regions=%d end=%p\n", ptr, extra, region - szone->regions, szone->num_regions, (void *)REGION_END(*region));
                    return 0;
                }
                break; // last encountered
            }
            if ((msize < MIN_BLOCK) && ((unsigned)ptr != REGION_END(*region) - QUANTUM)) {
                malloc_printf("*** malloc: invariant broken for %p this msize=%d\n", ptr, msize_and_free);
                return 0;
            }
            ...
            if (msize < MIN_BLOCK) {
                malloc_printf("*** malloc: invariant broken for free block %p this msize=%d\n", ptr, msize);
                return 0;
            }
            if (prev_free) {
                malloc_printf("*** malloc: invariant broken for %p (2 free in a row)\n", ptr);
                return 0;
            }
            free_list_checksum(szone, free_head);
            if (free_head->previous && !(MSIZE_FLAGS_FOR_PTR(free_head->previous) & THIS_FREE)) {
                malloc_printf("*** malloc: invariant broken for %p (previous %p is not a free pointer)\n", ptr, free_head->previous);
                return 0;
            }
            if (free_head->next && !(MSIZE_FLAGS_FOR_PTR(free_head->next) & THIS_FREE)) {
                malloc_printf("*** malloc: invariant broken for %p (next is not a free pointer)\n", ptr);
                return 0;
            }
            if (PREVIOUS_MSIZE(follower) != msize) {
                malloc_printf("*** malloc: invariant broken for free %p followed by %p in region [%x-%x] (end marker incorrect) should be %d; in fact %d\n", ptr, follower, REGION_ADDRESS(*region), REGION_END(*region), msize, PREVIOUS_MSIZE(follower));
                return 0;
            }
            ptr = follower;
            prev_free = PREV_FREE;
...
static kern_return_t small_in_use_enumerator(task_t task, void *context, unsigned type_mask, vm_address_t region_address, unsigned num_regions, memory_reader_t reader, vm_range_recorder_t recorder) {
    ...
    while (index < num_regions) {
        ...
        vm_range_t	range = {REGION_ADDRESS(region), REGION_SIZE(region)};
...
static INLINE void *small_malloc_from_free_list(szone_t *szone, msize_t msize, boolean_t *locked) {
    ...
    while (free_list < szone->free_list + MAX_GRAIN - 1) {
        // try bigger grains
        ptr = *free_list;
        if (ptr) {
...
static INLINE large_entry_t *large_entries_alloc_no_lock(szone_t *szone, unsigned num) {
    ...
    if (is_vm_allocation) {
        return (void *)allocate_pages(szone, round_page(size), 0);
    } else {
...
static vm_range_t large_free_no_lock(szone_t *szone, large_entry_t *entry) {
    ...
    if (large_entry_for_pointer_no_lock(szone, (void *)range.address)) {
        malloc_printf("*** Freed entry 0x%x still in use; num_large_entries=%d\n", range.address, szone->num_large_entries);
...
static void huge_entry_append(szone_t *szone, huge_entry_t huge) {
    ...
    while (1) {
        ...
        new_huge_entries = szone_malloc_should_clear(szone, (num_huge_entries + 1) * sizeof(huge_entry_t), 0);
        SZONE_LOCK(szone);
        if (num_huge_entries == szone->num_huge_entries) {
            ...
            if (num_huge_entries) memmove(new_huge_entries, old_huge_entries, num_huge_entries * sizeof(huge_entry_t));
...
static void *large_and_huge_malloc(szone_t *szone, unsigned num_pages, boolean_t cleared_requested) {
    ...
    if (num_pages >= (1 << vm_page_shift)) {
        ...
        addr = allocate_pages(szone, huge.size, szone->debug_flags);
        if (!addr) return NULL;
        huge.address = addr;
        ...
        if (!cleared_requested) {
            ...
            vm_range_t	*range_to_use = first_range_greater_or_equal(szone->large_to_deallocate, LARGE_CACHE_SIZE, size + guard_pages);
            if (range_to_use) {
                // that one will do!
        ...
        if (!addr) {
            ...
            SZONE_UNLOCK(szone);
            addr = allocate_pages(szone, size, szone->debug_flags);
...
static void szone_free(szone_t *szone, void *ptr) {
    ...
    if (((unsigned)ptr) & (vm_page_size - 1)) {
        szone_error(szone, "Non-page aligned pointer being freed", ptr);
        return;
    }
    SZONE_LOCK(szone);
    ...
    if (entry) {
        ...
        if (large_entry_for_pointer_no_lock(szone, ptr)) {
            malloc_printf("*** Just after freeing 0x%x still in use num_large_entries=%d\n", ptr, szone->num_large_entries);
            large_cache_debug_print(szone);
            large_debug_print(szone);
            sleep(3600);
        }
    ...
    if (vm_range_to_deallocate.address) {
        ...
        if (large_entry_for_pointer_no_lock(szone, (void *)vm_range_to_deallocate.address)) {
            malloc_printf("*** Invariant broken: 0x%x still in use num_large_entries=%d\n", vm_range_to_deallocate.address, szone->num_large_entries);
            large_cache_debug_print(szone);
            large_debug_print(szone);
            sleep(3600);
        }
...
static void *szone_realloc(szone_t *szone, void *ptr, size_t new_size) {
    ...
    if (!((szone->debug_flags & SCALABLE_MALLOC_ADD_GUARD_PAGES) && PROTECT_SMALL) && (new_size < LARGE_THRESHOLD)) {
        ...
        msize_t	next_msize;
        // If the next block is free, we coalesce
        // Note that we avoid next_block on page boundaries, which is a simple way to avoid going beyong our current region
#if DEBUG
        ...
        if ((next_msize_and_free & THIS_FREE) && ((vm_address_t)next_block & (vm_page_size - 1)) && next_msize && (old_size + (next_msize << SHIFT_QUANTUM) >= new_size)) {
            msize_t		coalesced_msize = (new_size - old_size + QUANTUM - 1) >> SHIFT_QUANTUM;
            msize_t		leftover_msize = next_msize - coalesced_msize;
            // malloc_printf("Realloc in place for %p;  current msize=%d next_msize=%d\n", ptr, MSIZE_FLAGS_FOR_PTR(ptr), next_msize);
            SZONE_LOCK(szone);
            if (MSIZE_FLAGS_FOR_PTR(next_block) != next_msize) {
                SZONE_UNLOCK(szone);
                // malloc_printf("Optimistic locking failed for %p\n", ptr);
            } else {
                free_list_remove_ptr(szone, next_block, next_msize);
                ...
                MSIZE_FLAGS_FOR_PTR(ptr) += coalesced_msize;
                MSIZE_FLAGS_FOR_PTR(FOLLOWING_PTR(ptr, MSIZE_FLAGS_FOR_PTR(ptr) & ~PREV_FREE)) &= ~ PREV_FREE;
#if DEBUG
...
static void szone_destroy(szone_t *szone) {
    unsigned	index;
#if NEW_MALLOC
    malloc_zone_unregister(&szone->basic_zone);
#endif
    ...
    while (index--) { // we skip the first region, that is the zone itself
        ...
        deallocate_pages(szone, REGION_ADDRESS(region), REGION_SIZE(region), 0);
    }
}
...
static boolean_t szone_check_all(szone_t *szone, const char *function) {
    ...
    if (! (++szone_check_counter % 10000)) {
        malloc_printf("At szone_check_all(%s) %d\n", function, szone_check_counter);
    }
    ...
    while (index < szone->num_regions) {
        ...
        if (!szone_check_region(szone, region)) {
            ...
            malloc_printf("*** Region %d incorrect szone_check_all(%s) counter=%d\n", index-1, function, szone_check_counter);
    ...
    while (index < MAX_GRAIN) {
        if (! free_list_check(szone, index)) {
            SZONE_UNLOCK(szone);
            szone->debug_flags &= ~ CHECK_REGIONS;
            malloc_printf("*** Free list incorrect (grain=%d) szone_check_all(%s) %d\n", index, function, szone_check_counter);
...
static void szone_print(szone_t *szone, boolean_t verbose) {
    ...
    while (index < szone->num_regions) {
        ...
        malloc_printf("Region [0x%x-0x%x, %dKB] \tIn_use=%d ", REGION_ADDRESS(*region), REGION_END(*region), (int)REGION_SIZE(*region) / 1024, in_use);
        if (verbose) {
            malloc_printf("\n\tSizes in use: ");
            while (ci < 512) {
...
malloc_zone_t *create_scalable_zone(size_t initial_size, unsigned debug_flags) {
    ...
    if (!vm_page_shift) {
        ...
        if (MIN_BLOCK * QUANTUM < sizeof(free_list_t) + PTR_HEADER_SIZE) {
            malloc_printf("*** malloc: inconsistant parameters\n");
        }
    }
    num_pages = round_region_num_pages(initial_size);
    addr = allocate_pages(NULL, num_pages << vm_page_shift, 0);
    if (!addr) return NULL;
    ...
    msize_used += msize; szone->num_small_objects++;
    szone->regions[0].address_and_num_pages = addr | num_pages;
    szone->num_regions = 1;
    szone->num_bytes_free_in_last_region = (num_pages << vm_page_shift) - ((msize_used+1) << SHIFT_QUANTUM) + PTR_HEADER_SIZE;
    CHECK(szone, __PRETTY_FUNCTION__);
#if NEW_MALLOC
    malloc_zone_register(&szone->basic_zone);
#endif
...
void scalable_zone_info(malloc_zone_t *zone, unsigned *info_to_fill, unsigned count) {
    ...
    memmove(info_to_fill, info, sizeof(unsigned)*count);
}
