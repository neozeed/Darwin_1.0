...
#define PHASE_OUT_OLD_MALLOC	0	// over time remove usage of this macro

#define INITIAL_ZONES	16  // After this number, we well reallocate for new zones
...
/* The following constants are compatible with a pre-existing logging tool */
...
malloc_zone_t *malloc_create_zone(vm_size_t start_size, unsigned flags) {
    if (!malloc_num_zones) {
        ...
        if (getenv("MallocGuardEdges")) {
            ...
            malloc_printf("malloc: protecting edges\n");
            if (getenv("MallocDoNotProtectPrelude")) {
                malloc_debug_flags |= SCALABLE_MALLOC_DONT_PROTECT_PRELUDE;
                malloc_printf("malloc: ... but not protecting prelude guard page\n");
            }
            if (getenv("MallocDoNotProtectPostlude")) {
                malloc_debug_flags |= SCALABLE_MALLOC_DONT_PROTECT_POSTLUDE;
                malloc_printf("malloc: ... but not protecting postlude guard page\n");
            }
        }
        ...
        if (flag) {
            ...
            malloc_printf("malloc: recording stacks using recorder %p\n", malloc_logger);
        }
        if (getenv("MallocHelp")) {
            malloc_printf("malloc: environment variables that can be set for debug:\n"
    ...
    return create_scalable_zone(start_size, malloc_debug_flags);
}
...
void *malloc_zone_malloc(malloc_zone_t *zone, size_t size) {
    void	*ptr;
    if ((unsigned)size >= (unsigned)~vm_page_size) {
        /* Probably a programming error */
        fprintf(stderr, "*** malloc: argument too large: %d\n", (unsigned)size);
        return NULL;
    }
...
void *malloc_zone_calloc(malloc_zone_t *zone, size_t num_items, size_t size) {
    void	*ptr;
#if 1
#warning malloc_zone_calloc DISABLED FOR FOUNDATIONs COPY OF MALLOC
    if (!zone->destroy) {
        static boolean_t warned = 0;
        if (!warned) {
            printf("*** malloc: disabled malloc_zone_calloc because of zone malformed %p\n", zone);
            warned = 1;
        }
        size = num_items * size;
    ...
    if (malloc_logger) malloc_logger(MALLOC_LOG_TYPE_ALLOCATE | MALLOC_LOG_TYPE_HAS_ZONE | MALLOC_LOG_TYPE_CLEARED, (unsigned)zone, size, 0, (unsigned)ptr, 0);
    return ptr;
}
...
void malloc_zone_register(malloc_zone_t *zone) {
    ...
    if (malloc_num_zones >= INITIAL_ZONES) {
        ...
        MALLOC_LOCK();
        if (copy) memmove(zones, malloc_zones, malloc_num_zones * sizeof(malloc_zone_t *));
        malloc_zones = zones;
    }
...
void malloc_zone_unregister(malloc_zone_t *z) {
    ...
    MALLOC_UNLOCK();
    fprintf(stderr, "*** malloc: malloc_zone_unregister() failed for %p\n", z);
}
...
void free(void *ptr) {
    ...
    if (zone) {
        ...
        fprintf(stderr, "*** malloc: Deallocation of freed pointer: %p\n", ptr);
    }
}
...
void *valloc(size_t size) {
    ...
    if (size < vm_page_size) size = vm_page_size;
    return malloc_zone_calloc(zone, 1, size);
}
...
kern_return_t malloc_get_all_zones(task_t task, memory_reader_t reader, vm_address_t **addresses, unsigned *count) {
    ...
    err = reader(task, (vm_address_t)&malloc_zones, sizeof(void *), (void **)&zones_address_ref);
    // printf("Read malloc_zones=%p\n", *zones_address_ref);
    if (err) {
        fprintf(stderr, "*** malloc_get_all_zones: error reading zones_address at 0x%x\n", (unsigned)&malloc_zones);
        return err;
    }
    zones_address = *zones_address_ref;
    // printf("Reading num_zones at address %p\n", &malloc_num_zones);
    err = reader(task, (vm_address_t)&malloc_num_zones, sizeof(unsigned), (void **)&num_zones_ref);
    if (err) {
        fprintf(stderr, "*** malloc_get_all_zones: error reading num_zones at 0x%x\n", (unsigned)&malloc_num_zones);
        return err;
    }
    ...
    *count = num_zones;
    ...
    if (err) {
        fprintf(stderr, "*** malloc_get_all_zones: error reading zones at 0x%x\n", (unsigned)&zones_address);
        return err;
    }
...
void set_malloc_singlethreaded(boolean_t single) {
    ...
    if (!warned) {
#if PHASE_OUT_OLD_MALLOC
        fprintf(stderr, "*** malloc: OBSOLETE: set_malloc_singlethreaded(%d)\n", single);
#endif
        warned = 1;
    }
}
...
void malloc_singlethreaded() {
    static boolean_t warned = 0;
    if (!warned) {
        fprintf(stderr, "*** malloc: OBSOLETE: malloc_singlethreaded()\n");
        warned = 1;
    }
}

int malloc_debug (int level) {
    fprintf(stderr, "*** malloc: NOT IMPLEMENTED: malloc_debug()\n");
    return 0;
}

unsigned _malloc_get_all_regions(void **regions) {
    fprintf(stderr, "*** malloc: OBSOLETE: _malloc_get_all_regions()\n");
    return 0;
}
...
static size_t exo_zone_size(exo_zone_t *zone, const void *ptr) {
    ...
    fprintf(stderr, "*** malloc: exo_zone_size unknown flags()\n");
    return 0;
}
...
static void malloc_freezedry_recorder(task_t task, void *rr, unsigned type, vm_range_t *ranges, unsigned num) {
    ...
    if (recorded->num == MAX_RANGES) fprintf(stderr, "*** malloc_freezedry_recorder: exceeding capacity\n");
}
