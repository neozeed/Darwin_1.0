...
#ifdef LOADABLE_FS_SUPPORT
#import <kernserv/loadable_fs.h>
#endif /* LOADABLE_FS_SUPPORT */
...
static int AttemptVolFSMount( void );
...
GetBTreeNodeInfo(int fd, off_t btreeOffset, UInt32 *nodeSize, UInt32 *firstLeafNode)
{
	...
	if ( bTreeHeaderPtr->node.kind != kBTHeaderNode ) {
		result = FSUR_IO_FAIL;
		printf("hfs.util: ERROR: bTreeHeaderPtr->node.kind != kBTHeaderNode\n");
		goto free;
	}
...
static ssize_t readAt( int fd, void * bufPtr, off_t offset, ssize_t length )
{
	...
	dprintf(("DKIOCBLKSIZE %s failed, %s\n", dev,
		strerror(errno)));
	result = FSUR_IO_FAIL;
	goto Return;
    }
    ...
    if ( readResult != rawLength ) {
        dprintf(("hfs.util: readAt: read(%d,$%x,$%x): $%x, errno=%d: %s\n",fd,(u_int32_t)bufPtr,rawLength,readResult,errno,strerror(errno)));
        result = FSUR_IO_FAIL;
        goto Return;
    }
...
static int AttemptVolFSMount( void ) {
    ...
    if (pid == 0) {
        ...
        /* IF WE ARE HERE, WE WERE UNSUCCESFULL */
        dprintf(("hfs.util: ERROR: AttemptVolFSMount: execl('%s') returned %d\n", VOLFS_MOUNT_COMMAND, execlResult));
        result = FSUR_IO_FAIL;
        goto Return;
    }
