...
dav_error *dav_new_error(pool *p, int status, int error_id, const char *desc)
{
    ...
    ap_log_error(APLOG_MARK, APLOG_ERR|APLOG_NOERRNO, NULL,
		 "DEBUG:: %d %d %s",
		 status, error_id, desc ? desc : "(no desc)");
#endif
...
void dav_check_bufsize(pool * p, dav_buffer *pbuf, int extra_needed)
{
...
void dav_set_bufsize(pool * p, dav_buffer *pbuf, int size)
{
...
void dav_buffer_place_mem(pool *p, dav_buffer *pbuf, const void *mem, int amt,
			  int pad)
{
...
static int dav_elem_size(const dav_xml_elem *elem, int style,
	...
	if (style == DAV_X2T_FULL_NS_LANG) {
	    ...
	    */
		...
		size += (9 + DAV_NS_LEN(i) + 2 +
			 strlen(DAV_GET_NS_URI(namespaces, i)) + 1);
	    }
		  ...
		  int *psize)
{
    ...
    int size = dav_elem_size(elem, style, namespaces, ns_map) + 1;
    char *s = ap_palloc(p, size);

    dav_write_elem(s, elem, style, namespaces, ns_map);
    s[size - 1] = '\0';
...
static dav_error * dav_process_if_header(request_rec *r, dav_if_header **p_ih)
{
    ...
    *p_ih = NULL;
	...
	case '<':
	    ...
	    len = strlen(parsed_uri.path);
	    if (len > 1 && parsed_uri.path[len - 1] == '/')
		parsed_uri.path[len - 1] = '\0';
	...
	case '(':
	    ...
	    if ((ih = dav_add_if_resource(r->pool, ih, uri)) == NULL) {
		/* ### dav_add_if_resource() should return an error for us! */
		/* ### don't log an error. just use the err struct! */
		ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, r,
			      "Internal server error parsing If-header.");
		return dav_new_error(r->pool, HTTP_BAD_REQUEST,
				     DAV_ERR_IF_PARSE,
		...
		default:
		    /* ### don't log an error. just use the err struct! */
		    ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, r,
				  "Invalid If-header: Unexpected character encountered.");
		    return dav_new_error(r->pool, HTTP_BAD_REQUEST,
					 DAV_ERR_IF_UNK_CHAR,
					 "Invalid \"If:\" header: Unexpected "
	...
	default:
	    /* ### don't log an error. just use the err struct! */
	    ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, r,
			  "Invalid If-header: Unexpected character encountered \"%c\".", *str);
	    return dav_new_error(r->pool, HTTP_BAD_REQUEST,
				 DAV_ERR_IF_UNK_CHAR,
				 "Invalid \"If:\" header: Unexpected "
				 "character encountered.");
 ...
 */
static dav_error * dav_validate_resource_state(request_rec *r,
                                               ...
                                               int flags)
{
    ...
    /* ### it would be nice to remove this dependency on "r" */
    const dav_hooks_liveprop *liveprop_hooks = DAV_GET_HOOKS_LIVEPROP(r);
    etag = (*liveprop_hooks->getetag)(resource);
    ...
    */
	...
	if (lock_list != NULL) {
	    return dav_new_error(r->pool, HTTP_LOCKED, 0, 
    ...
    */
	...
	if (ifhdr_scan->uri != NULL && strcmp(uri, ifhdr_scan->uri) != 0) {
	    /*
	...
	for (state_list = ifhdr_scan->state;
	    ...
	    case dav_if_etag:
		if ((state_list->condition == DAV_IF_COND_NOT    && !strcmp(state_list->etag, etag)) ||
		    (state_list->condition == DAV_IF_COND_NORMAL &&  strcmp(state_list->etag, etag))) {
		    /*
		    ...
		    */
		    ...
		    return dav_new_error(r->pool, HTTP_PRECONDITION_FAILED,
					 0,
					 "An entity-tag was specified in "
					 "the \"If:\" header, but the "
					 "resource's ETag does not match "
		...
		if (lockdb == NULL) {
		    ...
		    */
		    ...
		    return dav_new_error(r->pool, HTTP_PRECONDITION_FAILED,
					 0,
					 "A State-token was supplied in "
					 "an \"If:\" header, but a lock "
					 "database is not available for "
					 "to provide the required lock.");
#endif
		}
		...
		for (lock = lock_list; lock != NULL; lock = lock->next) {
			...
			*/
			...
			return dav_new_error(r->pool,
					     ...
					     0,
					     "A State-token was supplied "
					     "in an \"If:\" header with a "
					     "\"Not\" condition. The "
					     "State-token was found in "
					     "the locks on this resource, "
					     "thereby failing the "
					     "precondition test.");
#endif
		    }
		    ...
		    **
		    ** Break out of the lock scan -- we just need to find a
		    ** match (actually, there shouldn't be any other matches
		    ** in the lock list).
		    */
		    num_matched = 1;
		    break;
		...
		if (num_matched == 0
		    ...
		    */
		    ...
		    return dav_new_error(r->pool, HTTP_PRECONDITION_FAILED,
					 0,
					 "A State-token was supplied in "
					 "an \"If:\" header, but it was "
					 "not found in the locks on this "
					 "resource, thereby failing the "
					 "precondition test.");
#endif
		}
	...
	** may have succeeded.
	**
	** If we saw one of the resource's locktoken, then we're done.
    ...
    */
	...
	for (state_list = ifhdr_scan->state;
		...
		for (lock = lock_list; lock != NULL; lock = lock->next) {
		    ...
		    if (!(*locks_hooks->compare_locktoken)(state_list->locktoken, lock->locktoken)) {
			/*
			** We found a match! We're set... we have a
			** matching state list, and the If: header
...
{
    dav_error *err;
    dav_get_props_result fail_dep_result = { 0 };
    dav_text *empty_lock_discovery;
    const dav_hooks_repository *repos_hooks = DAV_GET_HOOKS_REPOSITORY(ctx->r);

    if ((err = dav_validate_resource_state(ctx->r, ctx->resource,
					   ctx->uri.buf, ctx->lockdb,
					   ctx->if_header, ctx->flags)) == NULL) {
    ...
    /* If the request itself failed, just return error, not multistatus. */
    if ((*repos_hooks->is_same_resource)(ctx->resource, ctx->root)) {
	/* ### maybe push a higher-level description? */
	return err;
    }

    /* ### BEGIN
    ** ### This section should move out to the caller and be inserted
    ** ### just once. Further, the 424 on the lockdiscovery should
    ** ### only occur for the LOCK method. (### verify re: other methods)
    */
    empty_lock_discovery = ap_pcalloc(ctx->pool, sizeof(dav_text));
    empty_lock_discovery->text =
	"<D:propstat>" DEBUG_CR
	"<D:prop><D:lockdiscovery/></D:prop>" DEBUG_CR
	"<D:status>HTTP/1.1 424 Failed Dependency</D:status>" DEBUG_CR
	"</D:propstat>" DEBUG_CR;

    fail_dep_result.propstats = empty_lock_discovery;
    dav_add_response(ctx, ctx->r->uri, 0, &fail_dep_result);
    /* ### END */

    dav_add_response(ctx, ctx->uri.buf, err->status, NULL);

    /* ### RFC 2518 8.10.4 says 409 (Conflict),
     * ### but 8.10.10 shows 207 (Multistatus). */
    /* ### we should return NULL so that we can scan/test ALL resources */
    /* ### returning NULL should happen after the above code move */
    return dav_new_error(ctx->pool, HTTP_CONFLICT, 0, NULL);
}
                                 ...
                                 dav_lockdb *lockdb)
{
    ...
    const dav_hooks_locks *locks_hooks = DAV_GET_HOOKS_LOCKS(r);
    const dav_hooks_repository *repos_hooks = DAV_GET_HOOKS_REPOSITORY(r);
    ...
    */
    if (lockdb == NULL) {
        if (locks_hooks != NULL) {
	        ...
	        /* ### should map result to something nice */
	        ap_log_rerror(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, r,
            ...
            return dav_new_error(r->pool, HTTP_INTERNAL_SERVER_ERROR, 0,
                "Resource validation failed because no lock hooks were found.");
        }
    }

    /* (1) Validate r->filename @ specified depth */
	...
	/*
	** ### actually, it would be bad form to call this function with
	** ### a depth>0 and not provide a ptr to a response.
	*/
	if (response != NULL)
	    *response = ctx.response;
    }
    else {
	err = dav_validate_resource_state(r, resource, r->uri,
					  lockdb, if_header, flags);
    }

    /* (2)  Validate parent of r->filename if we should be checking the parent */

    if (err == NULL && (flags & DAV_VALIDATE_PARENT)) {
        dav_resource *parent_resource = (*repos_hooks->get_parent_resource)(resource);
	const char *parent_uri  = (*repos_hooks->get_uri)(parent_resource);

	err = dav_validate_resource_state(r, parent_resource, parent_uri,
					  lockdb, if_header, flags);
    }

    if (lock_db_opened_locally && locks_hooks != NULL)
					  ...
					  int *parent_was_writable)
{
	...
	parent = (*repos_hooks->get_parent_resource)(resource);
					   ...
					   int parent_was_writable)
{
    ...
    if (resource != NULL) {
	    ...
	    /* ### should we do anything with the response? */
            if ((err = (*repos_hooks->remove_resource)(resource,
						       &response)) != NULL) {
	        body = ap_psprintf(r->pool,
