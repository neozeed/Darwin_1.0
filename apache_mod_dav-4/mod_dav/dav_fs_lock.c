...
**    DIRECT LOCK:    [DAV_LOCK_DIRECT, 2 chars, 1 int, time_t, uuid_t,
**                     2 char []s]
**       A length-independent format of the lock_discovery structure is stored
**
**    INDIRECT LOCK:  [DAV_LOCK_INDIRECT, uuid_t, int size, char key[]]
**       The key to the collection lock that resulted in this indirect lock
*/
...
/* ack. forward declare. */
static int dav_fs_remove_locknull_member(pool *p, const char *filename, dav_buffer *pbuf);
...
#define dav_size_indirect(a)	(1 + sizeof(uuid_t) + sizeof(int) + \
				 (a)->key.dsize)
				   ...
				   const dav_locktoken *locktoken)
{
	...
	(void)dav_create_opaquelocktoken(dav_get_uuid_state(lockdb->info->r),
					 &comb->token.uuid);
    }
    else {
				      ...
				      dav_lockdb **lockdb)
{
    ...
    comb->priv.lockdb_path = dav_get_lock_database(r);
...
{
    return time(NULL) >= expires && expires != DAV_TIMEOUT_INFINITE;
}
	...
	*ptr++ = DAV_LOCK_DIRECT;	/* Direct lock - lock_discovery struct follows */
	...
	if (dp->owner == NULL) {
	    *ptr++ = '\0';
	    ...
	    memcpy(ptr, dp->owner, strlen(dp->owner) + 1);	/* Then the two var-length strings */
	    ptr += strlen(dp->owner) + 1;
	}

	/* ### the old "uri" field... keep until a format change */
	*ptr++ = '\0';
					   ...
					   dav_lock_indirect **indirect)
{
    ...
    */
    ...
    (void) (*dav_hooks_db_dbm.fetch)(lockdb->info->db, key, &val);
	...
	case DAV_LOCK_DIRECT:
	    ...
	    memcpy(dp, val.dptr + i, sizeof(dp->f));
	    i += sizeof(dp->f);
            dp->locktoken = ap_palloc(lockdb->info->pool, sizeof(*dp->locktoken));
            memcpy(dp->locktoken, val.dptr + i, sizeof(*dp->locktoken));
            i += sizeof(*dp->locktoken);
	    if (*(val.dptr + i) == '\0') {
		++i;
	    }
	    else {
		dp->owner = ap_pstrdup(lockdb->info->pool, val.dptr + i);
		i += strlen(dp->owner) + 1;
	    }

	    /* ### skip over the old "uri" field... it is no longer used */
	    i += strlen(val.dptr + i) + 1;
		...
		*direct = dp;
		...
		if (*key.dptr == DAV_TYPE_FNAME) {
			...
			dav_fs_remove_locknull_member(lockdb->info->pool, fname, &buf);
		    }
		}
	    }
	    break;
	...
	case DAV_LOCK_INDIRECT:
	    ...
	    memcpy(ip->locktoken, val.dptr + i, sizeof(*ip->locktoken));
	    i += sizeof(*ip->locktoken);
	    ip->key.dsize = *((int *) (val.dptr + i));	/* length of datum */
	    i += sizeof(ip->key.dsize);
	    ip->key.dptr = ap_palloc(lockdb->info->pool, ip->key.dsize); 
	    memcpy(ip->key.dptr, val.dptr + i, ip->key.dsize);
	    i += ip->key.dsize;
	...
	default:
	    --i;
	    /* ### don't log this... just return an error */
	    ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, NULL,
			 "Corrupt lock db (offset %i, [%x])", i, *(val.dptr + i));
...
**    Returns DAV_LOCK_*.
*/
static int dav_fs_load_locknull_list(pool *p, const char *dirpath,
			             dav_buffer *pbuf) 
{
    struct stat finfo;
    int f;
    int result = DAV_LOCK_ERROR;
    ...
    if ((f = open(pbuf->buf, O_RDONLY | O_BINARY)) == -1) {
	return DAV_LOCK_OK;
    }

    if (fstat(f, &finfo) == -1) {
	ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, NULL,
		     "Opened but could not stat file %s", pbuf->buf);
	goto loaderror;
    }

    dav_set_bufsize(p, pbuf, finfo.st_size);
    if (read(f, pbuf->buf, finfo.st_size) != finfo.st_size) {
	ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, NULL,
		     "Failure reading locknull file for %s", dirpath);
...
**    Returns DAV_LOCK_*.
*/
static int dav_fs_save_locknull_list(pool *p, const char *dirpath,
			             dav_buffer *pbuf)
{
    const char *pathname;
    int f;
    int result = DAV_LOCK_OK;;
	...
	if (remove(pathname) != 0) {
	    ...
	    return DAV_LOCK_ERROR;
	}
	return DAV_LOCK_OK;
    }

    if ((f = open(pathname, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY,
                  DAV_FS_MODE_FILE)) == -1) {
	ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, NULL,
		      "Error opening %s for writing", pathname);
	return DAV_LOCK_ERROR;
    }

    if (write(f, pbuf->buf, pbuf->cur_len) != pbuf->cur_len) {
	ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, NULL,
	      "Error writing %i bytes to %s", pbuf->cur_len, pathname);
	result = DAV_LOCK_ERROR;
    }
...
**    for directory path.  Returns DAV_LOCK_*.
*/
static int dav_fs_remove_locknull_member(pool *p, const char *filename,
					 dav_buffer *pbuf)
{
    int result;
    int len;
	...
	*fname++ = '\0';
    ...
    if ((result = dav_fs_load_locknull_list(p, dirpath, pbuf)) != DAV_LOCK_OK)
	return result;
    ...
    if (dirty) {
	if ((result = dav_fs_save_locknull_list(p, dirpath, pbuf)) != DAV_LOCK_OK)
	    return result;
    }
...
int dav_fs_get_locknull_members(
    ...
    const dav_resource *resource)
{
    ...
    if ((result = dav_fs_load_locknull_list(p, dirpath, &buf)) != DAV_LOCK_OK) {
	/* ### fix this error reporting */
        return dav_new_error(p, HTTP_INTERNAL_SERVER_ERROR, 0,
			     "Could not load .locknull file.");
    }

    dav_buffer_append(p, &buf, fname);
    buf.cur_len++;	/* we want the null-term here */

    if ((result = dav_fs_save_locknull_list(p, dirpath, &buf)) != DAV_LOCK_OK) {
	/* ### fix this error reporting */
        return dav_new_error(p, HTTP_INTERNAL_SERVER_ERROR, 0,
    ...
    const dav_resource *resource)
{
    ...
    if ((result = dav_fs_remove_locknull_member(p, pathname, &buf)) != DAV_LOCK_OK) {
	/* ### map this result... */
	...
	*/
	...
	(void)dav_fs_save_lock_record(lockdb, key, NULL, NULL);
	key = dav_fs_build_key(p, resource);
	(void)dav_fs_save_lock_record(lockdb, key, ld, id);
    }
#endif
				      ...
				      const dav_locktoken *locktoken)
{
    ...
    (void)dav_fs_save_lock_record(lockdb, key, dh, ih);
    ...
    */
	...
	&& (result = dav_fs_remove_locknull_member(lockdb->info->pool,
