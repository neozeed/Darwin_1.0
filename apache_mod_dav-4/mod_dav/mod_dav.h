...
typedef struct
{
    int alloc_len;	/* how much has been allocated */
    int cur_len;	/* how much is currently being used */
...
#define DAV_IF_STATE_VALID	0
#define DAV_IF_STATE_INVALID	1
...
typedef struct dav_xml_elem
{
    ...
    int annotation;			/* info about this element */
    /* Note: specific annotations should be [1,9999]. none yet. */
...
#define DAV_GET_NS_URI(ns_array, i)	(((const char **)(ns_array)->elts)[i])
...
dav_error * dav_validate_request(request_rec *r, dav_resource *resource,
				 ...
				 dav_response **response, int flags,
                                 dav_lockdb *lockdb);
#define DAV_VALIDATE_PARENT   0x40
#define DAV_VALIDATE_RESOURCE 0x80
...
struct dav_hooks_locks
{
     ...
     * (as returned by dav_get_lock_database()) to decide where to place
...
/* These should be not be intermixed with HTTP_ status codes */
#define DAV_LOCK_OK		0
#define DAV_LOCK_ERROR		1	/* arbitrary error */

/* NOTE: must be disjoint from DAV_LOCK_* */
...
typedef struct
{
     ...
     * The resources need not exist. NULL is only returned for some kind of
     * fatal error.
     */
    ...
    /* Get the URI for a given resource. This should always be defined,
     * since the resource descriptor must have been generated from a
     * request URI.
     *
     * ### this can return NULL if called on walker_ctx.resource or .res2
     */
    const char * (*get_uri)(
        const dav_resource *resource
    );
    ...
    *
    ...
    /* Create and open a file descriptor for the given (file) resource.
     * If the file exists, it is overwritten.
     *
     * Returns NULL if the file could not be created/opened.
     */
    /*
    ** ### This should return something in addition to a FILE *. That
    ** ### will allow the provider to store context across the create/close
    ** ### and handle the resulting file when it is closed (e.g. transfer
    ** ### into a database and delete the temp file).
    */
    FILE * (*create_file)(
        const dav_resource *resource
    );

    /* Close the file which was created by create_file() for the given resource.
     * Returns != 0 => error closing file (e.g. out of disk space) */
    int (*close_file)(
        const dav_resource *resource,
        FILE *f
    );
     ...
     *
     * Result == 0 if the collection was created successfully. Also, the
