...
static int dav_insert_ns(dav_xml_ctx *ctx, const char *ns_uri)
{
    array_header *namespaces = ctx->doc->namespaces;
    int i;
    const char **pelt;

    for (i = namespaces->nelts; i--;) {
	if (strcmp(ns_uri, DAV_GET_NS_URI(namespaces, i)) == 0)
	    return i;
    }

    pelt = ap_push_array(namespaces);
    *pelt = ns_uri;		/* assume ns_uri is const or in a pool */
    return namespaces->nelts - 1;
}
...
static void dav_start_handler(void *userdata, const char *name, const char **attrs)
{
	...
	if (strncmp(attr->name, "xmlns", 5) == 0) {
	    ...
	    ns_scope->ns = dav_insert_ns(ctx, quoted);
...
int dav_parse_input(request_rec * r, dav_xml_doc **pdoc)
{
	...
	*pdoc = NULL;
	...
	/* ### if len == -1, then an error occurred. what to do? */
