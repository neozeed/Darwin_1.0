...
{
    const dav_hooks_locks *hooks = ctx->lockdb->hooks;
    ...
    if ((err = (*hooks->append_locks)(ctx->lockdb, ctx->resource, 1,
				      ctx->lock)) != NULL) {
...
*/
dav_error * dav_get_direct_resource(pool *p,
				    dav_lockdb *lockdb,
				    ...
				    const dav_resource **direct_resource)
{
	...
	*/
	...
	resource = (*repos_hooks->get_parent_resource)(resource);
    }
	       ...
	       const dav_locktoken *locktoken)
{
    ...
    if (locktoken != NULL
	&& (err = dav_get_direct_resource(r->pool, lockdb, repos_hooks,
					  locktoken, resource,
					  &lock_resource)) != NULL) {
	/* ### add a higher-level desc? */
	/* ### should return err! */
	...
	dav_buffer_init(r->pool, &ctx.uri, (*repos_hooks->get_uri)(lock_resource));
...
static dav_error * dav_inherit_walker(dav_walker_ctx *ctx, int calltype)
{
	...
	repos_hooks = DAV_GET_HOOKS_REPOSITORY(ctx->r);
	if ((*repos_hooks->is_same_resource)(ctx->resource, ctx->root))
	    return NULL;
    }
...
**
**    Returns DAV_LOCK_* on error (which are disjoint from DAV_RESOURCE_*).
*/
