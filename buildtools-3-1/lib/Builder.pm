...
sub build () 
{
  ...
  if (($target eq 'all') || ($target eq 'headers')) {
    ...
    &printcmd ('mkdir', '-p', $params->{'BUILDROOT'}."/dev");
    system    ('mkdir', '-p', $params->{'BUILDROOT'}."/dev");
    &printcmd ('mount_devfs', 'devfs', $params->{'BUILDROOT'}."/dev");
    system    ('mount_devfs', 'devfs', $params->{'BUILDROOT'}."/dev");

    my @installhdrscommand = &buildcmd ($package, $params, $bparams, 'installhdrs');
    $ENV{'UNAME_SYSNAME'} = 'Darwin';
    $ENV{'UNAME_RELEASE'} = '1.0';
    $ENV{'PATH'} = '/sbin:/usr/sbin:/bin:/usr/bin:/usr/local/bin';
    print 'UNAME_SYSNAME=Darwin UNAME_RELEASE=1.0 PATH=/sbin:/usr/sbin:/bin:/usr/bin:/usr/local/bin ';
    &printcmd (@installhdrscommand);
    system (@installhdrscommand);
    my $errstat = $?;

    &printcmd ('umount', $params->{'BUILDROOT'}."/dev");
    system    ('umount', $params->{'BUILDROOT'}."/dev");

    if ($errstat != 0) { die (&checkret ($?)); }
    print "\n";
  }
