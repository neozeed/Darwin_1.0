...
char	*progname;
char	gVolumeName[kHFSPlusMaxFileNameChars] = {"untitled"};
	...
	char **argv;
{
	...
	while ((ch = getopt(argc, argv, "hNwb:c:i:n:v:")) != EOF)
		...
		case 'v':
			...
			if (strrchr(gVolumeName, '/') != 0)
				fatal("\"%s\" is an invalid hfs name (has slash)", optarg);
			break;
...
hfs_newfs(char *device, int forceHFS)
{
	...
	if ( ((UInt64) dip.totalSectors * (UInt64) dip.sectorSize) % 4096 )
		warnx("Warning: size of %s partition is not an even multiple of 4K", device);
	...
	if (forceHFS || gWrapper) {
		...
		if (gNoCreate == 0) {
			...
			if (gWrapper) {
				...
				printf("newfs_hfs: Initialized %s as a %dMB HFS volume\n",
	...
	if (gWrapper || !forceHFS) {
		...
		if (gNoCreate == 0) {
			...
			if (retval == 0)
				printf("newfs_hfs: Initialized %s as a %dMB HFS Plus volume\n",
					device, dip.totalSectors/2048);
		}
	}
...
static void hfsplus_params (UInt32 sectorCount, UInt32 sectorSize, hfsparams_t *defaults)
{
	...
	strncpy(defaults->volumeName, gVolumeName, 31);
...
static void hfs_params(UInt32 sectorCount, UInt32 sectorSize, hfsparams_t *defaults)
{
	...
	if (gNoCreate && !gWrapper) {
		printf("%ld sectors at %ld bytes per sector\n", sectorCount, sectorSize);
		printf("HFS format parameters:\n");
		printf("\tvolume name: \"%s\"\n", gVolumeName);
		printf("\tblock-size: %ld\n", defaults->blockSize);
...
CalcBTreeClumpSize(UInt32 blockSize, UInt32 nodeSize, UInt32 driveBlocks, int catalog)
{
	...
	if (catalog)
		...
		limitClumpSectors = 2 * 1024 * 1024 / 512;	/* overall limit of 2MB */
	/* 
