...
do_modesense(int fd, struct mode_sense_reply *msrp, int page)
{
	...
	char reply[256];	/* bufffer to retrieve mode sense data */
	int offset;
	struct param_list_header *reply_header = (struct param_list_header *)&reply;
	...
	/* we need to skip over the block descriptors (if any) */
	offset = reply_header->plh_blkdesclen + sizeof(struct param_list_header);
	bcopy(&reply[offset], msrp, sizeof(struct mode_sense_reply));
	return 0;
}
...
fake_floppy:
		...
		if (nblocks < 8*32*1024)		/* <=528.4 MB */
			ntracks = 16;
		else if (nblocks < 16*32*1024)		/* <=1.057 GB */
			ntracks = 32;
		else if (nblocks < 32*32*1024)		/* <=2.114 GB */
			ntracks = 54;
		else if (nblocks < 64*32*1024)		/* <=4.228 GB */
			ntracks = 128;
		else					/* >4.228 GB */
			ntracks = 255;
