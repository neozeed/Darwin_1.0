...
#if 0
OSErr DeallocateFile( SVCB *vcb, HFSCatalogNodeID parDirID, ConstStr31Param catalogName )
{
	OSErr			err;
	...
	err = DeallocateFork( vcb, catalogData.nodeID, kDataForkType, catalogData.dataExtents, &recordDeleted );
	if( err != noErr )
		goto Exit;

	// Deallocate resource fork extents
	err = DeallocateFork( vcb, catalogData.nodeID, kResourceForkType, catalogData.rsrcExtents, &recordDeleted );
	if( err != noErr )
		goto Exit;

	//	[2355121] If we actually deleted extent records, update the B-tree header	
	if (recordDeleted)
...
OSErr	ZeroFileBlocks( SVCB *vcb, SFCB *fcb, UInt32 startingSector, UInt32 numberOfSectors )
{
	...
	UInt32					bufferSize			= gFSBufferSize;
	UInt32					bufferSizeSectors	= bufferSize >> kSectorShift;
	UInt32					currentPosition		= startingSector << kSectorShift;
		
	if ( gFSBufferInUse == true )
		return( fileBoundsErr );
		
	gFSBufferInUse	= true;											//	Mark the buffer as inUse
	ClearMemory( gFSBufferPtr, bufferSize );						//	Zero our buffer
	ClearMemory( &iopb, sizeof(iopb) );										//	Zero our param block
	
	iopb.ioRefNum	= ResolveFileRefNum( fcb );
	iopb.ioBuffer	= gFSBufferPtr;
