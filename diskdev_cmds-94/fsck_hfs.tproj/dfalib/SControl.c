...
//#include <Errors.h>
//#include <Devices.h>
//#include <DevicesPriv.h>
#define Is_Native_Mask 0x08
...
Ptr	gFSBufferPtr;
UInt32	gFSBufferSize;
Boolean	gFSBufferInUse;

int gGUIControl;

//ParallelFCB * gFCBPBuf;	/*	Parallel FCB array*/
	...
	*ScavRes		= 0;	
	...
	switch ( ScavOp )
	{
		...
		case scavVerify:								//	VERIFY
			...
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_ExtBTChk, kStatusMessage );
			...
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_ExtFlChk, kStatusMessage );
			...
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_CatBTChk, kStatusMessage );
			if (result = CatBTChk(GPtr))
				break;
			if (result = CheckForStop(GPtr))
				break;
			
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_CatFlChk, kStatusMessage );
			if (result = CatFlChk(GPtr))
				break;
			if (result = CheckForStop(GPtr))
				break;
			
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_CatHChk, kStatusMessage );
			...
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_VolumeBitMapChk, kStatusMessage );
			...
			if ( ! IsWrapperVolume( GPtr->volumeType, GPtr->inputFlags ) )	WriteMsg( GPtr, M_VInfoChk, kStatusMessage );
...
static int ScavSetUp( SGlob *GPtr)
{
	...
	err = InitDFACache( Blk_Size );		// Initialize a block cache of 512 byte blocks
	if ( err != noErr )
	{
...
//	gFCBPBuf	= GPtr->fcbPBuf;
	gFSBufferPtr	= AllocateMemory(32768);	/* XXX leak ??*/
	gFSBufferSize	= 32768;
...
static int ScavTerm( SGlobPtr GPtr )
{
	...
	if ( GPtr->volumeBitMapPtr != nil )
	{
		...
		{
		   #if BSD
		   	DisposeMemory(GPtr->volumeBitMapPtr->buffer);
		   #else
		...
		DisposeMemory( GPtr->volumeBitMapPtr );
		err = MemError();
	}
