...
OSStatus GetBlockProc ( SFCB *filePtr, UInt32 blockNum, GetBlockOptions options, BlockDescriptor *block )
{
	...
	result = GetCacheBlock( ResolveFileRefNum(filePtr), blockNum, block->blockSize, cacheOptions, &block->buffer, &block->blockReadFromDisk);
...
OSStatus
SetBlockSizeProc ( SFCB *filePtr, ByteCount blockSize, ItemCount minBlockCount)
{
#pragma unused (filePtr)
