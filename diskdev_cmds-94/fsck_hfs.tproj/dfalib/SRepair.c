...
static	OSErr	UpdateInMemoryStructures( SGlobPtr GPtr );
...
RebuildBtrees:	
	...
	if ( err == noErr )
		err	= rebuildErr;											//	If we have no other errors set it to our rebuild BTree error
...
static	OSErr	FixBTreeHeaderReservedFields( SGlobPtr GPtr, short refNum )
{
	...
	err = GetBTreeHeader( GPtr, refNum, &header );
	ReturnIfError( err );
	...
	err = ReleaseCacheBlock( (Ptr)header, rbWriteMask );
...
static	OSErr	UpdateVolumeBitMap( SGlobPtr GPtr, Boolean preAllocateOverlappedExtents )
{
	...
	for ( bufferNumber = 0 ; bufferNumber < volumeBitMap->numberOfBuffers ; bufferNumber++ )
	{
		...
		{
			err = CreateVolumeBitMapBuffer( GPtr, bufferNumber, preAllocateOverlappedExtents );
			ReturnIfError( err );
		}
		...
		if ( isHFSPlus )
		{
			...
			if ( GPtr->calculatedAllocationsFCB->fcbPhysicalSize < volumeBitMap->bitMapSizeInBytes ) //	Volume bitmap must be at least this big
			{
				M_DebugStr("\p Allocations file size does not match bitMapSizeInBytes");
				return( 123 );
				//ее	Call ExtendFile() here
			}
		}
		...
		for ( i = 0 ; i < blockCount ; i++ )
		{
			...
			CopyMemory( (volumeBitMap->buffer + i*Blk_Size), vbmBlockP, Blk_Size );
			ReleaseCacheBlock( vbmBlockP, rbWriteMask );
		}
	}
...
static	OSErr	UpdVal( SGlobPtr GPtr, RepairOrderPtr p )					//	the valence repair order
{
	...
	switch( p->type )													//	branch on error type
	{
		...
		case E_DirCnt:													//	invalid count of Dirs in volume
			if ( p->incorrect != calculatedVCB->vcbFolderCount )				//	do we know what we're doing?
				return ( IntError( GPtr, R_IntErr ) );					//	no, crump
			calculatedVCB->vcbFolderCount = p->correct;						//	yes, update
			GPtr->VIStat |= S_MDB;										//	make sure we update the MDB
			break;
			
		case E_FilCnt:													//	invalid count of FIles in volume
			if ( p->incorrect != calculatedVCB->vcbFileCount )				//	do we know what we're doing?
				return ( IntError( GPtr, R_IntErr ) );					//	no, crump
			calculatedVCB->vcbFileCount = p->correct;						//	yes, update
			GPtr->VIStat |= S_MDB;										//	make sure we update the MDB
			break;
		...
		case E_DirVal:													//	invalid interior directory valence
			...
			{
				if ( p->incorrect != record.hfsPlusFolder.valence)	//	do we know what we're doing?
					return ( IntError( GPtr, R_IntErr ) );
				record.hfsPlusFolder.valence = p->correct;			//	yes, update
			}
			else
			{
				if ( p->incorrect != record.hfsFolder.valence )	//	do we know what we're doing?
					return ( IntError( GPtr, R_IntErr ) );
				record.hfsFolder.valence = p->correct;			//	yes, update
			}
				
			result = ReplaceBTreeRecord( GPtr->calculatedCatalogFCB, &key, hint, &record, recSize, &hint );
			if ( result )
				return ( IntError( GPtr, result ) );
			break;
	}
...
static	OSErr	FixEmbededVolDescription( SGlobPtr GPtr, RepairOrderPtr p )
{
	...
	err = GetBlock_glue( gbDefault, totalSectors - 2, (Ptr *) &mdb, calculatedVCB->vcbVRefNum, calculatedVCB );	//	No Flags, MDB is always block
	ReturnIfError( err );
	...
	//	Fix the MDB
	err = GetBlock_glue( gbDefault, 2, (Ptr *) &mdb, calculatedVCB->vcbVRefNum, calculatedVCB );	//	No Flags, MDB is always block
	ReturnIfError( err );
...
#pragma unused (p)
	...
	//	Fix the wrapper catalog's first (and only) extent in alternate MDB
...
struct OrphanRepairOrder
{
	...
	HFSExtentKey		theKey;			// the key to delete
};
				...
				if ( err != noErr )
				{
					...
					if ( err == btNotFound )
					{
						...
						if ( isHFSPlus )
						{
							...
							CopyCatalogName( (CatalogName *)&foundKey.hfsPlus.nodeName, (CatalogName *)&threadData.nodeName, isHFSPlus );
							err = InsertBTreeRecord( GPtr->calculatedCatalogFCB, &key, &threadData, sizeof(HFSPlusCatalogThread), &threadHint );
						}
						else
						{
...
/* need a reload function in the OS!!!! */
static	OSErr	UpdateInMemoryStructures( SGlobPtr GPtr )
{
	return (0);
}
...
static	OSErr	FixOverlappingExtents( SGlobPtr GPtr )
{
	...
	for ( i = 0 ; i < (**extentsTableH).count; i++ )
	{
		if ( (**extentsTableH).extentInfo[i].fileNumber < kHFSFirstUserCatalogNodeID )
		{
			...
			WriteMsg( GPtr, M_InternalFileOverlap, kErrorMessage );
