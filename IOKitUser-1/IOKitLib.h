...
/*! @function IOMasterPort
    ...
    @param size The task's bootstrap port, pass bootstrap_port from mach/mach_init.h.
...
<br>	kIOTerminatedNotification Delivered after an IOService has been terminated.
    @param matching A CF dictionary containing matching information, of which one reference is consumed by this function. IOKitLib can construct matching dictionaries for common criteria with helper functions such as IOServiceMatching, IOOpenFirmwarePathMatching.
...
/*! @function IORegistryEntryGetProperty
    ...
    @discussion This function has limited use to retrieve a binary copy of simple property values without using CF containers. Currently, only OSData, OSString (& OSSymbol), OSNumber & OSBoolean properties are retrievable with IORegistryEntryGetProperty, and the size is limited to that of io_struct_inband_t (4096 bytes).
    @param entry The registry entry handle whose property table read one property.
    @param propertyName The name of the property in the registry entry's property table as a C-string.
    @param buffer The caller's buffer to receive the binary representation of the property if it exists.
    @param size The size of the property copied is returned.
    @result A kern_return_t error code. */
