            ...
            CFURLRef                                 url )
{
    ...
    error = kKEXTReturnError;
    boolval = IOURLCreateDataAndPropertiesFromResource(kCFAllocatorDefault, url, NULL, &dict, attribs, &err);
    CFRelease(attribs);
    if ( boolval ) do {
        mode_t modeVal;
        CFBooleanRef cfbool;
        CFNumberRef mode;
        CFNumberRef uid1 = NULL;
        CFNumberRef uid2 = NULL;

        cfbool = CFDictionaryGetValue(dict, kIOURLFileExists);
        boolval = false;
        if ( cfbool ) {
            boolval = CFEqual(cfbool, kCFBooleanTrue);
        }
        if ( !boolval ) {
            return kKEXTReturnResourceNotFound;
        }

        uid1 = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &uidVal);
        uid2 = (CFNumberRef)CFDictionaryGetValue(dict, kIOURLFileOwnerID);
        if ( !uid1 || !uid2 || !CFEqual(uid1, uid2) ) {
            ...
            KEXTReturn                             * error )
{
    ...
    *error = kKEXTReturnSuccess;
    ...
    memset(manager, NULL, sizeof(KEXTManager));
...
static CFArrayRef
_KEXTBundleCopyModuleOrPersonality(
            CFDictionaryRef                          props,
            Boolean                                  isModule )
{
    CFDictionaryRef sVal;
    CFArrayRef pVal;
    ...
    if ( isModule ) {
        sing = CFSTR(kModuleKey);
        plur = CFSTR(kModulesKey);
    }
    else {
        sing = CFSTR(kPersonalityKey);
        plur = CFSTR(kPersonalitiesKey);
    }

    pVal = CFDictionaryGetValue(props, plur);
    if ( !pVal ) {
        const void * vals[1];
        sVal = CFDictionaryGetValue(props, sing);
        if ( !sVal )
            return NULL;
        vals[0] = sVal;
        pVal = CFArrayCreate(kCFAllocatorDefault, vals, 1, &kCFTypeArrayCallBacks);
    }
    else {
        CFRetain(pVal);
    }

    if ( CFArrayGetCount(pVal) < 1 ) {
        CFRelease(pVal);
        pVal = NULL;
    }
            ...
            Boolean                                  isCFStyle )
{
    ...
    array = _KEXTBundleCopyModuleOrPersonality(props, true);
    if ( !array ) {
        return NULL;
    }
            ...
            CFMutableArrayRef                      * keys )
{
    ...
    array = _KEXTBundleCopyModuleOrPersonality(props, false);
    if ( !array ) {
        return NULL;
    }
            ...
            Boolean                                * cfStyle )
{
    ...
    *cfStyle = true;
    ...
    newPath = CFStringCreateWithFormat(
                        kCFAllocatorDefault,
                        NULL,
                        CFSTR("%@/Contents/Info-macos.plist"),
                        urlString);
            ...
            CFArrayRef                             * persons )
{
    ...
    if ( !CFStringHasSuffix(path, CFSTR("kext")) &&
    ...
    ret = _KEXTManagerCreateProperties(url, &properties, &isCFStyle);
    if ( ret != kKEXTReturnSuccess) {
        CFRelease(date);
        return ret;
    }
    ...
    *persons = _KEXTManagerCreatePersonsArray(bundleKey, properties, &keys);
    if ( *persons ) {
        if ( keys ) {
            // XXX -- We really don't need this...
            ...
            CFURLRef                                 bundleUrl )
{
    ...
    // Create entries.
    ret = _KEXTManagerCreateEntities(bundleUrl, &bundle, &modules, &persons);
    if ( ret != kKEXTReturnSuccess ) {
        return ret;
    }
    ...
    do {
        ...
        if ( bundle2 ) {
            ...
            vendor1 = KEXTBundleGetProperty(bundle, CFSTR("BundleVendor"));
            vendor2 = KEXTBundleGetProperty(bundle2, CFSTR("BundleVendor"));
            vendormatch = ( (!vendor2 && !vendor1) ||
                            (vendor1 && vendor2 && CFEqual(vendor1, vendor2)) );

            vers1 = KEXTBundleGetProperty(bundle, CFSTR("BundleVersion"));
            vers2 = KEXTBundleGetProperty(bundle2, CFSTR("BundleVersion"));
            versionmatch = ( (!vers1 && !vers2) ||
                             (vers1 && vers2 && CFEqual(vers1, vers2)) );

            if ( !vendormatch || !versionmatch ) {
...
{
    CFArrayRef array;
    CFMutableDictionaryRef urlRelCopy;
    CFIndex count;
    CFIndex index;
    SInt32 err;
    ...
    for ( index = 0; index < count; index++ ) {
        ...
        base = CFURLCopyAbsoluteURL((CFURLRef)CFArrayGetValueAtIndex(array, index));
        path = CFURLGetString(base);
        newUrl = CFURLCreateWithString(kCFAllocatorDefault, path, NULL);
        CFRelease(base);

        // If the bundle's url already exists, then avoid creating a new one.
        bundleKey = CFDictionaryGetValue(urlRelCopy, newUrl);
        if ( !bundleKey ) {
            // This is possibly a new bundle.
            KEXTManagerAddBundle(manager, newUrl);
        }
        else {
            ...
            CFDictionaryRemoveValue(urlRelCopy, newUrl);
        }
        CFRelease(newUrl);
    }
            ...
            CFURLRef                                 url )
{
    ...
    do {
        ...
        error = _KEXTManagerScanBundles(manager, url);
        if ( error != kKEXTReturnSuccess ) {
            break;
        }
    } while ( false );
...
// Create a dependency list for the given module.  
            ...
            CFMutableArrayRef                        array )
{
    ...
    isTrue = false;
    req = KEXTModuleGetProperty(module, CFSTR(kRequiresKey));
    if ( req ) {
        tid = CFGetTypeID(req);

        if ( CFStringGetTypeID() == tid ) {
            mod = KEXTManagerGetModule(manager, (CFStringRef)req);
            if ( !mod )
                return false;

            if ( !_KEXTManagerGraphDependencies(manager, mod, array) )
                return false;
        }
            ...
            count = CFArrayGetCount(req);
            for ( i = 0; i < count; i++ ) {
                CFStringRef name;

                name = CFArrayGetValueAtIndex((CFArrayRef)req, i);
                if ( !name )
                    continue;
    ...
    req = KEXTModuleGetProperty(module, CFSTR("Kernel Resource"));
    if ( req ) {
       isTrue = CFBooleanGetValue(req);
    }
            ...
            CFArrayRef                             * array )
{
    ...
    ret = _KEXTManagerGraphDependencies(manager, module, modules);
            ...
            KEXTModuleRef                            module )
{
    ...
    if ( !_KEXTManagerGraphDependencies(manager, module, array) ) {
        CFRelease(array);
        return kKEXTReturnMissingDependency;
    }
            ...
            KEXTModuleRef                            module )
{
    ...
    if ( !modName || !CFStringGetCString(modName, name, 256, kCFStringEncodingASCII) ) {
        return kKEXTReturnError;
    }
            ...
            KEXTPersonalityRef                       personality )
{
    ...
    str = KEXTPersonalityGetProperty(personality, CFSTR("IOClass Names"));
    if ( str ) {
        if ( !CFStringGetCString(str, name, 256, kCFStringEncodingASCII) ) {
            return kKEXTReturnError;
        }
