...
/*
    ...
    The KEXTBundle container is a logical representation of a kernel extension bundle. The kernel extension bundle's filesystem representation is as follows:
 ...
 FooDriver.kext/
    ...
    The Info-macosx.plist is an XML property list containing information specific to the kernel extension.  The property list has the following format:
...
<dict>
  <key>CFBundleName</key>
     <string>FooDriver</string>
  <key>CFBundleVersion</key>
      <string>1.0.0d2</string>
  <key>CFBundleIconFile</key>
     <string>FooDriver.icns</string>
  <key>Vendor</key>
      <string>Foo Tech, Inc.</string>
  <key>CFBundlePackageType</key>
     <string>KEXT</string>
  <key>Date</key>
      <date>1999-11-03T20:49:29Z</date>
  <key>CFBundleSignature</key>
      <string>????</string>
  <key>CFBundleGetInfoString</key>
      <string>Foo Tech kernel extension</string>
  <key>Modules</key>
     <array>
         <dict>
             <key>Requires</key>
             ...
             <key>File</key>
                 <string>driver_module.kmod</string>
             <key>Name</key>
                 <string>com.foo.driver_module</string>
             <key>Format</key>
  ...
  <key>Personalities</key>
     <array>
         <dict>
             <key>IOMatchCategory</key>
                 <string>FooController</string>
             <key>Name</key>
                 <string>Foo Controller</string>
             <key>IOImports</key>
                 <string>(name of provider)</string>
    ...
    Top level plist properties are specific to the KEXT bundle itself and has information about required by the Finder and the KEXTManager along with some other useful things such as versioning information and vendor names.  The Modules key references an array of dictionaries containing information about the modules a kernel extension bundle may contain.  Module information includes the module's file name, what to link it against, other modules it relies on, and a unique name to identify it.  This is required for all kernel extensions.

    The Personalities key references and array of one or more driver 'personalities' (a personality is information which describes various attributes of a driver, such as matching criteria, run-time configuration, the name kernel module on which it depends, etc.). The personality key is required for IOKit style drivers, but is not required for loading modules into the kernel and can be omitted.
*/
...
*/
Boolean 		KEXTModuleIsLoaded(KEXTModuleRef module, KEXTReturn * error);
...
*/
KEXTManagerRef		KEXTManagerCreate(KEXTManagerBundleLoadingCallbacks * bundleCallbacks, KEXTManagerModuleLoadingCallbacks * moduleCallbacks, KEXTManagerPersonalityLoadingCallbacks * personalityCallbacks, KEXTManagerConfigsCallbacks * configCallbacks, void * context, KEXTReturn * error);
/*!
