...
KEXTPersonalityRef KEXTPersonalityCreate(CFStringRef parentKey, CFDictionaryRef properties)
{
    ...
    // Do some conversions on the various properties to bring
    // them up to date.
    score = CFDictionaryGetValue(dict, CFSTR("IOProbe Score"));
    if ( score ) {
        CFDictionarySetValue(dict, CFSTR("IOProbeScore"), score);
        CFDictionaryRemoveValue(dict, CFSTR("IOProbe Score"));
    }

    name = CFDictionaryGetValue(dict, CFSTR("IOClass Names"));
    if ( name ) {
        CFDictionarySetValue(dict, CFSTR("IOClass"), name);
        CFDictionaryRemoveValue(dict, CFSTR("IOClass Names"));
    }

    name = CFDictionaryGetValue(dict, CFSTR("IOImports"));
    if ( name ) {
        CFDictionarySetValue(dict, CFSTR("IOProviderClass"), name);
        CFDictionaryRemoveValue(dict, CFSTR("IOImports"));
    }
