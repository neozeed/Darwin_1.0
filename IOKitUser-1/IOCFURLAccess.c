...
CF_DEFINECONSTANTSTRING(kIOURLFileExists, "kIOURLFileExists");
CF_DEFINECONSTANTSTRING(kIOURLFileDirectoryContents, "kIOURLFileDirectoryContents");
CF_DEFINECONSTANTSTRING(kIOURLFileLength, "kIOURLFileLength");
CF_DEFINECONSTANTSTRING(kIOURLFileLastModificationTime, "kIOURLFileLastModificationTime");
CF_DEFINECONSTANTSTRING(kIOURLFilePOSIXMode, "kIOURLFilePOSIXMode");
CF_DEFINECONSTANTSTRING(kIOURLFileOwnerID, "kIOURLFileOwnerID");

CF_DEFINECONSTANTSTRING(kIOFileURLExists, "kIOFileURLExists");
CF_DEFINECONSTANTSTRING(kIOFileURLPOSIXMode, "kIOFileURLPOSIXMode");
CF_DEFINECONSTANTSTRING(kIOFileURLSize, "kIOFileURLSize");
...
CFMutableArrayRef _IOContentsOfDirectory(CFAllocatorRef alloc, char *path, CFURLRef base, CFStringRef matchingAbstractType) {
    ...
    if (extLen > 0) {
        CFStringGetBytes(extension, CFRangeMake(0, extLen), CFStringGetSystemEncoding(), 0, FALSE, extBuff, CFMaxPathSize, &extLen);
        extBuff[extLen] = '\0';
    }
