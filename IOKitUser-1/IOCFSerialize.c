...
DoCFSerializeString(CFStringRef object, IOCFSerializeState * state)
{
	...
	buffer = CFStringGetCStringPtr(object, kCFStringEncodingASCII);
	if (!buffer) {
		len = CFStringGetLength(object) + 1;
		buffer = malloc(len);
		if (!buffer || !CFStringGetCString(object, buffer, len, kCFStringEncodingASCII))
			return false;
	}
	...
	for (i = 0; i < (len - 1); i++) {
		if (buffer[i] == '<') {
			...
			if (!addChar(buffer[i], state)) return false;
		}
	}
