						...
						long	version)
{
	...
	if (cls->super_class)
	{
		...
		if (!clstmp)
		{
			_objc_inform("failed objc_getClass(%s) for %s->super_class", (const char *)cls->super_class, cls->name);
			goto Error;
		}
	...
	if (!clstmp)
	{
		_objc_inform("failed objc_getClass(%s) for %s->isa->isa", (const char *) meta->isa, cls->name);
		goto Error;
	}
	...
	if (meta->super_class)
	{
		...
		if (!clstmp)
		{
			_objc_inform("failed objc_getClass(%s) for %s->isa->super_class", (const char *)meta->super_class, cls->name);
			goto Error;
		}
...
// Why not OBJC_UNLOCK(&messageLock) right after cache miss?
	...
	smt = NXZoneMalloc (_objc_create_zone(), sizeof(struct objc_method));
	smt->method_name	= sel;
	smt->method_types	= "";
...
#ifdef OBJC_INSTRUMENTED
	new_cache = NXZoneMalloc (_objc_create_zone(),
...
#else
					NXZoneFree (_objc_create_zone(), oldEntry);
#endif
				}
			}
...
#else
				NXZoneFree (_objc_create_zone(), old_cache->buckets[index]);
#endif
			}
		}
	}
...
#else
	NXZoneFree (_objc_create_zone(), old_cache);
#endif
	return new_cache;
}
...
static void	_garbage_make_room		(void)
{
	...
	{
		first		= 0;
		garbage_refs	= NXZoneMalloc (_objc_create_zone(),
	...
	{
		tempGarbage	= NXZoneRealloc ((NXZone *) _objc_create_zone(),
						(void *) garbage_refs,
