...
#ifdef FREEZE
	// LaurentR-:
	// On OS X, we don't define this, so we always use _objc_create_zone()
	// but just in case!
...
SEL	sel_registerName	       (const char *	key)
{
	...
	copyKey = strcpy(objc_malloc(strlen(key) + 1), key);
	sel = __S(_sel_registerName) (copyKey);
	if (sel != (SEL)copyKey) {
		// found sel already in table; probably should not happen
		// with that getUid check above
		free(copyKey);
	}
	return sel;
}
								...
								unsigned int		sectionSize)
{
	...
	// LaurentR-: replaced NXZoneMalloc with objc_malloc()
	//frozenTable = NXZoneMalloc (NXDefaultMallocZone(), sizeof(hashTable));
	frozenTable 		= objc_malloc(sizeof(hashTable));
										...
										unsigned long	slide)
{
	...
	for (index = 1; index < numConflicts; index += 1)
	{
		...
		if ((pCon1 != pCon2) && (strcmp((char *) pCon1, (char *) pCon2) == 0)) {
			if (rocketLaunchingDebug)
				printf("H: resolve sel 0x%.8X/0x%.8X from 0x%.8X/0x%.8X (%s)\n", (unsigned)conflicts[index], (unsigned)pCon2, (unsigned)conflicts[index-1], (unsigned)pCon1, (char *)pCon1);
...
void	_sel_printStatistics	       (void)
{
	...
	printf ("cumulative selector hashtable statistics:\n");
									...
									int *		size)
{
	...
	// LaurentR-: This function is only called by objcunique so
	// no need to allocate in the objc zone.
