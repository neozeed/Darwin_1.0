...
static void				_objc_bindModuleContainingClass(Class cls);
#endif
...
static void _objc_bindModuleContainingClass (Class cls)
{
	...
	class_name = cls->name;
	name	   = (char *) malloc (strlen (class_name) + 20);
	while (*class_name == '%')
		class_name += 1;
...
static void undo_preuniqing_for_image (const header_info *  hi)
{
	...
	if (rocketLaunchingDebug)
		printf ("undoing preuniquing for %s\n", libraryNameForMachHeader (hi->mhdr));
...
static void	do_pended_map_selectors	       (void)
{
	...
	for (loop = 0; rocketLaunching && (loop < pended_map_selectors_count); loop += 1)
	{
		...
		if (_getObjcHeaderData (machhdr, &unused))
		{
			...
			if (_getObjcBackRefs (machhdr, &unused) == NULL)
			{
				if (rocketLaunchingDebug)
				{
					printf ("backing off because no backref table for: %s\n",
	...
	{
		if (rocketLaunchingDebug)
			printf ("backing off because main module is missing necessary conflict section\n");
	...
	for (loop = 0; loop < pended_map_selectors_count; loop += 1)
	{
		...
		{
			if (rocketLaunchingDebug)
				printf ("resolving conflicts in: %s\n", libraryNameForMachHeader (machhdr));
		...
		if (rocketLaunchingDebug)
			printf ("installing backref table for: %s\n", libraryNameForMachHeader (machhdr));
...
void	_objcInit	       (void)
{
	...
	ptrace(0xb0ff, 0, 0, 0);	// end of ObjC init
}
