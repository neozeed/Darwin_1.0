...
int main(int argc, char *argv[])
{
	...
	processor_set_control_port_t		default_control;
	...
	struct processor_set_load_info	load_info;
        ...
        ret = bootstrap_ports(mach_task_self(), &host_control, &device, &wledger, &pledger, &security);
        if (ret != KERN_SUCCESS) {
                mach_error("bootstrap_ports", ret);
                exit(EXIT_FAILURE);
        }
	...
	ret = host_processor_slots(host, slots, &num_slots);
	if (ret != KERN_SUCCESS) {
		mach_error(argv[0], ret);
                exit(EXIT_FAILURE);
	}
	...
	ret = host_processor_set_priv(host_control, default_pset, &default_control);
	if (ret != KERN_SUCCESS) {
		mach_error(argv[0], ret);
                exit(EXIT_FAILURE);
	}
	...
	ret = processor_set_info(default_control, PROCESSOR_SET_BASIC_INFO,
	...
	ret = processor_set_statistics(default_control, PROCESSOR_SET_LOAD_INFO,
