...
main(argc, argv)
	...
	char	*argv[];
{
	static mach_port_t	mport[5] = { MACH_PORT_NULL };
	int			i;
	...
	if (bootstrap_ports(mach_task_self(),
					&mport[0], &mport[1],
					&mport[2], &mport[3], &mport[4])
							!= KERN_SUCCESS) {
		fprintf(stderr, "%s: failed to get mach_priv port.\n", pgmname);
		exit(1);
	} 
	
	for (i = 1; i < 5; i++) {
		mach_port_deallocate(mach_task_self(), mport[i]);
		mport[i] = MACH_PORT_NULL;
	}
	
	if (mach_port_mod_refs(mach_task_self(),
				mport[0], MACH_PORT_RIGHT_SEND, 1)
							!= KERN_SUCCESS) {
		mach_port_deallocate(mach_task_self(), mport[0]);
		mport[0] = MACH_PORT_NULL;
	}
