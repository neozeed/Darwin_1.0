...
#if !DAEMONIZE
#warning kextd not in daemon mode.
#endif
...
static KEXTReturn _KEXTDAuthenticateBundleCB(CFURLRef url, void * context)
{
    ...
    if ( !ret ) {
        ...
        if ( CFStringGetCString(urlString, name, 256, kCFStringEncodingNonLossyASCII) )
            syslog(LOG_INFO, "%s failed authentication.", name);
...
static Boolean _KEXTDModuleWillLoadCB(KEXTManagerRef manager, KEXTModuleRef module, void * context)
{
    ...
    if ( ret ) {
        ...
        moduleName = KEXTModuleGetProperty(module, CFSTR("Name"));
        if ( !moduleName )
            return false;
...
static void _KEXTDModuleWasLoadedCB(KEXTManagerRef manager, KEXTModuleRef module, void * context)
{
    ...
    for ( i = 0; i < count; i++ ) {
        ...
        modName = KEXTPersonalityGetProperty(person, CFSTR("Module"));
        if ( !modName ) {
            continue;
        }

        name = KEXTModuleGetProperty(module, CFSTR("Name"));
        if ( !name ) {
            continue;
        }

        if ( !CFEqual(modName, name) ) {
...
static KEXTReturn _KEXTDModuleErrorCB(KEXTManagerRef manager, KEXTModuleRef module, KEXTReturn error, void * context)
{
    ...
    syslog(LOG_INFO, "error (%d) loading module: %s.\n", ret, name);
...
static inline Boolean _KEXTPersonalityNeedsModule(KEXTPersonalityRef person, CFStringRef modName)
{
    ...
    name = KEXTPersonalityGetProperty(person, CFSTR("Module"));
    if ( !name ) {
        return false;
    }
...
static void _KEXTDAddPersonalitiesWithModuleToUnloadedList(KEXTDRef kextd, CFStringRef modName)
{
    ...
    for ( i = 0; i < count; i++ ) {
        ...
        name = KEXTPersonalityGetProperty(entity, CFSTR("Module"));
...
static void * _KEXTDKmodWait(void * info)
{
    ...
    kr = bootstrap_ports(mach_task_self(), &kmodPort, &p1, &p2, &p3, &p4);
    error = KERN2KEXTReturn(kr);
    if ( kr != KERN_SUCCESS ) {
        syslog(LOG_INFO, "error (%d) obtaining mach port.\n", error);
        return (void *)error;
    }
    ...
    while ( 1 ) {
        ...
        if ( kr != KERN_SUCCESS ) {
            syslog(LOG_INFO, "error (%d): kmod_control.\n", kr);
            continue;
        }
...
KEXTReturn KEXTDStartMain(KEXTDRef kextd, Boolean beVerbose, KEXTBootlevel bootlevel)
{
    ...
    daemon(0, 0);
    if ( errno ) {
        syslog(LOG_INFO, "failed to daemonize process. Aborting!\n");
        return kKEXTReturnError;
    }
#endif

    k = (KEXTD *)kextd;
    k->_manager = KEXTManagerCreate(&bcb, &modcbs, NULL, &cfgcbs, kextd, &error);
    if ( !k->_manager )
        return error;
    ...
    if ( !k->_runloop ) {
        syslog(LOG_INFO, "error allocating runloop.\n");
        return NULL;
    }
    ...
    if ( !k->_signalsource ) {
        syslog(LOG_INFO, "error allocating signal runloop source.\n");
        return NULL;
    }
    CFRunLoopAddSource(k->_runloop, k->_signalsource, kCFDefaultRunLoopMode);

    sourceContext.perform = _KEXTDPerform;
    k->_kernelsource = CFRunLoopSourceCreate(kCFAllocatorDefault, 2, &sourceContext);
    if ( !k->_kernelsource ) {
        syslog(LOG_INFO, "error allocating kernel runloop source.\n");
        return NULL;
    }
    CFRunLoopAddSource(k->_runloop, k->_kernelsource, kCFDefaultRunLoopMode);
    ...
    if ( poll ) {
        ...
        timer = CFRunLoopTimerCreate(kCFAllocatorDefault, date, period, 10, _KEXTDTimerCallout, &timerContext);
        CFRelease(date);
        if ( !timer ) {
            syslog(LOG_INFO, "error allocating kmod runloop timer.\n");
            return kKEXTReturnError;
        }

        CFRunLoopAddTimer(k->_runloop, timer, kCFDefaultRunLoopMode);
        CFRelease(timer);
    }
#endif
    ...
    if ( (error = _KEXTDSendPersonalities(kextd, bootlevel)) != kKEXTReturnSuccess ) {
        ...
        syslog(LOG_INFO, "error (%d) sending personalities to IOCatalogue.", error);
        return error;
    }
...
void KEXTDScanPaths(KEXTDRef kextd)
{
    ...
    for ( i = 0; i < count; i++ ) {
        ...
        if ( url ) {
            ...
            if ( error != kKEXTReturnSuccess ) {
                syslog(LOG_INFO, "error (%d) scanning path.\n", error);
            }
        }
    }
}
