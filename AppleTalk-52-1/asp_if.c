...
l_atalk:
	...
	if ((at_send_to_dev(descr, ASPIOC_GetStatus,
	...
	ctrl.len = 0;
	ctrl.buf = ctrlbuf;
	data.maxlen = StatusBufferSize;
	data.len = 0;
	data.buf = StatusBuffer;

	while ((rval = ATgetmsg(descr, &ctrl, &data, 0)) == -1) {
		if (errno == EINTR)
			continue;
		if (errno == EINVAL)
			*SPError = ASPERR_ParamErr;
	...
	/*
	if (rval = read(descr, StatusBuffer, StatusBufferSize) == -1) {
		*SPError = ASPERR_SystemErr;
		goto l_error;
	}
	*/
...
#define NOT_NEEDED_SELECT 1
#ifdef NOT_NEEDED_SELECT
/*
