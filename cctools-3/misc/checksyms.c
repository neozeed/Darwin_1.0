...
enum bool verification)
{
    ...
    char *install_name, *base_name, *dot, *name, *p, *q, *suffix;
    enum bool framework, no_major_version_letter_ok;
    FILE *fp;
	...
	base_name = NULL;
	framework = FALSE;
	no_major_version_letter_ok = FALSE;
#ifdef __OPENSTEP__
...
#ifdef ALLOW_MACOSX_PR1_PATHS
	/*
	 * The directory 
	 * /System/Library/Frameworks/CarbonCore.framework/Versions/A/Support
	 * is used with:
	 * libGDBCFMHelper.dylib
	 */ 
	else if(strncmp(install_name, "/System/Library/Frameworks/CarbonCore."
			"framework/Versions/A/Support/",
		sizeof("/System/Library/Frameworks/CarbonCore."
			"framework/Versions/A/Support/") - 1) == 0){
	    base_name = install_name + sizeof("/System/Library/Frameworks/"
			"CarbonCore.framework/Versions/A/Support/") - 1;
	    no_major_version_letter_ok = TRUE;
	}
	/*
	 * The directory 
	 * /System/Library/Frameworks/Carbon.framework/Versions/A/Libraries
	 * is used with:
	 * libGDBCFMHelper.dylib
	 * libCommonPanels.dylib
	 * libhltb.dylib
	 * liblltb.dylib
	 * libnav.dylib
	 * libqd.dylib
	 */
	else if(strncmp(install_name, "/System/Library/Frameworks/Carbon."
			"framework/Versions/A/Libraries/",
		sizeof("/System/Library/Frameworks/Carbon."
			"framework/Versions/A/Libraries/") - 1) == 0){
	    base_name = install_name + sizeof("/System/Library/Frameworks/"
			"Carbon.framework/Versions/A/Libraries/") - 1;
	    no_major_version_letter_ok = TRUE;
	}
	/*
	 * The directory 
	 ...
	 * libRIP.A.dylib
	 */ 
	else if(strncmp(install_name, "/System/Library/Frameworks/CoreGraphics."
			"framework/Versions/A/Libraries/",
		sizeof("/System/Library/Frameworks/CoreGraphics."
			"framework/Versions/A/Libraries/") - 1) == 0){
	    base_name = install_name + sizeof("/System/Library/Frameworks/"
			"CoreGraphics.framework/Versions/A/Libraries/") - 1;
	}
	/*
	 * The directory 
	 * /System/Library/CoreServices/Encodings
	 * is used with:
	 * libArabicConverter.B.dylib
	 * libCyrillicConverter.B.dylib
	 * libGreekConverter.B.dylib
	 * libHebrewConverter.B.dylib
	 * libJapaneseConverter.B.dylib
	 * libKoreanConverter.B.dylib
	 * libLatin2Converter.B.dylib
	 * libLatin5Converter.B.dylib
	 * libLatinSuppConverter.B.dylib
	 * libSymbolConverter.B.dylib
	 * libThaiConverter.B.dylib
	 * libVietnameseConverter.B.dylib
	 */ 
	else if(strncmp(install_name, "/System/Library/CoreServices/Encodings/",
		sizeof("/System/Library/CoreServices/Encodings/") - 1) == 0){
	    base_name = install_name + sizeof("/System/Library/CoreServices/"
			"Encodings/") - 1;
	}
#endif /* ALLOW_MACOSX_PR1_PATHS */
	 ...
	 */
		...
		printf("dynamic library has non-conventional directory in its "
		       "install name (%s) (should be /Local/Lib... not "
		       "/Local//Lib...\n", install_name);
	    }
	    if(verification == TRUE)
		printf("dylib_non_conventional_directory\n");
	    exit_status = EXIT_FAILURE;
	    return;
	}
#endif /* ALLOW_MACOSX_PR1_PATHS */
	...
	if(framework == TRUE){
		...
		*dot = '.';
		...
		if(verification == TRUE)
		    printf("dylib_non_conventional_name\n");
		exit_status = EXIT_FAILURE;
		*dot = '\0';
	    }
	    else{
		...
		*dot = '.';
		...
		if(strcmp(dot, ".dylib") == 0){
		    if(no_major_version_letter_ok == FALSE){
			if(detail == TRUE){
			    printf("%s: ", ofile->file_name);
			    if(arch_name != NULL)
				printf("(for architecture %s): ", arch_name);
			    printf("dynamic library does not have major "
				   "version letter in its install name (%s)\n",
				   install_name);
			}
			if(verification == TRUE)
			    printf("dylib_non_conventional_name\n");
		     ...
		     * has the suffix in the wrong place.
		     */
		    if(*p == '_'){
			q = strchr(p, '.');
			if(q != NULL && strcmp(q, ".dylib") == 0){
			    if(detail == TRUE){
				printf("%s: ", ofile->file_name);
				if(arch_name != NULL)
				    printf("(for architecture %s): ",arch_name);
				*q = '\0';
				printf("dynamic library's suffix (%s) ", p);
				*q = '.';
				printf("not before version letter in install "
				       "name (%s)\n", install_name);
			    }
			    if(verification == TRUE)
				printf("dylib_non_conventional_name\n");
			    exit_status = EXIT_FAILURE;
			    goto skip_dylib_check;
			}
		    }
#endif /* ALLOW_MACOSX_PR1_PATHS */
		}
