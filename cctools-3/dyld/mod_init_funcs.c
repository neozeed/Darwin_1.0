...
enum bool bind_now)
{
	...
	do{
		...
		for(j = 0; j < dyst->nmodtab; j++){
		    ...
		    if(link_state == REGISTERING)
			SET_LINK_STATE(q->images[i].modules[j], INITIALIZING);
			...
			if(bind_now == FALSE)
			    SET_LINK_STATE(q->images[i].modules[j], LINKED);
			else
