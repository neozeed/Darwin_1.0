...
char *dylib_name)
{
	...
	if(resolvedname == NULL){
		...
		strcat(p, dylib_name + (sizeof("@executable_path/") - 1));
		return(p);
	    }
	}
...
ino_t ino)
{
	...
	if(map_image(dylib_name, "library", library_size, fd, file_addr, 
	    ...
	    &seg1addr, &slide_value, &images_dyld_stub_binding_helper) ==
		FALSE){
...
unsigned long object_size)
{
	...
	if(map_image(name, "object file image", object_size, -1, NULL, 
	    ...
	    &has_coalesced_sections, &init, &term, &seg1addr, &slide_value,
...
struct mach_header *mh)
{
	...
	for(i = 0; i < mh->ncmds; i++){
		...
		if(load_library_image(dl_load, NULL) == FALSE &&
		   return_on_error == TRUE){
...
void)
{
	...
	for(p = &library_images; p != NULL; p = p->next_images){
		...
		if(p->images[i].remove_on_error == TRUE){
			...
			/* TODO: send the event message that the images is
			   being removed */
