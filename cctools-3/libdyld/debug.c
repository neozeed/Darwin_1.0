...
static void set_dyld_debug_error_data(
 ...
 * to be file level static.  Again this is not thread save but given the
...
{
	SET_MACH_DYLD_DEBUG_ERROR(msg_result, 88);
...
struct dyld_debug_module *module)
{
	...
	k = user_dyld_debug_defining_module(state.debug_port, rcv_timeout,
        ...
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 87);
            d = DYLD_FAILURE;
	}
#else
	...
	if(d != DYLD_SUCCESS)
	    set_dyld_debug_error_data(d,0,0,86,__FILE__,__LINE__);
#endif
...
boolean_t *bound)
{
	...
	k = user_dyld_debug_is_module_bound(state.debug_port, rcv_timeout,
        ...
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 85);
            d = DYLD_FAILURE;
	}
	else if(d != DYLD_SUCCESS)
	    SET_LOCAL_DYLD_DEBUG_ERROR(84);
#else
	mig_error = 0;
	...
	if(mig_error == 0 && d != DYLD_SUCCESS)
	    set_dyld_debug_error_data(d,0,0,83,__FILE__,__LINE__);
#endif
...
struct dyld_debug_module module)
{
	...
	k = user_dyld_debug_bind_module(state.debug_port, rcv_timeout,
        ...
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 82);
            d = DYLD_FAILURE;
	}
#else
	mig_error = 0;
	...
	if(mig_error == 0 && d != DYLD_SUCCESS)
	    set_dyld_debug_error_data(d,0,0,81,__FILE__,__LINE__);
#endif
...
unsigned long *module_nameCnt)
{
	...
	k = user_dyld_debug_module_name(state.debug_port, rcv_timeout,
        ...
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 80);
            d = DYLD_FAILURE;
	}
	else if(d != DYLD_SUCCESS)
	    SET_LOCAL_DYLD_DEBUG_ERROR(79);
#else
	mig_error = 0;
	...
	if(mig_error == 0 && d != DYLD_SUCCESS)
	    set_dyld_debug_error_data(d,0,0,78,__FILE__,__LINE__);
#endif
...
unsigned long rcv_timeout)
{
	...
	k = user_dyld_debug_monoutput(state.debug_port, rcv_timeout, &d);
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 77);
            d = DYLD_FAILURE;
	}
	else if(d != DYLD_SUCCESS)
	    SET_LOCAL_DYLD_DEBUG_ERROR(76);
#else
	mig_error = 0;
	d = user_dyld_debug_monoutput(state.debug_port, send_timeout,
		rcv_timeout);
	if(mig_error == 0 && d != DYLD_SUCCESS)
	    set_dyld_debug_error_data(d,0,0,75,__FILE__,__LINE__);
#endif
...
void (*dyld_event_routine)(struct dyld_event event))
{
	...
	if(server_loop_params == NULL){
	    SET_ERRNO_DYLD_DEBUG_ERROR(errno, 74);
	    return(DYLD_FAILURE);
	}
...
#endif
	{
	    ...
	    SET_MACH_DYLD_DEBUG_ERROR(k, 73);
	    return(DYLD_FAILURE);
	}
	...
	if(pthread_create(&server_loop_thread, NULL,
	    ...
	    SET_ERRNO_DYLD_DEBUG_ERROR(errno, 71);
	    return(DYLD_FAILURE);
	}
	...
	k = user_dyld_debug_add_event_subscriber(state.debug_port, rcv_timeout,
        ...
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 70);
            d = DYLD_FAILURE;
	}
	else if(d != DYLD_SUCCESS)
	    SET_LOCAL_DYLD_DEBUG_ERROR(69);
#else
...
mach_port_t subscriber)
{
	...
	k = user_dyld_debug_add_event_subscriber(state.debug_port, rcv_timeout,
		inconsistent_data_ok, subscriber, &d);
        if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 68);
            d = DYLD_FAILURE;
	}
	else if(d != DYLD_SUCCESS)
	    SET_LOCAL_DYLD_DEBUG_ERROR(67);
#else
	user_dyld_debug_add_event_subscriber(state.debug_port, send_timeout,
		rcv_timeout, inconsistent_data_ok, subscriber);
#endif
...
struct _dyld_debug_task_state *state)
{
	...
	if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 57);
	    return(DYLD_FAILURE);
	}
...
struct _dyld_debug_task_state *state)
{
	...
	if(data.debug_thread == MACH_PORT_NULL ||
	   data.debug_port == MACH_PORT_NULL){
		...
		k = task_suspend(target_task);
		if(k != KERN_SUCCESS){
		    SET_MACH_DYLD_DEBUG_ERROR(k, 53);
		    return(DYLD_FAILURE);
		}
	    }
...
task_port_t target_task)
{
	...
	do{
		...
		k = vm_read(target_task, address, size, &data, &data_count);
		    ...
		    SET_MACH_DYLD_DEBUG_ERROR(k, 49);
		    return(DYLD_FAILURE);
		}
		address += size;
	    }
...
#endif
	if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 44);
	    return(DYLD_FAILURE);
	}
	if(address != data_seg_vmaddr + dyld_vmslide){
	    SET_LOCAL_DYLD_DEBUG_ERROR(43);
	    return(DYLD_FAILURE);
	}
#ifdef __MACH30__
...
struct dyld_data_section *dyld_data)
{
	...
	do{
		...
		SET_MACH_DYLD_DEBUG_ERROR(k, 38);
		return(DYLD_FAILURE);
	    }
	}while(k != KERN_NO_SPACE && mh == NULL);
...
#endif
	if(k != KERN_SUCCESS){
	    SET_MACH_DYLD_DEBUG_ERROR(k, 33);
	    return(DYLD_FAILURE);
	}
