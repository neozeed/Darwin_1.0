...
#include <Script.h>					// for smRegionCode
...
CFStringRef CFStringGetNameOfEncoding(CFStringEncoding theEncoding) {
    ...
    if (!theName) {
        if (theEncoding == kCFStringEncodingUnicode) {
            theName = CFSTR("Unicode");
        } else if (theEncoding == kCFStringEncodingUTF8) {
            theName = CFSTR("UTF-8");
        } else if (theEncoding == kCFStringEncodingNonLossyASCII) {
            theName = CFSTR("Non-lossy ASCII");
        } else {
            const UInt8 *encodingName = CFStringEncodingName(theEncoding);

            if (encodingName) {
                theName = CFStringCreateWithCString(NULL, encodingName, CFStringGetSystemEncoding());
            }
...
CFStringEncoding CFStringConvertIANACharSetNameToEncoding(CFStringRef  charsetName) {
    ...
    } else if (CFStringCompare(lowerCharsetName, CFSTR("utf-16"), kCFCompareCaseInsensitive) == kCFCompareEqualTo) {
        CFRelease(lowerCharsetName);
    ...
    if (mappingTable == NULL) {
        ...
        while (*encodings != kCFStringEncodingInvalidId) {
            ...
            if (nameList) {
                while (*nameList) {
                    CFStringRef name = CFStringCreateWithCString(NULL, *nameList++, CFStringGetSystemEncoding());
...
CFStringRef CFStringConvertEncodingToIANACharSetName(CFStringEncoding encoding) {
    ...
    if (!theName) {
        if (encoding == kCFStringEncodingUnicode) {
            theName = CFSTR("iso-10646-ucs-2");
        } else {
            const char **nameList = CFStringEncodingCanonicalCharsetNames(encoding);

            if (nameList && *nameList) {
                theName = CFStringCreateWithCString(NULL, *nameList, CFStringGetSystemEncoding());
            }
       }
...
UInt32 CFStringConvertEncodingToNSStringEncoding(CFStringEncoding theEncoding) {
    switch (theEncoding) {
        ...
        case kCFStringEncodingUTF8: return NSUTF8StringEncoding;
...
static const UInt16 _CFToDOSCodePageList[] = {
...
static const UInt16 _CFToWindowsCodePageList[] = {
...
CF_INLINE SInt32 bsearchEncoding(unsigned short target) {
