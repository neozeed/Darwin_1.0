...
static CFMutableDictionaryRef loadPListDict(CFStringRef filename)
{
    ...
    size_t cfilenamelen = CFStringGetMaximumSizeForEncoding(CFStringGetLength(filename),
							    CFStringGetSystemEncoding()) + 1;
    char *cfilename = (char*)malloc(cfilenamelen);

    if (!cfilename) return nil;

    if (!CFStringGetCString(filename, cfilename, PATH_MAX, CFStringGetSystemEncoding()) ||
...
static void savePListDict(CFDictionaryRef plist, CFStringRef filename)
{
	...
	const void *bytes = (length) ? CFDataGetBytePtr(xmldata) : (const void *)"";
	size_t cfilenamelen = CFStringGetMaximumSizeForEncoding(CFStringGetLength(filename),
								CFStringGetSystemEncoding()) + 1;
	char *cfilename = (char*)malloc(cfilenamelen);
	int filedesc;
