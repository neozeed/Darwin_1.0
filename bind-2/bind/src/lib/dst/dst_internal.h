...
#ifdef REPORT_MALLOC
#define DST_REPORT(a,b)		exit(b);
#else
#define DST_REPORT(a,b)		perror(a), exit(b)
#endif
...
int dst_bsafe_des(const u_int8_t *key, const u_int8_t *ibuf,
		  u_int8_t *obuf, const int len, const int mode);
#endif /* BSAFE */

int dst_rsaref_init();
#ifdef RSAREF
int dst_rsaref_des(const u_int8_t *key, const u_int8_t *ibuf,
		   u_int8_t *obuf, const int len, const int mode);
#endif /* RSAREF */
...
int       dst_s_get_passwd( u_int8_t *pass, const char *msg,
			    const int pass_len);
