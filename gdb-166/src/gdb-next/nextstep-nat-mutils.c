...
unsigned int child_get_pagesize ()
{
  ...
  if (stats == NULL) {
    ...
    if (kret != KERN_SUCCESS) {
      error ("Unable to determine maximum page size: %s (0x%lx)", MACH_ERROR_STRING (kret), kret);
    }
    stats = &stats_data;
  }
...
{
  error ("assertion failure on line %u of \"%s\" in function \"%s\": %s (0x%lx)\n",
	 line, file, func, str);
}

void gdb_check_fatal (const char *str, const char *file, unsigned int line, const char *func)
{
  internal_error ("assertion failure on line %u of \"%s\" in function \"%s\": %s (0x%lx)\n",
	 line, file, func, str);
}
