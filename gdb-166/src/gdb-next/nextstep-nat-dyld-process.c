...
(const struct next_inferior_status *s, struct dyld_objfile_entry *e)
{
  ...
  if (! e->dyld_valid) {
    dyld_debug ("Unable to determine name for dynamic library at 0x%lx (offset 0x%lx)"
		"; skipping (dyld information not valid for library)\n",
  ...
  if (s->dyld_status.state != dyld_started) {
    dyld_debug ("Unable to determine name for dynamic library at 0x%lx (offset 0x%lx)"
		"; skipping (no dyld thread present in inferior)\n",
		(unsigned long) e->dyld_addr, 
		(unsigned long) e->dyld_slide);
    return;
  }
  ...
  if (kret != KERN_SUCCESS) {
    warning ("Unable to deallocate memory used to read module name: %s (0x%lx)",
...
void dyld_load_symfile (struct dyld_objfile_entry *e) 
{
  ...
  if (load_symbols) {
    printf_filtered ("Reading symbols for framework %s...", name);
    ...
    
    {
      ...
      addrs.text_addr = e->dyld_slide;
      addrs.data_addr = e->dyld_slide;
      addrs.bss_addr = e->dyld_slide;
  ...
  if (inferior_auto_start_cfm_flag && (strstr (leaf, "CarbonCore") != NULL)) {
    ...
    if (sym != NULL) {
      ...
      next_status->cfm_info.info_api_cookie = (void *) ((unsigned long) addr);
