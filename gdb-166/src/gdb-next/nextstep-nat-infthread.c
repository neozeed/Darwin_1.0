  ...
  (struct next_inferior_status *inferior, int step, thread_t current, int stop_others)
{
  ...
  if (step) {
    next_save_exception_ports (inferior->task, &inferior->saved_exceptions_step);
    inferior->saved_exceptions_stepping = 1;
  }
}
...
void prepare_threads_after_stop (struct next_inferior_status *inferior)
{
  ...
  if (inferior->saved_exceptions_stepping) {
    next_restore_exception_ports (inferior->task, &inferior->saved_exceptions_step);
    inferior->saved_exceptions_stepping = 0;
  }
