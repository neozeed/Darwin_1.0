 ...
 exception_type_t exception_type, exception_data_t exception_data, mach_msg_type_number_t data_count)
{
  ...
  assert ((inferior->saved_exceptions.masks[original_port_index] & (1 << exception_type)) != 0);
 ...
 mach_msg_type_number_t data_count)
{
  ...
  inferior_debug ("                  port: 0x%lx\n", port);
  inferior_debug ("           thread_port: 0x%lx\n", thread_port);
  inferior_debug ("             task_port: 0x%lx\n", task_port);
  inferior_debug ("        exception_type: 0x%lx (%s)\n", exception_type, unparse_exception_type (exception_type));
  inferior_debug ("            data_count: 0x%lx\n", data_count);

  for (i = 0; i < data_count; ++i) {
    inferior_debug ("   exception_data[%d]: 0x%lx\n", i, exception_data[i]);
  }
