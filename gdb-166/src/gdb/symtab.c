     ...
     char ***canonical;
{
  ...
  *canonical = canonical_arr;
  ...
  filename = s->filename;
  if ((symname != NULL) && ((symname[0] == '-') || (symname[0] == '+')))
    {
  ...
  if (allow_objc_selectors_flag)
    {
      char *copy = *argptr;
      char *ss = copy;
      for (ss = *argptr; (*ss != '\0') && !isspace (*ss); ss++)
	;
      while (isspace (*ss)) 
	ss++;
...
#if 0
  /* If nothing to match on, let's just return now.
     Otherwise matching on the empty string can take quite a while,
     since it involves building a list of every known symbol... */

  if ((sym_text == NULL || *sym_text == '\0') && /* empty string or NULL */
      (word     == NULL || *word     == '\0'))   /* empty string or NULL */
    {
      if (! query ("Really complete on empty string?"))
        return return_val;
    }
#endif
