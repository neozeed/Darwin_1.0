...
#else
      lowest_sect = bfd_get_section_by_name (abfd, TEXT_SECTION_NAME);
      if (lowest_sect == NULL)
...
struct objfile *
allocate_objfile (abfd, flags, mapaddr)
     bfd *abfd;
     int flags;
     CORE_ADDR mapaddr;
{
...
void
update_section_tables (struct dyld_objfile_info *result, struct target_ops *target)
{
  struct objfile *objfile;
