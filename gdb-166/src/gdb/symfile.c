  ...
  for (i = 0; i < MAX_SECTIONS && addrs->other[i].name; i++)
    {
  	...
  	continue;
#if 0
      if (strcmp (TEXT_SECTION_NAME, osp->name) == 0)
	SECT_OFF_TEXT = osp->sectindex ;
      else if (strcmp (DATA_SECTION_NAME, osp->name) == 0)
	SECT_OFF_DATA = osp->sectindex ;
      else if (strcmp (BSS_SECTION_NAME, osp->name) == 0)
     ...
     int verbo;
{
  ...
  if (!addrs->addrs_are_offsets)
    {
      lower_sect = objfile->obfd->sections;
...
struct objfile *
symbol_file_add (name, from_tty, addrs, mainline, flags, mapaddr, prefix)
     const char *name;
     int from_tty;
     struct section_addr_info *addrs;
     int mainline;
     ...
     const char *prefix;
{
...

  {
    ...
    update_section_tables (NULL, &current_target);
    update_section_tables (NULL, &exec_ops);
  }
  ...
  if (args == NULL)
      ...
      while (*argv != NULL)
	{
	  if (STREQ (*argv, "-mapped"))
		...
		{
		  symbol_file_add (name, from_tty, NULL, 1, flags, 0, NULL);
#ifdef HPUXHPPA
		  RESET_HP_UX_GLOBALS ();
#endif
		}
	      else
		{
		  ...
		  symbol_file_add (name, from_tty, &section_addrs, 0, flags, 0, NULL);
		}
     ...
     int from_tty;
{
	  ...
	  if (expecting_option)
	      ...
	      switch (argcnt)
		{
 		...
 		case 1: 
 		  ...
 		  opt[option_index].name = "LC_SEGMENT.__TEXT";
 		  opt[option_index++].value = arg;
 		  break;
 		case 2: 
 		  opt[option_index].type = OPT_SECTION;
 		  opt[option_index].name = "LC_SEGMENT.__DATA";
 		  opt[option_index++].value = arg;
 		  break;
 		case 3: 
 		  opt[option_index].type = OPT_SECTION;
 		  opt[option_index].name = "LC_SEGMENT.__BSS";
 		  opt[option_index++].value = arg;
 		  break;
 		default:
	    ...
	    case OPT_SECTION:
	      {
                ...
                if (strcmp (sec, TEXT_SECTION_NAME) == 0)
                  section_addrs.text_addr = addr;
                else if (strcmp (sec, DATA_SECTION_NAME) == 0)
                  section_addrs.data_addr = addr;
                else if (strcmp (sec, BSS_SECTION_NAME) == 0)
...
#endif
      section_addrs.text_addr = 0;
      section_addrs.data_addr = 0;
      section_addrs.bss_addr = 0;
      section_addrs.addrs_are_offsets = 1;
#if 0
  ...
  symbol_file_add (name, from_tty, &section_addrs, 0, flags, mapaddr, NULL);
