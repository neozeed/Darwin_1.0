...
#undef  CC1_SPEC
#define CC1_SPEC "%{!static:-mno-schedule-prologue -fno-optimize-register-move}"
/* The macro definition above had " %(cc1_spec) " at the end,
   but the old gcc driver may not understand this spec syntax.  */

#if 1 /* Only the old Objective C++ compiler needs this macro definition.  */
...
/* When doing static code gen, the epilogue is generated as rtl.
   For dynamic code gen, the epilogue is currently generated directly
   as assembly language.  */

#undef  FUNCTION_EPILOGUE
#define FUNCTION_EPILOGUE(FILE, SIZE)     \
  do if (flag_pic) ix86_expand_epilogue (); while (0)
#endif
...
/* An attempt to ensure that we do things right in machopic.c.
   It's a bizarre object format, but we're stuck with it.
