...
(define_insn "*movdf_softfloat32"
  [(set (match_operand:DF 0 "nonimmediate_operand" "=r,r,o,r,r,r,r,m")
	(match_operand:DF 1 "input_operand" "r,o,r,G,H,F,m,r"))]
  ...
  "*
{
  ...
  [(set_attr "type" "*,load,store,*,*,*,load,store")
   (set_attr "length" "8,8,8,8,12,16,8,8")])
...
(define_split
  [(set (mem:SV (plus:SI (match_operand:SI 0 "gpc_reg_operand" "")
...
(define_split
  [(set (match_operand:SV 0 "vector_register_operand" "")
	(mem:SV (plus:SI (match_operand:SI 1 "gpc_reg_operand" "")
...
(define_insn ""
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
        (mem:SV (match_operand:SI 1 "gpc_reg_operand" "b")))]
  ""
  "lvx %0,0,%1"
  [(set_attr "type" "vec_load")])
...
;; *** AS WRITTEN, THIS ONLY WORKS FOR ABI_RHAPSODY!!! ***
(define_insn "call_indirect_sysv"
  [(call (mem:SI (match_operand:SI 0 "register_operand" "r,r"))
...
(define_insn "call_value_indirect_sysv"
  [(set (match_operand 0 "register_operand" "=fg,fg")
	(call (mem:SI (match_operand:SI 1 "register_operand" "r,r"))
...
(define_insn "*ret_call_nonlocal_aix32"
  [(set (match_operand 0 "" "=fgv,fgv")
	(call (mem:SI (match_operand:SI 1 "call_operand" "s,s"))
	      (match_operand 2 "" "fg,fg")))  /* vmx had "g,g"  */
   (use (match_operand:SI 3 "immediate_operand" "O,n"))
   (clobber (match_scratch:SI 4 "=l,l"))]
  "(DEFAULT_ABI == ABI_AIX || DEFAULT_ABI == ABI_NT)
   && (INTVAL (operands[3]) & CALL_LONG) == 0"
...
;;
;; Hackery for vec_abs(), which maps to a sequence of three instructions.
;; This solution is completely unsatisfactory. It requires looking at the
;; opcode name to determine what code we generate.  This is as a consequence
;; of the vector stuff only using signatures such as "xfx" meaning
;; "vector := func(vector)" without the possibility of explicitly naming the
;; function (which would allow us to write the mappings properly!)
;; We use "xfx_complex" for vec_abs as there are no other such signatures.
;; Note: we have to use define_expand() because SCRATCH registers of type
;; SVmode cause problems (egcs tries to save them on the stack using r1+offs
;; mode, which isn't kosher.)  Hence the use of gen_reg_rtx(). 

(define_expand "xfx_complex"
  [(use (match_operand:SV 0 "vector_register_operand" "=v"))
   (use (match_operand:SV 1 "vector_register_operand" "v"))
   (use (match_operand:SV 2 "any_operand" ""))]
  ""
  "
{
  rtx spare;
  spare = gen_reg_rtx (SVmode);
  emit_insn (gen_rtx_CLOBBER (VOIDmode, spare));
  emit_insn (gen___vec_abs (operands[0], operands[1], operands[2], spare));

  DONE;
}")

(define_insn "__vec_abs"
  [(set (match_operand:SV 0 "vector_register_operand" "=v")
	(unspec:SV [(match_operand:SV 1 "vector_register_operand" "v")
		    (match_operand 2 "any_operand" "")
                    (match_operand:SV 3 "vector_register_operand" "=v")] 9))]
  ""
  "*
{
  const char *op = XSTR (operands[2], 0);	/* The opcode name.  */

  if (! strcmp (op, \"vabsssb\") || ! strcmp (op, \"vabsssh\")
   || ! strcmp (op, \"vabsssw\"))
    switch (op[6]) {
    case 'b': return \"vspltisb %3,0\;vsubsbs %3,%3,%1\;vmaxsb %0,%1,%3\";
    case 'h': return \"vspltisb %3,0\;vsubshs %3,%3,%1\;vmaxsh %0,%1,%3\";
    case 'w': return \"vspltisb %3,0\;vsubsws %3,%3,%1\;vmaxsw %0,%1,%3\";
    }
  else
  if (! strcmp (op, \"vabsfp\") || ! strcmp (op, \"vabssb\")
   || ! strcmp (op, \"vabssh\") || ! strcmp (op, \"vabssw\"))
    switch (op[5]) {
    case 'p': return \"vspltisw %3,-1\;vslw %3,%3,%3\;vandc %0,%1,%3\";
    case 'b': return \"vspltisb %3,0\;vsububm %3,%3,%1\;vmaxsb %0,%1,%3\";
    case 'h': return \"vspltisb %3,0\;vsubuhm %3,%3,%1\;vmaxsh %0,%1,%3\";
    case 'w': return \"vspltisb %3,0\;vsubuwm %3,%3,%1\;vmaxsw %0,%1,%3\";
    }
  abort();
}"
  [(set_attr "type" "vec_simple")
...
;;
;; Hackery to get reversed args for vcmple and vcmplt, which map to
;; vcmpge and vcmpgt with reversed arguments..
;; This is because our assembler doesn't yet recognise these "instructions"
;; Currently, we simply change the 'l' to a 'g', which is despicable.
;;
(define_insn "xfxx_simple"
  ...
  "*
{
  const char *opnam = XSTR (operands[3], 0);	/* The opcode name.  */ 

  if (opnam[4] == 'l'
     && (!strncmp (opnam, \"vcmplt\", 6) || !strncmp (opnam, \"vcmple\", 6)))
    {
      static char buf[32];
      strcpy(buf, opnam); strcat(buf, \" %0, %2, %1\");
      buf[4] = 'g';  /* Hackery! Change from vcmplt to vcmpgt.  */
      return buf;
    }
  return \"%3 %0,%1,%2\";
}"
