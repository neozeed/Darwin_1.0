...
/* Don't use the egcs' builtin setjmp and longjmp for exception handling,
   since they are not AltiVec or PIC-base-register aware.  */
...
/* Define cutoff for using external functions to save vector registers.
   Currently on Rhapsody, always use inline save/restores.  Bloat City!  */

#undef VECTOR_SAVE_INLINE 
#define VECTOR_SAVE_INLINE(FIRST_REG) ((FIRST_REG) >= 78 && (FIRST_REG) <= 109)
...
/* <tur 25May99> For VMX, BIGGEST_ALIGNMENT is 128.
   ...
   *** UNFORTUNATELY those naughty people in Kernel City have assumed only
   32-bit alignment.  This is plain stupid.  What are we supposed to do
   with kernel code with vector registers?  */

/* No data type wants to be aligned rounder than this.  */
#undef  BIGGEST_ALIGNMENT
#define BIGGEST_ALIGNMENT (flag_vec ? 128 : 32)
extern int flag_vec;
