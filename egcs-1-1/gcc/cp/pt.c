     ...
     tree in_decl;
{
  ...
  switch (TREE_CODE (t))
	...
	/*
	 * if the function causing this template to instantiated
	 * is inline, then don't output this instantiated template
	 * until we know for sure that the inline has been used.
	 */

	if (current_function_decl && DECL_INLINE(current_function_decl)) 
		DECL_DEFER_OUTPUT(r) = 1 ;
