     ...
     tree binfo, type;
{
  ...
  /* Tell the type itself about at least one of its vtables!
      Had to add this here in build_vtable, and also in finish_vtbls.
  */
  if (TYPE_BINFO_VTABLE (type) == 0)
    TYPE_BINFO_VTABLE (type) = decl;
#endif
...
#ifdef NEXT_SEMANTICS
	  /* Tell the type itself about at least one of its vtables!   Had
             to add this here in finish_vtbls, and also in build_vtable.  */
	  if (TYPE_BINFO_VTABLE (t) == 0)
	    TYPE_BINFO_VTABLE (t) = decl;
#endif
	}
     ...
     tree name;
{
  *current_lang_stack++ = current_lang_name;
  ...
  if (name == lang_name_java)
    {
      strict_prototype = strict_prototypes_lang_cplusplus;
      current_lang_name = name;
    }
  else if (name == lang_name_cplusplus)
    {
      strict_prototype = strict_prototypes_lang_cplusplus;
#ifdef OBJCPLUS
