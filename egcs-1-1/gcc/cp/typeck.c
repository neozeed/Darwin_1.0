     ...
     tree values;
{
  ...
  /* Walk the values list placing the converted constants into valarray and counting
  ...
  /* A single constant value is shorthand for a duplicated value.  Duplicate it
     now.  */
     ...
     tree exp;
{
  ...
  while (1)
	...
	/*
	 * Address of function is the same as calling the function
	 * as far as the call graph is concerned. 
	 * Let the call graph know of the dependency and if we are
	 * not currently in an inline, tell the call graph that
	 * the function is really called. For static constructors,
	 * current_function_decl will be NULL_TREE.
	 *
	 */
	...
	add_call_graph_dependent(current_function_decl, x) ;

	if ((current_function_decl == NULL_TREE) || 
	   !DECL_INLINE(current_function_decl))
		call_graph_function_called(x ,call_graph_call_action) ;
