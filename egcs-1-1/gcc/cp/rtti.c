...
/* Apple's PIC code is incompatible with .common sections,
   ...
   record any vtable in TYPE_BINFO_VTABLE, and I use that info, here.
*/
    ...
    tree type;
{
  ...
  if (TYPE_VIRTUAL_P (type))   /* ==> Should be a vtable somewhere */
    {
      tree tbiv = TYPE_BINFO_VTABLE( type );
      if (tbiv)
	{
	  ...
	  TREE_PUBLIC (tdecl) = TREE_PUBLIC (tbiv);
	  DECL_EXTERNAL (tdecl) = DECL_EXTERNAL (tbiv);
	  return; 
	}
      else
	{
