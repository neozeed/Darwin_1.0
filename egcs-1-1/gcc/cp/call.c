...
/*
 * call_graph_call_action - This function is called on each decl node
 * of each function called as a result of the current function being
 * called. It is also called on the current function being called.
 */

void call_graph_call_action(decl)
	tree decl ;
{

  /*
   * if we are calling an instantiated template function, 
   * un-defer its output. The compiler doesn't always seem to fill
   * in the language specific fields so guard against that with
   * DECL_LANG_SPECIFIC.
   */
     ...
     tree function, result_type, parms;
{
  ...
  /*calculate decl to be added to call graph.*/
  ...
  /*if pattern not recognized. Probably an indirect call.*/
  ...
  /*
   * update call graph
   * Prevent recursive calls from making loops in the graph.
   */

  if (current_function_decl != decl) 
  	add_call_graph_dependent(current_function_decl, decl) ;

  /*
   * if the caller is anonymous (as it is for static constructors)
   * or if this isn't an inline, then we are really calling 
   * a function. Let call graph know.
   */

  if ((current_function_decl == NULL_TREE) || 
    !DECL_INLINE(current_function_decl))
	call_graph_function_called(decl,call_graph_call_action) ;
