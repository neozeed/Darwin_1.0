...
/* Nonzero for -fvec: Enable the SIMD programming model.  */
...
finish_objects (method_type, needs_cleaning)
     int method_type;
     int needs_cleaning;
{
  static tree dfndecl;
     ...
     int method_type;
{
  ...
  /* Finish up. */
#ifdef NEXT_SEMANTICS
  if (method_type == 'I' && needs_cleaning && DYNAMIC_DESTRUCTORS && dfndecl)
    {
      /* As the last action in the constructor-caller, make it
         call atexit to schedule the call of the corresponding
	 dtor-caller.  Note that the compiler has already generated
	 the dtor-caller, by the time we're here doing the ctor-caller.  */
  ...
  pop_momentary ();
#ifdef NEXT_SEMANTICS
  /* Squirrel away the current (dtor-caller) function, so that
     the above "Finish[ing] up" of the ctor-caller can call atexit
     to schedule the calling of this guy. */
  if (method_type == 'D' && DYNAMIC_DESTRUCTORS)
...
#ifdef NEXT_SEMANTICS
  finish_objects ('I', needs_cleaning);
#else
  finish_objects ('I');
#endif
}
...
finish_file ()
{
  ...
  for (fnname = pending_templates; fnname; fnname = TREE_CHAIN (fnname))
    {
      ...
      /*
       * skip template if he have marked it as not being used.
       * This can happen if an inline instantiated the template.
       */
  ...
  if (needs_messing_up)
    {
#ifdef NEXT_SEMANTICS
      /* We have to pass the dtors?-flag to the ctor-caller in
	 order that it can use "atexit" to schedule the dtor-caller. */ 
      do_ctors (needs_cleaning);
#else
      do_ctors ();
#endif
