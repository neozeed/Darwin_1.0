     ...
     char *filename;
{
  ...
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_PIXEL],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_PIXEL]));
  ridpointers[(int) RID_VECTOR] = get_identifier ("__vector");
  SET_IDENTIFIER_AS_LIST (ridpointers[(int) RID_VECTOR],
			  build_tree_list (NULL_TREE, ridpointers[(int) RID_VECTOR]));
...

	{
	  ...
	  if ((ptr = is_reserved_word (token_buffer, p - token_buffer)))
	      ...
	      if (ptr->rid)
		{
                  ...
                  if (flag_vec && ptr->rid == RID_VECTOR)
                    {
		      int is_keyword = 0;
...
#define TOTAL_PARTS ((HOST_BITS_PER_WIDE_INT / HOST_BITS_PER_CHAR) * 2 + 2)
	...
	*p = 0;
	...
	/* ???MERGE--> make warn_4cc => warn_multichar?
	      Remove warn_multichar completely?    */
	else if (num_chars != 1)
     ...
     tree t;
{
  ...
  if (r && flag_vec && (r->rid == RID_VECTOR || r->rid == RID_PIXEL)
