 ...
 * Call graph tools.
 * 
 * This call graph was originally implemented (6/2/99) to allow 
 * the usage of template instantiations to be tracked in C++.
 * It consists of a list of parent nodes, each of which have
 * a list of child nodes. The parent node's purpose field points
 * to the caller's decl node. The parent's value field points to 
 * the child list. Each child represents a callee and the purpose
 * field points to the callee's decl node. Each callee also has
 * node on the parent list since it could potentially make calls also.
 * Each child node's value field points to the callee's node on the
 * parent list.
 * 
 * 6/22/99 -  Initial. Place all inlines as parents in the graph.
 *            Any templates instantiated by the inline are placed in
 *            call graph as children and the decl node has the 
 *            defer output bit set. If the inline is used, the decl
 *            nodes of the children have the defer output reset. 
 *            In addition, the parent and all child list nodes
 *	      have the tree used bit set to true.
 *
 */


/*
 * add_call_graph_dependent - For the given caller, add a callee.
 */

void add_call_graph_dependent(caller, callee) 
	tree caller, callee ;
{

  tree parent_item, child_on_parent_list ;

  /*prevent recursive functions from making loops in the graph. */

  if (caller == callee)
	return ;

  /*if caller is anonymous, then caller can't be added to call graph.*/

  if (caller == NULL_TREE)
	parent_item = NULL_TREE ;

  /*if caller is already on caller list.*/

  else if (parent_item = purpose_member(caller,call_graph)) {

	/*if callee is already on the caller's callee list, do nothing.*/

	if (purpose_member(callee, TREE_PURPOSE(parent_item))) 
		return ;

	}

  /*add caller to caller list.*/

  else 
	parent_item = call_graph = perm_tree_cons(caller, NULL_TREE, call_graph) ;
  
  /*add callee to the caller list as a potential caller.*/

  if ((child_on_parent_list = purpose_member(callee,call_graph)) == NULL_TREE)
	child_on_parent_list = 
	  call_graph = perm_tree_cons(callee, NULL_TREE, call_graph) ;
  
  /*if caller is not anonymous, add a callee to caller's callee list.*/

  if (parent_item != NULL_TREE)
  	TREE_VALUE(parent_item) = 
	   perm_tree_cons(callee,child_on_parent_list,TREE_VALUE(parent_item)) ;

}


static void call_graph_mark_used(callee_item,action)
	tree callee_item ;
	void (*action)() ;
{
  ...
  if (!callee_item || TREE_USED(callee_item))
	return ;

  
  for ((*action)(TREE_PURPOSE(callee_item)),TREE_USED(callee_item) = 1,
	walk_ptr = TREE_VALUE(callee_item) ;
       walk_ptr ; walk_ptr = TREE_CHAIN(walk_ptr)) {

	call_graph_mark_used(TREE_VALUE(walk_ptr), action) ;
	}
...
/*
 * call_graph_function_called - locate callee in call graph and all callee's
 * dependents. For each one, mark the list node used and call the supplied
 * action routine on the decl node for the function. The action is called
 * only once per function.
 *
 */

void call_graph_function_called(callee,action) 
	tree callee ;
	void (*action)() ;
{
  tree callee_item ;

  if ((callee_item = purpose_member(callee,call_graph)) == NULL_TREE)
	return ;

  if (TREE_USED(callee_item))
	return ;

  call_graph_mark_used(callee_item,action) ;
  return ;
}
