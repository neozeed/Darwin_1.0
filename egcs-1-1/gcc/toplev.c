     ...
     char **envp;
{
  ...
  /* Don't allow optimization levels LESS than one, since the compiler
     might crash otherwise!  */
#ifdef FIXED_PIC_REG
  /* Inlining seems to be turned on automatically with -O3, even if explicitly
     disabled, and this can cause the compiler to crash.  Though the level of
     optimization is limited here to a max of 2, inlining can still be enabled
     explicitly with -finline-functions, if this is actually desired.  */
  if (optimize > 2)
    optimize = 2;
  else if (optimize < 1)
#endif
