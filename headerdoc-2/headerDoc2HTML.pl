...
# 					print "|$s|\n";
	    ...
	    if (ref($apiOwner) ne "Header") { # if we've been filling a class object, add it to the header
...
sub printVersionInfo {
    my $av = APIOwner->VERSION();
    my $hev = HeaderElement->VERSION();
    my $hv = Header->VERSION();
    my $cppv = CPPClass->VERSION();
    my $fv = Function->VERSION();
    my $tv = Typedef->VERSION();
    my $sv = Struct->VERSION();
    my $cv = Constant->VERSION();
    my $vv = Var->VERSION();
    my $ev = Enum->VERSION();
    my $uv = Utilities->VERSION();
    my $me = MinorAPIElement->VERSION();
