...
sub tocString {
    ...
    $contentFrameName = &Utilities::safeName($contentFrameName);  
...
sub createFramesetFile {
    ...
    $rootFileName =~ s/(.*)\.h/$1/; 
    $rootFileName = &Utilities::safeName($rootFileName);
...
sub createContentFile {
    ...
    if (1 || $isMacOS) {$rootFileName = &Utilities::safeName($rootFileName);};
...
sub _getFunctionsAndParamsExportString {
    ...
    foreach my $obj (sort objName @funcObjs) {
        ...
        $tmpString = &Utilities::convertCharsForFileMaker($tmpString);
        $tmpString =~ s/$sep/\t/g;
        ...
        if (@taggedParams) {
        	...
        	foreach my $taggedParam (@taggedParams) {
        		...
        		$tmpParamString = &Utilities::convertCharsForFileMaker($tmpParamString);
    ...
    foreach my $obj (sort objName @enums) {
        ...
        if (@constants) {
        	foreach my $field (@constants) {
        		...
        		$tmpFieldString = &Utilities::convertCharsForFileMaker($tmpFieldString);
...
sub printObject {
    ...
    print "constants:\n";
    &Utilities::printArray(@{$self->{CONSTANTS}});
    print "functions:\n";
    &Utilities::printArray(@{$self->{FUNCTIONS}});
    print "typedefs:\n";
    &Utilities::printArray(@{$self->{TYPEDEFS}});
    print "structs:\n";
    &Utilities::printArray(@{$self->{STRUCTS}});
    print "\n";
}
