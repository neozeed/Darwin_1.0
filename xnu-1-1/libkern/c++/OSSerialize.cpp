...
bool OSSerialize::previouslySerialized(const OSObject* o)
{
	char temp[16];
	OSString *tagString;
...
bool OSSerialize::addXMLStartTag(const OSObject *o, const char *tagString)
{
...
bool OSSerialize::initWithCapacity(unsigned int inCapacity)
{
	...
	data = (char *) kalloc(inCapacity);
	if (!data) {
		tags->release();
		return false;
	}
	bzero((void *)data, inCapacity);
