...
{
    if (((const OSSymbol *) aString)->flags & kOSStringNoCopy)
...
bool OSSymbol::isEqualTo(const OSObject *obj) const
{
    OSSymbol *	sym;
    OSString *	str;

    if ((sym = OSDynamicCast(OSSymbol, (OSObject *)obj )))
	return isEqualTo(sym);
    else if ((str = OSDynamicCast(OSString, (OSObject *)obj )))
