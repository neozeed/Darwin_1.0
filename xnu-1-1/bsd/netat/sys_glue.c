	...
	void *proc;
{
	...
	return((err = at_ioctl(gref, cmd, arg)));
}
 ...
 */
/* *** separate out the error wakeup stuff to eliminate use
       of MSG_ERROR and atalk_putnext(), moving towards 
...
void atalk_notify(gref, errno)
	...
	int errno;
{
  /* *** code from udp_notify() in netinet/udp_usrreq.c 
