...
static int atp_unpack_bdsp(struct atp_state *, gbuf_t *, struct atp_rcb *, 
			   int, int);
void atp_retry_req(), atp_trp_clock(), asp_clock();
...
void atp_unlink()
{
	untimeout(asp_clock, (void *)&atp_inited);
	untimeout(atp_trp_clock, (void *)&atp_inited);
	...
	void *arg;
{
	...
	timeout(atp_trp_clock, (void *)arg, (1<<5));
}
