...
at_ifaddr_t  *ifID_table[IF_TOTAL_MAX];
	/* the table of ifID structures, one per interface 
	   (not just ethernet), indexed by DDP port
...
if_types_t if_types[IF_TYPENO_CNT] = {{ IFT_ETHER, IF_TOTAL_MAX }};
...
static void getIfNames(if_name_t *);
...
void ddp_bit_reverse(), ddp_shutdown();
...
#endif
				error = (!(at_state.flags & AT_ST_STARTED))?
				  		ENOTREADY : 0;
				if (error == 0) {
					int size;
					...
					size = list_pids((int *)gbuf_rptr(tmpm), 
							 10); /* *** 10 just for now *** */
					gbuf_wset(tmpm, (size * sizeof(int)));
					gbuf_set_type(tmpm, MSG_DATA);
					((ioc_t *)gbuf_rptr(m))->ioc_count = gbuf_msgsize(tmpm);
					ioc_ack(0, m, gref);
				} else {
					dPrintf(D_M_ELAP,D_L_INFO,
...
#endif
				if ((gbuf_cont(m) = gbuf_alloc(sizeof(if_name_t) * IF_TOTAL_MAX, 
			    		PRI_MED)) == NULL) {
					ioc_ack(ENOBUFS, m, gref);
					break;
				}
				getIfNames((if_name_t *)gbuf_rptr(gbuf_cont(m)));
				gbuf_wset(gbuf_cont(m),sizeof(if_name_t) * IF_TOTAL_MAX);
				iocbp->ioc_count   = sizeof(if_name_t) * IF_TOTAL_MAX;
				ioc_ack(0, m, gref);
				break;
			default:
#ifdef APPLETALK_DEBUG
     ...
     caddr_t data;
{
	...
	request.ifr_addr.sa_len = 2 + sizeof(struct etalk_addr);
	bcopy (data, &request.ifr_addr.sa_data[0], sizeof(struct etalk_addr));
...
/*
	if ((*nddp->if_ioctl)(nddp, SIOCADDMULTI, (caddr_t)&request))
		return(-1);
*/
	if (if_addmulti(nddp, &request.ifr_addr, 0))
	    return -1;
	else
	    return 0;
	...
	if (*(int *)data) {
		...
		dPrintf(D_M_PAT, D_L_STARTUP,
			 ...
			 *(unsigned*)data, (*(unsigned *)(data+2))&0x0000ffff, 
			 (unsigned)ifID));
/*
		if ((*nddp->if_ioctl)(nddp, SIOCDELMULTI, (caddr_t)&request))
			return(-1);
*/
		bzero(data, sizeof(struct etalk_addr));
	}

	if (if_delmulti(nddp, &request.ifr_addr))
	    return -1;
	else
	    return 0;
}
