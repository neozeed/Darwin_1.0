	...
	aurp_state_t *state;
{
	...
	timeout(AURPsndRIReq, state, AURP_RetryInterval*HZ);
	state->rcv_tmo = 1;
}
	...
	aurp_state_t *state;
{
	...
	if (state->rsp_m == 0) {
		...
		if ((m = (gbuf_t *)gbuf_alloc(msize+AURP_MaxPktSize, PRI_MED)) == 0) {
			timeout(AURPsndRIRsp, state, AURP_RetryInterval*HZ);
			state->snd_tmo = 1;
			return;
		}
		gbuf_wset(m,msize);
	...
	aurp_state_t *state;
{
	...
	/* start the retry timer */
	timeout(AURPsndRIUpd, state, AURP_RetryInterval*HZ);
	state->snd_tmo = 1;
	...
	gbuf_t *m;
{
	...
	if (((snd_state == AURPSTATE_WaitingForRIAck1) ||
		...
		if (snd_state == AURPSTATE_WaitingForRIAck1) {
			...
			untimeout(AURPsndRIRsp, state);
			dat_m = state->rsp_m;
			state->rsp_m = 0;
			flag = 1;
		} else {
			/* ack from the tunnel peer to our RI update */
			untimeout(AURPsndRIUpd, state);
			dat_m = state->upd_m;
			state->upd_m = 0;
			flag = 2;
		}
