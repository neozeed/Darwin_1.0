     ...
     struct sockopt *sopt;
{
	...
	case SOPT_GET:
		...
		case DDP_STRIPHDR:
			...
			break;
		case DDP_SLFSNDOFF:
			optval = at_pcb->ddp_flags & DDPFLG_SLFSNDOFF;
			error = sooptcopyout(sopt, &optval, sizeof optval);
			break;
	...
	case SOPT_SET:
		...
		case DDP_HDRINCL:
			...
			if (optval)
				...
				at_pcb->ddp_flags &= ~DDPFLG_HDRINCL;
			break;
		case DDP_CHKSUM_ON:  /* *** not yet implemented *** */
			error = sooptcopyin(sopt, &optval, sizeof optval,
					    sizeof optval);
			if (error)
				break;
		...
		case DDP_STRIPHDR:
			...
			if (optval)
				...
				at_pcb->ddp_flags &= ~DDPFLG_STRIPHDR;
			break;
		case DDP_SLFSNDOFF:  /* *** not yet implemented *** */
			error = sooptcopyin(sopt, &optval, sizeof optval,
					    sizeof optval);
			if (error)
				break;
			if (optval)
				at_pcb->ddp_flags |= DDPFLG_SLFSNDOFF;
			else
				at_pcb->ddp_flags &= ~DDPFLG_SLFSNDOFF;
			break;
                default:
...
static	u_short ddp_checksum(mp, offset)
     register gbuf_t	*mp;
     register int	offset;
{
...
{
	dPrintf(D_M_DDP, D_L_VERBOSE, ("ddp_get_stats() entry"));
	bcopy(&at_ddp_stats, statsp, sizeof(at_ddp_stats));
	return(0);
}
      
StaticProc void sum_pkt_chain(m)
     gbuf_t *m;
{
	gbuf_t *tmp_m = m;
	register at_ddp_t 
...
{
	register at_ifaddr_t	*ifID, *ifIDTmp;
	...
	if ((ddp->dst_node == 0xfe) && 
		...
		NET_ASSIGN(ddp->dst_net, ifID->ifARouter.s_net);
		dst_net = ifID->ifARouter.s_net;
		ddp->dst_node = ifID->ifARouter.s_node;
	}
