...
#define dprintf(a)	if (!fw_debug); else kprintf a

#define print_ip(a)	kprintf("%d.%d.%d.%d",				\
	...
	struct ifnet *rif, struct ifnet *oif)
{
	...
	/* Print command name */
	kprintf("ipfw: %d ", f ? f->fw_number : -1);
	if (!f)
		kprintf("Refuse");
	else
		...
		case IP_FW_F_COUNT:
			...
			break;
		case IP_FW_F_DIVERT:
			kprintf("Divert %d", f->fw_divert_port);
			break;
		case IP_FW_F_TEE:
			kprintf("Tee %d", f->fw_divert_port);
			break;
		case IP_FW_F_SKIPTO:
			kprintf("SkipTo %d", f->fw_skipto_rule);
			break;
#if DUMMYNET
		case IP_FW_F_PIPE:
			kprintf("Pipe %d", f->fw_skipto_rule);
			break;
#endif
		...
		case IP_FW_F_FWD:
			...
			if (f->fw_fwd_ip.sin_port)
				kprintf(":%d", f->fw_fwd_ip.sin_port);
			break;
#endif
		default:	
			kprintf("UNKNOWN");
	...
	case IPPROTO_TCP:
		...
		if ((ip->ip_off & IP_OFFMASK) == 0)
			kprintf(":%d ", ntohs(tcp->th_sport));
		else
			kprintf(" ");
		print_ip(ip->ip_dst);
		if ((ip->ip_off & IP_OFFMASK) == 0)
			kprintf(":%d", ntohs(tcp->th_dport));
		break;
	case IPPROTO_UDP:
		kprintf("UDP ");
		print_ip(ip->ip_src);
		if ((ip->ip_off & IP_OFFMASK) == 0)
			kprintf(":%d ", ntohs(udp->uh_sport));
		else
			kprintf(" ");
		print_ip(ip->ip_dst);
		if ((ip->ip_off & IP_OFFMASK) == 0)
			kprintf(":%d", ntohs(udp->uh_dport));
		break;
	case IPPROTO_ICMP:
		if ((ip->ip_off & IP_OFFMASK) == 0)
			kprintf("ICMP:%u.%u ", icmp->icmp_type, icmp->icmp_code);
		else
			kprintf("ICMP ");
		print_ip(ip->ip_src);
		kprintf(" ");
	...
	if (oif)
		kprintf(" out via %s%d", oif->if_name, oif->if_unit);
	else if (rif)
		kprintf(" in via %s%d", rif->if_name, rif->if_unit);
	if ((ip->ip_off & IP_OFFMASK)) 
		kprintf(" Fragment = %d",ip->ip_off & IP_OFFMASK);
	kprintf("\n");
	if (fw_verbose_limit != 0 && count == fw_verbose_limit)
		kprintf("ipfw: limit reached on rule #%d\n",
		f ? f->fw_number : -1);
    }
}
...
zero_entry(struct ip_fw *frwl)
{
	...
	if (fw_verbose) {
		if (frwl)
			kprintf("ipfw: Entry %d cleared.\n", frwl->fw_number);
		else
			kprintf("ipfw: Accounting cleared.\n");
	}
...
#endif
#if IPFIREWALL_FORWARD
	kprintf("rule-based forwarding enabled, ");
#else
	kprintf("rule-based forwarding disabled, ");
#endif
#if IPFIREWALL_DEFAULT_TO_ACCEPT
