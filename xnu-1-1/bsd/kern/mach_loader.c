...
)
{
	...
	kern_return_t		kret;
	load_return_t		lret;
	extern void map_ref_fixup(vm_map_t,vm_map_t);
	...
	*result = (load_result_t) { 0 };
	...
	set_task_map(current_task(),map);
	set_act_map(current_act(),map);
	/****** FIX ME ***/
	map_ref_fixup(old_map, map);
...
)
{
	...
	if (ret == LOAD_SUCCESS && dlp != 0) {
		...
		if (dylink_test) {
			addr = 0x20000000;
			vm_map(map, &addr, 0x10000000, 0, SHARED_LIB_ALIAS,
				shared_text_region_handle, 0, FALSE,
				VM_PROT_READ, VM_PROT_READ, VM_INHERIT_SHARE);
			addr = 0x30000000;
			vm_map(map, &addr, 0x10000000, 0, SHARED_LIB_ALIAS,
				shared_data_region_handle, 0, TRUE,
				VM_PROT_READ, VM_PROT_READ, VM_INHERIT_SHARE);
		}
