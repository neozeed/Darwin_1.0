...
extern void PE_get_timebase(unsigned long long *);
...
unsigned int debugid, arg1, arg2, arg3, arg4, arg5;
{
	...
	PE_get_timebase((unsigned long long *)&kd->timestamp);
	kd_bufptr++;
...
int totalcount=0;
	...
	if (count) {
		if ((kdebug_flags & KDBG_BUFINIT) && kd_bufsize && kd_buffer) {
			if (count > nkdbufs)
			        count = nkdbufs;
			if (!(kdebug_flags & KDBG_WRAPPED) && (kd_bufptr > kd_readlast))
			  {
			    copycount = kd_bufptr-kd_readlast;
			    if (copycount > count)
			      copycount = count;
			...
			else if (!(kdebug_flags & KDBG_WRAPPED) && (kd_bufptr == kd_readlast))
			  {
			    *number = 0;
			    return(0);
			  }
			else
			  {
			    ...
			    /* Note that by setting kd_readlast equal to kd_bufptr,
			       we now treat the kd_buffer read the same as if we weren't
			       wrapped and kd_bufptr was less than kd_readlast.
			    */
