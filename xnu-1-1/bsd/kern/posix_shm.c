...
pshm_mmap(struct proc *p, struct mmap_args *uap, register_t *retval, struct file *fp, vm_size_t pageoff) 
{
	...
	int prot = uap->prot;
	int flags = uap->flags;
	vm_offset_t	file_pos = (vm_offset_t)uap->pos;
	int fd = uap->fd;
	vm_map_t	user_map;
	...
	if (user_size  + file_pos > pinfo->pshm_length) {
		return(EINVAL);
	}
	...
	kret = vm_allocate(user_map, &user_addr, user_size, find_space);
	if (kret != KERN_SUCCESS) 
		goto out;
	docow = FALSE;	

	kret = vm_map(user_map, &user_addr, user_size,
