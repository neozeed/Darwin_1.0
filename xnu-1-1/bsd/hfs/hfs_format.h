 ...
 * hfs_format.c
 *
 * This file describes the on-disk format for hfs/HFS Plus volumes.
...
/*
 * Mac OS X special directory for linked and/or deleted files (HFS Plus only)
 *
 * to make this folder name sort last...
 * it has 0xFFE5 Unicode prefix (0xEF, 0xBF, 0xA5 in UTF-8)
 */
#define HFSPLUSMETADATAFOLDER  "\xEF\xBF\xA5\xEF\xBF\xA5\xEF\xBF\xA5 HFS Private Meta Data"

/*
 * Files in the HFS Private Meta Data folder have one of the following
 * prefixes followed by the file ID in decimal (no leading zeros).
 * e.g.  FileNodeData3296
 */
#define HFS_LINK_PREFIX		"FileNodeData"
#define HFS_DELETE_PREFIX	"DeletedFile"
...
/* HFS Plus Permissions info - 16 bytes */
struct HFSPlusPermissions {
	...
	u_int32_t 	permissions;	/* permissions (bytes: unused, owner, group, everyone) */
	u_int32_t 	specialDevice;	/* UNIX: device for character or block special file */
};
...
struct HFSPlusCatalogFolder {
	...
	u_int32_t 		backupDate;		/* date and time of last backup */
	HFSPlusPermissions	permissions;		/* permissions (for MacOS X) */
	FndrDirInfo 		userInfo;		/* Finder information */
	FndrOpaqueInfo	 	finderInfo;		/* additional Finder information */
...
struct HFSPlusCatalogFile {
	...
	u_int16_t 		flags;			/* file flags */
	u_int32_t 		linkCount;		/* used by Mac OS X */
	u_int32_t 		fileID;			/* file ID */
	u_int32_t 		createDate;		/* date and time of creation */
	u_int32_t 		contentModDate;		/* date and time of last content modification */
	u_int32_t 		attributeModDate;	/* date and time of last attribute modification */
