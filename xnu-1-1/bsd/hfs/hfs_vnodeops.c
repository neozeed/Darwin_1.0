...
extern cluster_close(struct vnode *vp, int  bsize, long secsize);

extern void hfs_set_metaname(char *name, struct hfsfilemeta *fm);

extern groupmember(gid_t gid, struct ucred *cred);

static int hfs_makenode( int mode, dev_t rawdev, struct vnode *dvp, struct vnode **vpp, struct componentname *cnp);
...
//    };
	...
	/* Create the vnode */
    retval = hfs_makenode(mode, 0, ap->a_dvp, ap->a_vpp, ap->a_cnp);
...
} */ *ap;
{
	...
	/* lock catalog b-tree */
	error = hfs_metafilelocking(VTOHFS(ap->a_dvp), kHFSCatalogFileID, LK_EXCLUSIVE, p);
	if (error != E_NONE) {
		VOP_ABORTOP(ap->a_dvp, ap->a_cnp);
		VPUT(ap->a_dvp);
		return (error);
	...
	/* Create the vnode */
	error = hfs_makenode(MAKEIMODE(vap->va_type, vap->va_mode), rawdev, ap->a_dvp, vpp, ap->a_cnp);
...
} */ *ap;
{
    ...
    if (vp->v_type == VDIR) {
        ...
        vap->va_bytes = 0;
	vap->va_nlink = 2 + hp->h_meta->h_valence;
		/* 
   ...
   if ((alist->volattr != 0) && (((alist->volattr & ATTR_VOL_INFO) == 0) ||
    ...
    if (((alist->volattr == 0) && ((alist->commonattr & HFS_ATTR_CMN_LOOKUPMASK) != 0)) ||
        ...
        if (alist->volattr != 0) {
            ...
            error = hfsLookup(VTOVCB(vp), 2, NULL,  -1, &catalogInfo);
            } else {
				catalogInfo.hint = kNoHint;
                error = hfsLookup(VTOVCB(vp), H_DIRID(hp), H_NAME(hp),  -1, &catalogInfo);
...
#if HFS_HARDLINKS
	/*
	 * Force an update of the data node instead of the link node
	 * by passing the fileID instead of parID and name.
	 */
...
#endif
	catalogInfo.hint = kNoHint;
	error = hfsLookup(VTOVCB(vp), pid, filename, -1, &catalogInfo);
    if (error != E_NONE) {
...
//  if (alist->commonattr & (ATTR_CMN_CRTIME | ATTR_CMN_MODTIME | ATTR_CMN_CHGTIME | ATTR_CMN_ACCTIME | ATTR_CMN_BKUPTIME)) {
	...
	if (alist->volattr == 0) {
		error = MacToVFSError( UpdateCatalogNode(HTOVCB(hp), pid, filename, H_HINT(hp), &catalogInfo.nodeData));
	}
   ...
   if (alist->volattr & ATTR_VOL_NAME) {
	...
	if (vcb->vcbVN[0] == 0) {
	  ...
	  if (error) {
		...
		hfs_set_metaname(vcb->vcbVN, hp->h_meta);
...
struct proc *p;
{
	...
	} else if ((cred->cr_uid != hp->h_meta->h_uid) &&
		       (retval = suser(cred, &p->p_acflag))) {
		return retval;
	};
...
} */ *ap;
{
	...
	struct hfsnode *fromSkipIfFirst, *toSkipIfFirst;
	...
	fromSkipIfFirst = toSkipIfFirst = NULL;
	...
	if (from_hp->h_meta->h_usecount > 1) {
		 ...
		 */
		if (from_hp->h_meta->h_siblinghead.cqh_first == from_hp) {
    		fromSkipIfFirst = from_hp;
			nhp = from_hp->h_sibling.cqe_next;
    	}
    	else {
        	VOP_UNLOCK(from_vp, 0, ap->a_p);
			nhp = from_hp->h_meta->h_siblinghead.cqh_first;
        };

		for (; nhp != (void *)&from_hp->h_meta->h_siblinghead; nhp = nhp->h_sibling.cqe_next) {
			
        	if (vget(HTOV(nhp), LK_EXCLUSIVE | LK_RETRY, ap->a_p))
				continue;		/* skip it, this shouldnt happen */

			/* Ignore any errors, we are doing a 'best effort' on flushing */
			(void) vinvalbuf(HTOV(nhp), V_SAVE, ap->a_cred, ap->a_p, 0, 0);
		};
	};
    ...
    if (to_hp->h_meta->h_usecount > 1) {
		...
		if (to_hp->h_meta->h_siblinghead.cqh_first == to_hp) {
    		toSkipIfFirst = to_hp;
            nhp = to_hp->h_sibling.cqe_next;
    	}
    	else {
            VOP_UNLOCK(to_vp, 0, ap->a_p);
            nhp = to_hp->h_meta->h_siblinghead.cqh_first;
        };


        for (; nhp != (void *)&to_hp->h_meta->h_siblinghead; nhp = nhp->h_sibling.cqe_next) {
...
Err_Exit:;
    ...
    if (to_hp->h_meta->h_usecount > 1) {
        for (nhp = to_hp->h_meta->h_siblinghead.cqh_last;
             	nhp != (void *)&to_hp->h_meta->h_siblinghead; nhp = nhp->h_sibling.cqe_prev) {
            if (toSkipIfFirst && (nhp == toSkipIfFirst))
                continue;
            else if (HTOV(nhp)==to_vp)
                VRELE(HTOV(nhp));		/* decrement,  return it locked */
            /*
             * A gotcha here, is that when locking, it might of failed
             * because the vnode was going away. Since vput() does not test for this,
             * we have to do this ourselves....but using the VXLOCK is bad, because
             * it should be private to vfs routines...but until another way is thought of...
             */
            else if (!(HTOV(nhp)->v_flag & VXLOCK)) {
                cache_purge(HTOV(nhp));
                VPUT(HTOV(nhp));
                };
            };
        };
	...
	if (from_hp->h_meta->h_usecount > 1) {
		for (nhp = from_hp->h_meta->h_siblinghead.cqh_last; 
					nhp != (void *)&from_hp->h_meta->h_siblinghead; nhp = nhp->h_sibling.cqe_prev) {
	        if (fromSkipIfFirst && (nhp == fromSkipIfFirst))
	        	continue;
	        else if (HTOV(nhp)==from_vp)
				VRELE(HTOV(nhp));		/* decrement,  return it locked */
            else if (!(HTOV(nhp)->v_flag & VXLOCK)){
                cache_purge(HTOV(nhp));
	        	VPUT(HTOV(nhp));
    ...
    /* Purge the vnodes from the namei...the names do not match vnodes now */
    cache_purge(from_vp);
    cache_purge(to_vp);
...
hfs_chid(struct hfsnode *hp, u_int32_t fid, u_int32_t pid, char* name)
{
	...
	hfs_set_metaname(name, hp->h_meta);
...
} */ *ap;
{
	...
	struct proc *p = current_proc();
	struct timeval tv;
	int retval, use_count, hasSiblings;
	int filebusy = 0;
    ...
    if (vp->v_usecount > 1) {
    	/* Mac OS clients can't delete busy files */
		if ((ap->a_cnp->cn_flags & NODELETEBUSY) || (hfsmp->hfs_private_metadata_dir == 0)) {
			retval = EBUSY;
			goto out;
        } else
        	filebusy = 1;
	...
	/* XXX SER MUST BE CHANGED BY MACOS X because of vnode_uncache */
	/* Uncache everything and make sure no other usecount */
	/*
	 * a. loop through the siblings looking for matches
	 ...
	 */
	if (hp->h_meta->h_siblinghead.cqh_first && (hp->h_meta->h_siblinghead.cqh_first != hp->h_meta->h_siblinghead.cqh_last)) {
	...
	if (hasSiblings) {
		...
		/* Lock the sibling list, this ensures no entry leaves or is added unexpectanly */
		
		/* Loop through all siblings, skipping ourselves */
		for (nhp = hp->h_meta->h_siblinghead.cqh_first; nhp != (void *)&hp->h_meta->h_siblinghead; nhp = nhp->h_sibling.cqe_next) {
			if (nhp == hp)		/* skip ourselves */
				continue;
			tvp = HTOV(nhp);
		
			/* Check to see if other forks are in use */
			simple_lock(&tvp->v_interlock);
			use_count = tvp->v_usecount;	
			...
			/* Get a lock, for the rest */
			/* The only error that vget returns is when the vnode is going away, so ignore it */
        	if (vget(tvp, LK_EXCLUSIVE | LK_RETRY, p))
        		continue;
		
			/*
			 * XXX SER vnode_uncache() unlocks the vnode and the wait
			 * to regain the lock. VERY BAD. This behavior should be changed in
			 * the future.
			...
			/* 
			 * Should be relocked at this point..now flush out the memory
			 * XXX SER An intelligient person would ask, why flush out changes
			 * that are going to be deleted? See the next comment.
			 */
			if ((nhp->h_nodeflags & IN_MODIFIED) || (VTOFCB(tvp)->fcbFlags & fcbModifiedMask))
				{
				DBG_ASSERT((VTOH(tvp)->h_nodeflags & IN_MODIFIED) != 0);
				VOP_UPDATE(tvp, &tv, &tv, 0);
			};
	...
	if (filebusy) {
		...
		hfs_set_metaname(nodeName, hp->h_meta);
	...
	/* XXX SER Anything else so that sibling, if any, goes away */
...
} */ *ap;
{
	...
	if (target_vp) {
        ...
        if (target_vp->v_type == VREG) {
            ...
            if (ISMAPPEDFILE(target_vp)) {
                ubc_unlink(target_vp);
            }
        }
	...
	if (retval == 0) {	
	    ...
	    retval = hfsLookup(HTOVCB(source_hp), H_FILEID(targetPar_hp), target_cnp->cn_nameptr, target_cnp->cn_namelen, &catInfo);
	}
	...
	hfs_set_metaname(catInfo.spec.name, source_hp->h_meta);
    ...
    } */ *ap;
{
	...
	/* Create the vnode */
	retval = hfs_makenode(IFLNK | ap->a_vap->va_mode, 0, ap->a_dvp, vpp, ap->a_cnp);
    DBG_VOP_UPDATE_VP(1, *ap->a_vpp);
 ...
 */
	...
	if (state->cbs_hfsPlus) {
		...
		/*XXX need to add name mangling for long names */
		result = ConvertUnicodeToUTF8(cnp->ustr.length * sizeof(UniChar),
			    cnp->ustr.unicode, NAME_MAX + 1, &utf8chars, catent.d_name);
	} else { /* hfs */
		switch(crp->recordType) {
		...
		result = hfs_to_utf8(state->cbs_vcb, cnp->pstr, NAME_MAX + 1,
				     &utf8chars, catent.d_name);
	}
...
} */ *ap;
{
    ...
    *(ap->a_eofflag) = 0;
    ...
    /* not using options (don't follow symlinks = 0), check vnode, and buffer space */
    if ((ap->a_options != 0) || (vp == NULL) || 
    ...
    */
    catalogInfo.hint = kNoHint; /* note, we may want to save the latest in state */
    while ((uio->uio_resid >= 0) && (maxcount !=0 )) {
        ...
        */
        OSErr result = GetCatalogOffspring(vcb, dirID, index, &catalogInfo.spec,
	    &catalogInfo.nodeData, NULL, NULL);
        if (result != noErr) {
            if (result == cmNotFound) {
                *(ap->a_eofflag) = TRUE;
                retval = E_NONE;
	...
	if ((dirID == kRootDirID)  &&
	    catalogInfo.nodeData.cnd_nodeID == VCBTOHFS(vcb)->hfs_private_metadata_dir  &&
	    catalogInfo.nodeData.cnd_type == kCatalogFolderNode) {
  	...
  	/* vp okay to use instead of root vp */
        PackCatalogInfoAttributeBlock(alist, vp, &catalogInfo, &attrptr, &varptr);
...
} */ *ap;
{
	...
	if (hp->h_meta->h_metaflags & IN_DELETED) {			
        ...
        error = hfs_metafilelocking(VTOHFS(vp), kHFSCatalogFileID, LK_EXCLUSIVE, p);
        if (error) goto out;
        error = hfs_metafilelocking(VTOHFS(vp), kHFSExtentsFileID, LK_EXCLUSIVE, p);
        if (error) {
            (void) hfs_metafilelocking(VTOHFS(vp), kHFSCatalogFileID, LK_RELEASE, p);
            goto out;
        }
...
} */ *ap;
{
	...
	/*
		NOTE: XXX vnodes need careful handling because fork vnodes that failed to be
			  ...
			  case v_type == VNON and h_meta == NULL... (when????)
	 */
	 ...
	 */
	cache_purge(vp);
	if ((vp->v_type != VNON) && devvp) {
		VRELE(devvp);
	};

	/* Free our data structs */
	if (vp->v_type != VNON) {
		FREE(tdata, M_HFSNODE);
		vp->v_data = NULL;
	};
...
} */ *ap;
{
	...
	*ap->a_retval = 1;
            ...
            break;
        case _PC_NAME_MAX:
            *ap->a_retval = (kHFSPlusMaxFileNameChars * 3);	/* max # of characters x max utf8 representation */
            break;
        case _PC_PATH_MAX:
	...
	} */ *ap;
{
	...
	if (retval) {
		DBG_ERR(("Could not lock catalog tree"));
        DBG_VOP_LOCKS_TEST(retval);
        DBG_ASSERT(*((int*)&ap->a_vp->v_interlock) == 0);
        return (retval);
    };
	...
	retval = hfsLookup(vcb, pid, filename, -1, &catInfo);
    ...
    if (ap->a_vp->v_type == VDIR) {
    		hp->h_meta->h_valence = catInfo.nodeData.cnd_valence;
		...
		hp->h_meta->h_size = vcb->blockSize *
		    (catInfo.nodeData.cnd_rsrcfork.totalBlocks +
 ...
 */
static int
hfs_makenode(mode, rawdev, dvp, vpp, cnp)
    int mode;
    dev_t rawdev;
    struct vnode *dvp;
    struct vnode **vpp;
    struct componentname *cnp;
{
    ...
    *vpp	= NULL;
    ...
    retval = hfsLookup(vcb, H_FILEID(parhp), cnp->cn_nameptr, cnp->cn_namelen, &catInfo);
    if (retval != E_NONE) {
	...
	if (vcb->vcbSigWord == kHFSPlusSigWord) {
		...
		catInfo.nodeData.cnd_specialDevice = rawdev;
		...
		/*
		 * The 32-bit pflags field has two bytes of significance which are
		 * combined with the mode to yield a 32-bit permissions field as follows:
		 *
		 *		+------------------------------------+
		 * pflags:	|XXXXXXXX|    A    |XXXXXXXX|    B   |
		 *		+------------------------------------+
		 *					    |
		 *					    V
		 *		+------------------------------------+
		 ...
		 *		+------------------------------------+
		 */
		catInfo.nodeData.cnd_permissions =
			(((pflags << 8) & 0xFF000000) |	/* A */
			((pflags << 16) & 0x00FF0000) |	/* B */
			(catmode & 0x0000FFFF));	/* mode */
	}

    /* Create a vnode for the object just created: */
    forkType = (catInfo.nodeData.cnd_type == kCatalogFolderNode) ? kDirectory : kDataFork;
    if ((retval = hfs_vcreate(vcb, &catInfo, forkType, &tvp))) {
        goto bad1;
    }
...
struct vnodeopv_entry_desc hfs_vnodeop_entries[] = {
    ...
    { &vop_lookup_desc, hfs_cache_lookup },			/* lookup */
    { &vop_create_desc, hfs_create },			/* create */
    { &vop_mknod_desc, hfs_mknod },				/* mknod */
...
struct vnodeopv_entry_desc hfs_specop_entries[] = {
	...
	{ &vop_close_desc, hfsspec_close },		/* close */
	{ &vop_access_desc, hfs_access },		/* access */
	{ &vop_getattr_desc, hfs_getattr },		/* getattr */
	{ &vop_setattr_desc, hfs_setattr },		/* setattr */
	{ &vop_read_desc, hfsspec_read },		/* read */
	{ &vop_write_desc, hfsspec_write },		/* write */
...
struct vnodeopv_entry_desc hfs_fifoop_entries[] = {
	...
	{ &vop_close_desc, hfsfifo_close },		/* close */
	{ &vop_access_desc, hfs_access },		/* access */
	{ &vop_getattr_desc, hfs_getattr },		/* getattr */
	{ &vop_setattr_desc, hfs_setattr },		/* setattr */
	{ &vop_read_desc, hfsfifo_read },		/* read */
	{ &vop_write_desc, hfsfifo_write },		/* write */
