...
/*
 	...
 	BuildInfo:	Built by:			Scott Roberts
 				...
 				From:				ConditionalMacros.i
 					...
 					Dated:			5/19/99
 					Last change by:	dle
 					Last comment:	Allow developers to set single Carbon conditionals w/MachO
...
****************************************************************************************************/
#define UNIVERSAL_INTERFACES_VERSION 0x0321
...
/****************************************************************************************************
	...
	in order to disambiguate.
		...
		TARGET_API_MACOS_X      - Code is being compiled to run on Mac OS X
...
****************************************************************************************************/
	...
	*/
	...
	/*
		gcc based compilers used on OpenStep -> MacOS X -> Mac OS X
	*/
	...
	#if defined(forCarbon) && (forCarbon == 0)
		/* BlueBox and Carbon both compile MI headers on MacOS X.
		Since cpp-precomp doesn't understand #question(answer), we can't
		use #system(macos) to tell the difference anymore.  So the
		solution is for BlueBox to -DforCarbon=0 on the compile line,
		which we do NOT override here. */
		/* BUT, we still need CFM=1, otherwise we get errors
		   for TempADBServiceRoutineUPP in DeskBus.i.  Mabye
		   this is fixed in TOT MI, but that creates other 
		   problems for BlueBox which still need to be solved. */
...
****************************************************************************************************/
#if TARGET_OS_WIN32
	/* QuickTime for Windows builds do not want __declspec(dllimport) */
	#undef FUNCTION_DECLSPEC
	#define FUNCTION_DECLSPEC 0
#endif
...
#if TARGET_OS_MAC
#if !defined(TARGET_API_MAC_OS8) && !defined(TARGET_API_MACOS_X) && !defined(TARGET_API_MAC_CARBON)
...
#define TARGET_API_MACOS_X 0
#elif defined(forCarbon) && forCarbon
/* grandfather in use of forCarbon */
#define TARGET_API_MAC_OS8 0
#define TARGET_API_MAC_CARBON 1
#define TARGET_API_MACOS_X 0
...
#ifndef TARGET_API_MACOS_X
#define TARGET_API_MACOS_X 0
#endif  /* !defined(TARGET_API_MACOS_X) */

#endif  /* !defined(TARGET_API_MAC_OS8) && !defined(TARGET_API_MACOS_X) && !defined(TARGET_API_MAC_CARBON) */

#if TARGET_API_MAC_OS8 && TARGET_API_MACOS_X
#error TARGET_API_MAC_OS8 and TARGET_API_MACOS_X are mutually exclusive
#endif  /* TARGET_API_MAC_OS8 && TARGET_API_MACOS_X */

#if !TARGET_API_MAC_OS8 && !TARGET_API_MAC_CARBON && !TARGET_API_MACOS_X
#error At least one of TARGET_API_MAC_* must be true
#endif  /* !TARGET_API_MAC_OS8 && !TARGET_API_MAC_CARBON && !TARGET_API_MACOS_X */
...
/****************************************************************************************************
	...
	OPAQUE_UPP_TYPES				- default: false. True for Carbon builds, UPP types are unique.
	ACCESSOR_CALLS_ARE_FUNCTIONS	- default: false. True for Carbon builds, preserve old #define-style accessors
	CALL_NOT_IN_CARBON	 			- default: true.  False for Carbon builds, hide calls not supported in Carbon
