...
//	}
    ...
    err = GetCatalogNode( vcb, kHFSRootFolderID, nil, kUndefinedStrLen, kNoHint, &spec, &catNodeData, &hint );
	if ( err == noErr ) {
...
OSErr GetVolumeNameFromCatalog( ExtendedVCB *vcb )
{
	...
	if ( true )
	{
        err = GetCatalogNode( vcb, kHFSRootFolderID, NULL, kUndefinedStrLen, kNoHint, &spec, &nodeData, &hint );
	
		if ( err == noErr )
		{
			BlockMoveData( spec.name, vcb->vcbVN, strlen(spec.name));
			vcb->volumeNameEncodingHint = nodeData.cnd_textEncoding;		// save encoding for FSFindTextEncodingForVolume
		}
	}
	else
	{
