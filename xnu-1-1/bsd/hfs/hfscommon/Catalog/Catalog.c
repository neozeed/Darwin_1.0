...
static OSErr IterateCatalogNode(ExtendedVCB *volume, CatalogIterator *catalogIterator,
				UInt16 index, FSSpec *nodeSpec, CatalogNodeData *nodeData,
				...
				FSSpec *nodeSpec, CatalogNodeData *nodeData, UInt32 *newHint)
{
   ...
   // if we did not find it by name, then look for an embedded file ID in a mangled name
    if ( result == cmNotFound && isHFSPlus)
        result = LocateCatalogNodeByMangledName(volume, parentID, name, nameLen, key, record, newHint);
	ReturnIfError(result);
	...
	else // classic HFS
	{
		...
		if (record->recordType == kHFSFolderRecord || record->recordType == kHFSFileRecord) {
			...
			result = hfs_to_utf8(volume, key->hfs.nodeName, NAME_MAX + 1, &actualDstLen, nodeSpec->name);
		} else
			result = cmNotFound;
	}
...
GetDirEntrySize(BTreeIterator *bip, ExtendedVCB * vol)
{
	...
	if (vol->vcbSigWord == kHFSPlusSigWord) {
		...
		result = hfs_to_utf8(vol, cnp->pstr, NAME_MAX + 1, &utf8chars, name);
		/*XXX ignoring error */
	}
...
GetCatalogOffspring(ExtendedVCB *volume, HFSCatalogNodeID folderID, UInt16 index,
		    FSSpec *nodeSpec, CatalogNodeData *nodeData,
		    HFSCatalogNodeID *nodeID, SInt16 *nodeType)
{
	...
	result = IterateCatalogNode(volume, catalogIterator, index, nodeSpec,
...
IterateCatalogNode( ExtendedVCB *volume, CatalogIterator *catalogIterator, UInt16 index,
		    FSSpec *nodeSpec, CatalogNodeData *nodeData, HFSCatalogNodeID *nodeID,
		    SInt16 *nodeType )
{
	...
	else /* hfs name */
	{
		if (record->recordType == kHFSFolderRecord || record->recordType == kHFSFileRecord) {
			...
			result = hfs_to_utf8(volume, offspringName->pstr, NAME_MAX + 1, &actualDstLen, nodeSpec->name);
				  ...
				  UInt32 catalogHint, const CatalogNodeData *nodeData)
{
	...
	switch (record->recordType)
	{
		...
		{
		  #if DEBUG_BUILD
			if (nodeData->cn_type != kCatalogFolderNode)
		...
		case kHFSFileRecord:
		{
			...
			if ((nodeData->nc_file.dataFork.totalBlocks > (0x7FFFFFFF/volume->blockSize)) ||
			    (nodeData->nc_file.resourceFork.totalBlocks > (0x7FFFFFFF/volume->blockSize)))
		...
		case kHFSPlusFolderRecord:
		{
			...
			record->hfsPlusFolder.permissions.ownerID = nodeData->cnd_ownerID;
			record->hfsPlusFolder.permissions.groupID = nodeData->cnd_groupID;
			record->hfsPlusFolder.permissions.permissions = nodeData->cnd_permissions;
			record->hfsPlusFolder.permissions.specialDevice = nodeData->cnd_specialDevice;
		...
		case kHFSPlusFileRecord:
		{
			...
			record->hfsPlusFile.permissions.ownerID = nodeData->cnd_ownerID;
			record->hfsPlusFile.permissions.groupID = nodeData->cnd_groupID;
			record->hfsPlusFile.permissions.permissions = nodeData->cnd_permissions;
			record->hfsPlusFile.permissions.specialDevice = nodeData->cnd_specialDevice;
...
#if HFS_HARDLINKS
			record->hfsPlusFile.linkCount = nodeData->cnd_linkCount;
#if DEBUG_BUILD
			if (record->hfsPlusFile.userInfo.fdType == kHardLinkFileType &&
			    record->hfsPlusFile.userInfo.fdCreator == kHardLinkCreator) {
			    if (record->hfsPlusFile.dataFork.logicalSize != 0)
			    	DebugStr("UpdateCatalogNode: link has data fork!");
			}
			if (record->hfsPlusFile.linkCount > 0) {
			    if (name != NULL)
			    	DebugStr("UpdateCatalogNode: node has name!");
			    if (nodeData->cnd_nodeID != record->hfsPlusFile.fileID)
			    	DebugStr("UpdateCatalogNode: fileID mismatch!");
			}
#endif
#endif
