    ...
    for (retval = 0, bp = NULL, firstpass = TRUE; uio->uio_resid > 0; bp = NULL) {
        ...
        if (bp->b_validend > 0) {
            ...
            if (bp->b_validend < bp->b_bcount) {
            	...
            	/* Should never happen so always assert */
    ...
    */
    ...
    for (retval = 0; uio->uio_resid > 0;) {
        ...
        if (writelimit > (off_t)fcb->fcbPLen) {
            ...
            DBG_RW(("\textending file by 0x%lX bytes; 0x%lX blocks free", (unsigned long)bytesToAdd, (unsigned long)vcb->freeBlocks));
    ...
    if (ap->a_bn < hp->h_uniformblocksizestart) {
        ...
        for (extent = 0; extent < LOGBLOCKMAPENTRIES; ++extent) {
            if ((hp->h_logicalblocktable[extent].logicalBlockCount > 0) &&
                ...
                extentSize = HTOFCB(hp)->fcbExtents[extent].blockCount * HTOVCB(hp)->blockSize;
...
//	DBG_VOP_PRINT_FUNCNAME();DBG_VOP_CONT(("\n"));
	...
	if (bp->b_bcount != GetLogicalBlockSize(vp)) {
    ...
    } */ *ap;
{
	...
	/* needs to be present till Unified buffer cache implementaiton */
	ubc_truncate(vp, (vm_offset_t)length);
	...
	if (retval) DBG_ERR(("Could not update truncate"));
