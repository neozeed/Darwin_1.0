...
vm_offset_t kdp_vtophys(space_t space, vm_offset_t vaddr);
	...
	unsigned len)
{
	...
	if (kdp_trans_off) {
		...
		while (resid != 0) {   
			if (kdp_pmap) {
				if ((cur_phys_src = 
					kdp_vtophys(kdp_pmap->space,trunc_page(cur_virt_src))) ==0)
					goto exit;
				cur_phys_src += (cur_virt_src & PAGE_MASK);
			} else {
				if ((cur_phys_src = kdp_vtophys(PPC_SID_KERNEL,cur_virt_src)) == 0)
					goto exit;
			}
...
{       
	...
	while (resid != 0) {
		if ((cur_phys_dst = kdp_vtophys(PPC_SID_KERNEL,cur_virt_dst)) == 0) 
			goto exit;
		if ((cur_phys_src = kdp_vtophys(PPC_SID_KERNEL,cur_virt_src)) == 0) 
			goto exit;
