...
/*
 * MK Scheduling Policy per-thread scheduling attributes (externally visible)
 * ** This structure is used to access/control attributes via the
 * ** new 'thread_set_sched()', etc., routines.
 */

typedef struct mk_sp_attributes {
	int			policy_id;			/* first, by convention */
	int			priority;			/* base priority */
	int			max_priority;		/* maximum priority */
	int			sched_data;			/* for use by policy */
	int			unconsumed_quantum;	/* leftover quantum (RR/FIFO)*/
} mk_sp_attribute_struct_t, *mk_sp_attributes_t;
