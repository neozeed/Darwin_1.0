	...
	vm_inherit_t	inheritance)
{
	...
	if (!IP_VALID(port)) {
		...
		if ((object = vm_object_enter(port, size, FALSE, FALSE))
			== VM_OBJECT_NULL)
			return(KERN_INVALID_OBJECT);
	...
	vm_sync_t	sync_flags)
{
	...
	while (amount_left > 0) {
		...
		if (sync_flags & VM_SYNC_KILLPAGES) {
		        if (object->ref_count == 1 && !object->shadow && !entry->needs_copy)
			        memory_object_kill_pages(object, offset, flush_size);
...
redo_lookup:
		...
		if(object->internal) {
			...
			/* under us.  */
			     ...
			     (object->size > ((vm_size_t)map_entry->vme_end -
			     map_entry->vme_start))) {
	...
	mach_msg_type_number_t	count)
{
	...
	upl_lock(upl);
	if(count) {
		kr = upl_commit(upl, (upl_page_info_t *)page_list);
	} else {
		kr = upl_commit(upl, (upl_page_info_t *) NULL);
	}
	upl_unlock(upl);
	return kr;
}
	...
	mach_msg_type_number_t	count)
{
	...
	upl_lock(upl);
	if(count) {
		kr = upl_commit_range(upl, offset, size, free_on_empty, 
					(upl_page_info_t *)page_list);
	} else {
		kr = upl_commit_range(upl, offset, size, free_on_empty,
	...
	int		abort_flags)
{
	kern_return_t kr;
	upl_lock(upl);
	kr = upl_abort_range(upl, offset, size, abort_flags);
	upl_unlock(upl);
	return kr;
}
