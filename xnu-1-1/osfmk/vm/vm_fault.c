					 ...
					 * page is provided */
{
	...
	kern_return_t   vnode_pager_data_request(
		ipc_port_t, ipc_port_t, vm_offset_t, vm_size_t, vm_prot_t);
#endif
...
#define RELEASE_PAGE(m)					\
	...
	interruptible = THREAD_UNINT;  /* vm_fault_interruptible */
#endif	/* MACH_KDB */
#endif	/* STATIC_CONFIG */
...
no_clustering:
		...
		if (next_object == VM_OBJECT_NULL) {
			...
			if ((vm_page_free_target - 
				...
				assert(object->ref_count > 0);
				vm_fault_cleanup(object, first_m);
				vm_page_wait();  /* kick off pageout daemon */
				if ((m = vm_page_grab()) != VM_PAGE_NULL) {
					/* need to kick off other parties */
					/* waiting on free pages */
					VM_PAGE_FREE(m);
				}
				return VM_FAULT_RETRY;
			}
...
FastPmapEnter:
	...
	kr = vm_fault_page(object, offset, fault_type,
			   (change_wiring && !wired),
			   ((!change_wiring) ? THREAD_ABORTSAFE : THREAD_UNINT),
