		      ...
		      mach_msg_type_number_t *count)
{
	...
	switch (flavor) {
		...
		case THREAD_STATE_FLAVOR_LIST:
			...
			if (*count < 3)  {
				panic("act_machine_get_state: THREAD_STATE_FLAVOR_LIST returns KERN_INVALID_ARGUMENT\n");
		...
		case PPC_THREAD_STATE:
			...
			if (*count < PPC_THREAD_STATE_COUNT) {			/* Is the count ok? */
				panic("act_machine_get_state: PPC_THREAD_STATE returns KERN_INVALID_ARGUMENT\n");
				return KERN_INVALID_ARGUMENT;
			}
		...
		case PPC_EXCEPTION_STATE:
			...
			if (*count < PPC_EXCEPTION_STATE_COUNT) {
				panic("act_machine_get_state: PPC_EXCEPTION_STATE returns KERN_INVALID_ARGUMENT\n");
				return KERN_INVALID_ARGUMENT;
			}
		...
		case PPC_FLOAT_STATE: 
			...
			if (*count < PPC_FLOAT_STATE_COUNT)  {
				panic("act_machine_get_state: PPC_FLOAT_STATE returns KERN_INVALID_ARGUMENT\n");
				return KERN_INVALID_ARGUMENT;
			}
...
#if 0
			for(i=0; i < 32; i++) {						/* (TEST/DEBUG) */
				if((*((unsigned int *)&fs->fpregs[i]) >> 16) == 0xFFF8) __asm__ volatile ("tweq r1,r1");	/* (TEST/DEBUG) */
			}
#endif
		...
		case PPC_VECTOR_STATE: 
			...
			if (*count < PPC_VECTOR_STATE_COUNT)  {
				panic("act_machine_get_state: PPC_VECTOR_STATE returns KERN_INVALID_ARGUMENT\n");
				return KERN_INVALID_ARGUMENT;
			}
				...
				for(i=0; i < 32; i++) {					/* Copy the saved registers and invalidate the others */
					for(j=0; j < 4; j++) {
						if(vrvalidwrk & 0x80000000) (vs->save_vr)[i][j] = 
								((unsigned long **)(&sv->save_vr0))[i][j];	/* We have this register saved */
		...
		default:
			panic("act_machine_get_state: default returns KERN_INVALID_ARGUMENT\n");
			return KERN_INVALID_ARGUMENT;
	}
}
...
struct ppc_saved_state * get_user_regs(thread_act_t act) {
	...
	sv->save_srr1 = MSR_EXPORT_MASK_SET;
	sv->save_mq	= 0;
	/* XXX: TO BE REMOVED WA for AltiVec compiler bug */
	sv->save_vrsave = 0xffffffff;						/* VRSAVE register (Altivec only) */
