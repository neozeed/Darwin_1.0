...
#if DEBUG
	printf("sectTEXT: %x, size: %x\n", sectTEXTB, sectSizeTEXT);
	printf("sectDATA: %x, size: %x\n", sectDATAB, sectSizeDATA);
	printf("sectOBJC: %x, size: %x\n", sectOBJCB, sectSizeOBJC);
	printf("sectLINK: %x, size: %x\n", sectLINKB, sectSizeLINK);
	printf("end: %x\n", end);
#endif
#endif /* __MACHO__ */
 ...
 */
	...
	for (i = 1; i < kMaxDRAMBanks; i++) {
		...
		if (args->PhysicalDRAM[i].base ==				/* Does the end of the last hit the start of the next? */
		  ...
		  args->PhysicalDRAM[addr].size) {
			printf("region 0x%08x size 0x%08x joining region 0x%08x size 0x%08x\n",
	...
	pmap_map(trunc_page(sectLINKB), trunc_page(sectLINKB), 
		round_page(sectLINKB+sectSizeLINK), VM_PROT_READ|VM_PROT_WRITE);
	pmap_map(trunc_page(end), trunc_page(end), 
		round_page(static_memory_end), VM_PROT_READ|VM_PROT_WRITE);
#endif /* __MACHO__ */
	...
	sync();isync();
#if DEBUG
	for(i=0; i<4; i++) printf("DBAT%1d: %08X %08X\n", 
		i, shadow_BAT.DBATs[i].upper, shadow_BAT.DBATs[i].lower);
	for(i=0; i<4; i++) printf("IBAT%1d: %08X %08X\n", 
