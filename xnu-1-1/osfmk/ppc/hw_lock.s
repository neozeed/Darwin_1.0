...
#define	CHECK_MUTEX_TYPE()							\
			...
			beq+	1f							__ASMNL__	\
			lis		r3,HIGH_ADDR(not_a_mutex)	__ASMNL__	\
			ori		r3,r3,LOW_ADDR(not_a_mutex)	__ASMNL__	\
...
#define CHECK_SIMPLE_LOCK_TYPE()					\
			...
			beq+	1f							__ASMNL__ 	\
			lis		r3,HIGH_ADDR(not_a_slock)	__ASMNL__ 	\
			ori		r3,r3,LOW_ADDR(not_a_slock) __ASMNL__ 	\
			bl		EXT(panic)					__ASMNL__ 	\
			lwz		r3,FM_ARG0(r1)				__ASMNL__ 	\
1:
...
#define CHECK_NO_SIMPLELOCKS()							\
			...
			beq+	1f 							__ASMNL__	\
			lis		r3,HIGH_ADDR(simple_locks_held) __ASMNL__	\
			ori		r3,r3,LOW_ADDR(simple_locks_held) __ASMNL__	\
			bl		EXT(panic)	 				__ASMNL__	\
			lwz		r3,FM_ARG0(r1)	 			__ASMNL__ 	\
1:
...
#define CHECK_THREAD(thread_offset)						\
			...
			beq+	1f 						__ASMNL__ 	\
			lis		r3,HIGH_ADDR(wrong_thread) __ASMNL__ \
			ori		r3,r3,LOW_ADDR(wrong_thread) __ASMNL__ 	\
			bl		EXT(panic)	 			__ASMNL__ 	\
			lwz		r3,FM_ARG0(r1)	 		__ASMNL__ 	\
1:
...
LEXT(_mutex_try)
			...
			bl		lockLock				/* Go get a lock on the mutex's interlock lock */
			mr.		r4,r3					/* Did we get it? */
			lwz		r3,FM_ARG0(r1)			/* Restore the lock address */
