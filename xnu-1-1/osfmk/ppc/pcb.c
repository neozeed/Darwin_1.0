	...
	void		(*continuation)(void))
{
    ...
    sv->save_lr = (unsigned int) continuation;			/* Set up the execution address */
    sv->save_srr0 = (unsigned int) continuation;		/* Here too */
	sv->save_srr1 = MSR_SUPERVISOR_INT_ON;				/* Set the normal running MSR */
	     ...
	     void (*continuation)(void))
{
  ...
  if ((thr_act = thread->top_act) != 0) {
    ...
    sv->save_srr0 = (unsigned int) continuation;
    /* sv->save_r3 = ARG ? */
    sv->save_r1 = (vm_offset_t)((int)kss - KF_SIZE);
	sv->save_srr1 = MSR_SUPERVISOR_INT_ON;
    *((int *)sv->save_r1) = 0;
    thr_act->mact.ksp = 0;			      
  }
