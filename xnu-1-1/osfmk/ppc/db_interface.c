	...
	vm_offset_t va)
{
	...
	pa = (vm_offset_t)LRA(space,(void *)va);

	if (pa != 0)
		return(pa);

	mp = hw_lock_phys_vir(space, va);
	...
	int		flag)
{
	...
	raddr = (unsigned int)db_vtophys(task->map->pmap->space, trunc_page(addr));	/* Get the real address */
	...
	task_t		task)
{
	...
	while (size > 0) {
		...
		phys_src = (unsigned int)db_vtophys(space, trunc_page(addr));  
		if (phys_src == 0) {
		...
		phys_dst = (unsigned int)db_vtophys(space, trunc_page(data)); 
		if (phys_dst == 0) {
	...
	task_t		task)
{
	...
	while (size > 0) {
		...
		phys_src = (unsigned int)db_vtophys(space, trunc_page(data)); 
		if (phys_src == 0) {
			db_printf("\nno memory is assigned to src address %08x\n",
				  data);
			db_error(0);
		...
		phys_dst = (unsigned int)db_vtophys(space, trunc_page(addr));  
		if (phys_dst == 0) {
			db_printf("\nno memory is assigned to dst address %08x\n",
				  addr);
			db_error(0);
	...
	vm_offset_t	addr2)
{
	...
	if(!(physa = db_vtophys(task1->map->pmap->space, trunc_page(addr1)))) return FALSE;	/* Get real address of the first */
	if(!(physb = db_vtophys(task2->map->pmap->space, trunc_page(addr2)))) return FALSE;	/* Get real address of the second */
