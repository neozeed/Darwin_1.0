...
.L_sw_ctx_cont:	
			...
			ori		r0,r0,lo16(CutTrace)			/* (TEST/DEBUG) */
			lwz		r2,THREAD_TOP_ACT(r3)			; (TEST/DEBUG) Get the old activation
...
/*
 *			Note: we can't instruction trace the next few instructions
 *			because we blast the MSR
 */

			mtmsr	r8								/* Turn off everything */
			isync
...
; 
			...
			stfd	f0,savefpscrpad(r3)		; Save the FPSCR
...
fsnosave:	lwz		r15,ACT_MACT_PCB(r17)	; Get the current level of the "new" one
			...
			dcbt	0,r11					; Touch line in
			lfd		f31,savefpscrpad(r14)	; Load junk 32 bits+fpscr
			lfd     f0, savefp0(r14)
			lfd     f1,savefp1(r14)
...
fsenable:	lwz		r9,SAVflags(r4)			/* Get the flags of the current savearea */
			...
			oris	r10,r10,hi16(floatUsed)	; Set that we used floating point
...
vrenable:	
			...
			lis		r7,hi16(SAVattach)		/* Get the attached flag */
			lwz		r5,SACvrswap(r5)		/* Get Virtual to Real translation */
			oris	r10,r10,hi16(vectorUsed)	; Set that we used vectors
			mr.		r15,r15					; See if we are doing this for user state
			stw		r8,savesrr1(r4)			; Set the msr of the interrupted guy
			andc	r9,r9,r7				/* Clear the attached bit */
			xor		r3,r4,r5				/* Get the real address of the savearea */
