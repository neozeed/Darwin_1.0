...
kern_return_t ps_write_file(paging_segment_t, vm_offset_t, vm_offset_t, unsigned int);	/* forward */
	...
	while((vs_count != 0) && (vs != NULL)) {
		...
		if(dp_pages_free < cluster_transfer_minimum)
			...
			error = vm_fault_list_request(transfer_object, 
					0, VM_SUPER_CLUSTER, &page_list, NULL, 
	...
	vm_size_t	size)
{
	...
	if(size < (page_list->size)) {
		upl_commit_range(page_list, 0, size, FALSE, NULL);
	} else {
		upl_commit(page_list, NULL);
	}
}
	...
	while (cnt && (error == KERN_SUCCESS)) {
		...
		if (actual_offset == (vm_offset_t) -1) {
			...
			upl_system_list_request((vm_object_t)
					...
					&page_list, NULL, 0, request_flags);
			if (clmap.cl_error) {
				upl_abort(page_list, UPL_ABORT_ERROR);
			} else {
				upl_abort(page_list, UPL_ABORT_UNAVAILABLE);
			}
		...
		upl_system_list_request((vm_object_t)
					vs->vs_control_port->ip_kobject,
					offset, size, size, &page_list, 
					NULL, 0, request_flags);
		if(ps->ps_segtype == PS_PARTITION) {
/*
			error = ps_read_device(ps, actual_offset, page_list,
				       size, &residual);
*/
		} else {
			vm_offset_t ioaddr;

			upl_map(kernel_map, page_list, &ioaddr);
			error = ps_read_file(ps, actual_offset, ioaddr, 
					size, &residual);
		...
		if ((error == KERN_SUCCESS) && (residual == 0)) {
			 ...
			 */
			pvs_object_data_provided(vs, page_list, offset, size);
		} else {
			size_wanted = size;
			if (error == KERN_SUCCESS) {
				if (residual == size) {
					...
					pvs_object_data_provided(vs, page_list,
							offset, lsize);
					cnt -= lsize;
					offset += lsize;
					if (size -= lsize) {
	...
	boolean_t	dp_internal)
{
	...
	vm_offset_t	mapped_data;
	vm_offset_t	mobj_base_addr;
	vm_offset_t	mobj_target_addr;
	int		mobj_size;
	vm_offset_t	vs_map_error_addr = 0;
	int		page_index;
	int		list_size;
	int		cl_size;
	...
	if(!dp_internal) {
		...
		upl_map(kernel_map, upl, &mapped_data);
		...
		for(transfer_size = list_size; transfer_size != 0;) {
			...
			if(actual_offset == (vm_offset_t) -1) {
				vs_map_error_addr = mobj_target_addr;
				error = 1;
				break;
			}
			...
			while(cnt > 0) {
			      ...
			      /* pages */
			      if(UPL_DIRTY_PAGE(page_list, page_index) ||
				((bs_low) &&
				UPL_PRECIOUS_PAGE(page_list, page_index))) {
			         page_index++;
	 	   	   ...
	 	   	   if(subx_size) {
				error = ps_write_file(ps, actual_offset,
							mapped_data, subx_size);
				if(error) {
					vs_map_error_addr = mobj_target_addr;
				}
		...
		upl_un_map(kernel_map, upl);
		if(error) {
			if(vs_map_error_addr > mobj_base_addr) {
				upl_commit_range(upl, 0, 
					vs_map_error_addr - mobj_base_addr,
					FALSE, page_list);
			}
			upl_abort(upl, 0);
		} else {
			upl_commit(upl, page_list);
		}
	} else {
...
*/
		...
		if ((error == KERN_SUCCESS) && (residual == 0)) {
			...
			*vsmap_ptr = write_vsmap;
			if(vs_cluster_write(vs, offset, 
					ioaddr, size, TRUE) != KERN_SUCCESS) {
			 	error = KERN_FAILURE;
...
kern_return_t ps_read_file(paging_segment_t, vm_offset_t, vm_offset_t, unsigned int, unsigned int *);	/* forward */
	...
	unsigned int		*residualp)
{
	...
	dev_offset = ps->ps_offset + offset;
	
	error = pager_vnode_pagein(ps->ps_vnode, 
		(vm_offset_t)ioaddr, dev_offset, (vm_size_t)size, NULL);
	...
	unsigned int		size)
{
	...
	if (ioaddr) {
	 	if(pager_vnode_pageout(ps->ps_vnode, ioaddr, 
			   (vm_offset_t) dev_offset, (vm_size_t)size, NULL)) {
					result = KERN_FAILURE;
		}
		else {
			result = KERN_SUCCESS;
