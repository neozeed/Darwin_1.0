	...
	mach_msg_size_t		slist_size)
{
	...
	mr = ipc_mqueue_receive(mqueue, option, rcv_size,
				timeout, THREAD_ABORTSAFE,
				&kmsg, &seqno);
	...
	if (mr != MACH_MSG_SUCCESS) {
	  ...
	  if (mr == MACH_RCV_TOO_LARGE || mr == MACH_RCV_SCATTER_SMALL) {
	    if (option & MACH_RCV_LARGE) {
	       ...
	       * kmsg pointer instead of the message (which was left on the
	       * queue).
	       */
        ...
        mach_msg_size_t		scatter_list_size)
{
	...
	if ((masked_option == (MACH_SEND_MSG|MACH_RCV_MSG)) && enable_hotpath) {
		 ...
		 */
		if ((receiver->state & TH_RUN) != TH_WAIT) {
			assert(NCPUS > 1);
			HOT(c_mmot_cold_033++);
		fall_off:
		...
		if ((send_size + REQUESTED_TRAILER_SIZE(receiver->ith_option)
			 > receiver->ith_msize) ||
		    (receiver->ith_option & MACH_RCV_NOTIFY) ||
		...
		ipc_mqueue_release_msgcount(dest_mqueue);
		imq_unlock(dest_mqueue);
		ip_unlock(dest_port);
		 ...
		 */
		{
			...
			/*
			 * JMM - Hacked in version of setrun scheduler op
			 * that doesn't try to put thread on a runq.  Safe
			 * for now because we only do this for sp_mk threads.
			 */
			{
				mk_sp_info_t sched_info;

				assert(receiver->sp_info != SP_INFO_NULL);
				sched_info = (mk_sp_info_t)receiver->sp_info;
				sched_info->th_state = MK_SP_RUNNABLE;
			}
			...
			{   
			    ...
			    sfr = policy->sp_ops.sp_thread_done(policy, self);
		...
		case THREAD_RESTART:
			/* 
			 * something bad happened to the port/set.  If the port
			 * is still active, it must have just changed membership.
			 */
			if (!io_active(rcv_object)) {
				ipc_object_release(rcv_object);
		...
		/*
		 * If we allowed for just a receive too large indication
		 * and we got one, copy out the size (stored in our kmsg
		 * pointer) instead of the message.  If we got the error,
		 * but didn't set the flag, we just need to try the copy
		 * out of the message and see where that takes us.
		 */
		if ((option & MACH_RCV_LARGE) && 
		    (self->ith_state == MACH_RCV_TOO_LARGE)) {
			mach_msg_header_t *out_msg;

			out_msg = (rcv_msg != MACH_MSG_NULL)?rcv_msg:msg;
			if (copyout((char *) &self->ith_kmsg,
				    (char *) &msg->msgh_size,
				    sizeof(mach_msg_size_t)))
				return MACH_RCV_INVALID_DATA;
		...
		{
			ip_lock(dest_port);
			assert(dest_port->ip_messages.imq_msgcount > 0);
			dest_port->ip_messages.imq_msgcount--;
			ip_unlock(dest_port);
		}
