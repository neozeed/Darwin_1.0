...
static lock_t *			gIORegistryLock;
static IORecursiveLock *	gPropertiesLock;
static SInt32			gIORegistryGenerationCount;

#define UNLOCK	lock_done( gIORegistryLock )
#define RLOCK	lock_read( gIORegistryLock )
#define WLOCK	lock_write( gIORegistryLock );	\
...
IORegistryEntry * IORegistryEntry::initialize( void )
{
   bool			ok;
	...
	gRegistryRoot = new IORegistryEntry;
        gIORegistryLock = lock_alloc(true, NULL, NULL);
	gPropertiesLock = IORecursiveLockAlloc();
	gIORegistryPlanes = OSDictionary::withCapacity( 1 );

	assert( gRegistryRoot && gIORegistryLock && gPropertiesLock
		&& gIORegistryPlanes );
...
#define wrap2(func,type)						\
OSObject *								\
IORegistryEntry::func ## Object( OSObject * anObject, type * aKey)	\
{									\
    OSObject *	obj;							\
									\
    PLOCK;					\
    obj = getPropertyTable()->func ## Object( anObject, aKey );		\
    PUNLOCK;					\
									\
    return( obj );							\
}
                                ...
                                const IORegistryPlane * plane ) const
{
	...
	links = OSArray::withObjects( (OSObject **) &to, 1, 1 );
	result = (links != 0);
	if( result) {
