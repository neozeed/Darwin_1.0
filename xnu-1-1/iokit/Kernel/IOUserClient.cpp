...
OSDefineMetaClass( IOUserClient, IOService )
OSDefineAbstractStructors( IOUserClient, IOService )
                                    ...
                                    io_buf_ptr_t * buf )
{
    ...
    ool = (len >= msg_ool_size_small);

    if( ool ) {

	err = vm_allocate( kernel_map, (vm_offset_t *) &kdata, len, true );
	assert( err == KERN_SUCCESS );
	if( err)
	    return( err );

        bcopy( data, kdata, len );

	err = vm_map_copyin( kernel_map, (vm_offset_t) kdata, len,
			true /* src_destroy */, &copy);

	assert( err == KERN_SUCCESS );
	if( err == KERN_SUCCESS )
            *buf = (char *) copy;
	else
	    vm_deallocate( kernel_map, (vm_offset_t ) kdata, len );

    } else {

        kdata = (void *) kalloc( len );
        assert( kdata );
	if( kdata) {
            *buf = (char *) kdata;
            bcopy( data, kdata, len );
	...
	io_object_t *connection )
{
    ...
    err = service->newUserClient( owningTask, token,
		connect_type, &client );
...
kern_return_t is_io_connect_set_properties(
	...
	mach_msg_type_number_t propertiesCnt )
{
    return( is_io_registry_entry_set_properties( connection, properties, propertiesCnt ));
}
        ...
        IOByteCount *	outputCount )
{
    ...
    CHECK( IOUserClient, connect, client);
    if( (method = client->getExternalMethodForIndex(index))) {
      do {
        err = kIOReturnBadArgument;
        ...
        IOByteCount *	outputCount )
{
    ...
    CHECK( IOUserClient, connect, client);
    if( (method = client->getExternalAsyncMethodForIndex(index))) {
      do {
        err = kIOReturnBadArgument;
        if( kIOUCScalarIScalarO != (method->flags & kIOUCTypeMask))
            continue;
	...
	io_string_t	matching )
{
	...
	case kIOOFPathMatching:
	    ...
	    dict = IODiskMatching( (const char *) input,
