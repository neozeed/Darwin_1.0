...
//    RootHubID				rootHubID;	// Status of root hub, if 0, not initialized otherwise has virtual ID number
    ...
    volatile OHCIEndpointDescriptorPtr	pIsochHead;	// ptr to Isochtonous list
    ...
    Ptr					pDataAllocation;	// ptr to block used for TD, ED, ITD
    Ptr					pPCIAssignedAddresses;	// ptr to PCI assigned addresses
    ...
    UInt16		rootHubFuncAddress;	// Function Address for the root hub
    int			OptiOn;
    UInt32	isochBandwidthAvail;	// amount of available bandwidth for Isochronous transfers
};
...
{
    volatile UInt32			flags;			// 0x00 Data controlling transfer.
    ...
    IOUSBCompletion	    	  	completion;	// only used if last TD, other wise its  nil
    OHCIEndpointDescriptorPtr		pEndpoint;	// pointer to TD's Endpoint
    UInt32				pType;		// 0x20 Note this must appear at the same offset (32) in GTD & ITD structs
