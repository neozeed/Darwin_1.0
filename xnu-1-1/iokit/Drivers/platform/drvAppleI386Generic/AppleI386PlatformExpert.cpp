...
/* 
 * Backdoor routines for drivers which are not yet IOKit drivers.
 * (aka if_fxp.c ) 
 * FIXME/REMOVEME when if_fxp.c becomes part of IOKit.
 */

extern "C" {
void IOKitRegisterInterruptHookStub(void * /*unused */, int vector, int isclock);

void IOKitRegisterInterruptHookStub(void * /*unused */, int /* vector */, int isclock) 
{
	panic("RegisterInterruptHook routine called without being setup!\n");
}

void (*IOKitRegisterInterruptHook)(void *, int pic, int isclock) =
	IOKitRegisterInterruptHookStub;
}
