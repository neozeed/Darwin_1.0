...
bool AppleVIA::start(IOService *provider)
{
  ...
  // Calculate the bus and cpu speeds.
  getPlatform()->getDefaultBusSpeeds(&numSpeeds, &speedList);
  ...
  interruptControllerName = OSSymbol::withCStringNoCopy(kInterruptControllerName);
...
AppleVIADevice *AppleVIA::createNub(void)
{
  ...
  do {
    ...
    cName = OSSymbol::withCStringNoCopy(kInterruptControllerName);
    if (cName == 0) continue;
    ...
    if (viaDeviceType == kVIADeviceTypeCuda)
      name = OSSymbol::withCStringNoCopy("cuda");
    else if (viaDeviceType == kVIADeviceTypePMU)
      name = OSSymbol::withCStringNoCopy("pmu");
    else name = 0;
    if (name == 0) continue;
