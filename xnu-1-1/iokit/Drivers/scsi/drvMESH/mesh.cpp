...
void meshSCSIController::executeCommand( IOSCSICommand *scsiCommand )
{
	SCSICDBInfo			scsiCDB;
	...
	bzero( fCmdData, sizeof( fCmdData ) );		/// ??? unnecessary?
    ...
    *fMsgOutPtr++ = msgByte;
	...
	fNegotiatingSDTR = false;
	if ( scsiCDB.cdbFlags & kCDBFlagsNegotiateSDTR )
    {	fNegotiatingSDTR = true;
...
void meshSCSIController::cancelCommand( IOSCSICommand *scsiCommand )
{
...
void meshSCSIController::resetCommand( IOSCSICommand *scsiCommand )
{
...
void meshSCSIController::processInterrupt()
{
    ...
    *(UInt32*)&fCCL[ kcclStageLabel ] = 0;
    ...
    case kcclStageXfer:
	//	if ( cmdBuf->resultFlags & scsiAutosenseStarted )
    //	     doInterruptStageXferAutosense();
	//	else
    ...
    case kcclStageBucket:
        ...
        *(UInt32*)&fCCL[ kcclBatchSize ] = 0;				/* Clear our count       */
		...
		if ( (count != 0)
		  ...
		  || !(phase > kBusPhaseDATI) )
		{
			fCmdData->results.returnCode = kIOReturnOverrun;
		}
...
void meshSCSIController::completeCommand()
{
...

	{
		...
		case kSCSIStatusGood:
			if ( fCmdData->results.bytesTransferred != fCmdData->xferCount
			 &&  fCmdData->results.returnCode       == kIOReturnSuccess )
			{
				fCmdData->results.returnCode = kIOReturnUnderrun;
			}
			break;
...
void meshSCSIController::ProcessMSGI()
{
    ...
    case kSCSIMsgExtended:
        ...
        case kSCSIMsgSyncXferReq:				/* handle sync negotiation:     */
			...
			{
				fMsgOutPtr		= &fCCL[ kcclMSGOdata ];
				*fMsgOutPtr++   = kSCSIMsgExtended;     /* 0x01 Ext Msg     */
				*fMsgOutPtr++   = 0x03;                 /* 0x03 Message Len */
				*fMsgOutPtr++   = kSCSIMsgSyncXferReq;  /* 0x01 SDTR code   */
				offset = fMsgInBuffer[ 4 ];
				if ( offset == 0 )              /* Offset == 0 means async: */
				{
					*fMsgOutPtr++ = 0;          /* clear period byte        */
					*fMsgOutPtr++ = 0;          /* offset byte = 0 for async*/
				...
				else                            /* have offset ergo sync:   */
				{
					...
					*fMsgOutPtr++ = period / 4; /* SCSI 4ns granularity     */
					*fMsgOutPtr++ = offset;
...
void meshSCSIController::abortActiveCommand()
{
	...
	if ( fCmd )
	{
		fCmdData->results.returnCode = kIOReturnError;	// ??? use kIOAborted?
		completeCommand();
	}
