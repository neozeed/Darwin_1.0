...
void Sym8xxSCSIController::Sym8xxProcessIODone()
{
    ...
    if ( srb->srbSCSIResult == kIOReturnSuccess )
    {
        if ( srb->srbSCSIStatus != kSCSIStatusGood )
        {
            srb->srbSCSIResult = kIOReturnIOError;
        }
    }
...
void Sym8xxSCSIController::Sym8xxCompleteSRB( SRB *srb )
{
    ...
    scsiResults.returnCode = srb->srbSCSIResult;
...
//    printf( "SCSI(Symbios8xx): SIST = %04x DSTAT = %02x\n\r", sistReg, dstatReg  );
    ...
    else if ( sistReg & UDC )
    {
        if ( srb->srbSCSIResult == kIOReturnSuccess )
        {
            srb->srbSCSIResult = kIOReturnAborted;
        }
    ...
    {
        srb->srbSCSIResult = kIOReturnOffline;
...
void Sym8xxSCSIController::Sym8xxSCSIBusReset( SRB *srb )
{
    if ( srb )
    {
        if ( resetSRB )
        {
            srb->srbSCSIResult = kIOReturnBusy;
            Sym8xxCompleteSRB( srb );
            return;
        }    
        resetSRB = srb;
...
void Sym8xxSCSIController::Sym8xxProcessSCSIBusReset()
{
   ...
   Sym8xxClearFifo();
    ...
    if ( resetSRB )
    {
        resetSRB->srbSCSIResult = kIOReturnSuccess;
