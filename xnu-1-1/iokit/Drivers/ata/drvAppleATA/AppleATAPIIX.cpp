...
OSDefineMetaClassAndStructorsWithInit( AppleATAPIIX, AppleATA,
...
// Function inherited from IOATAController.
//
// Create a workloop and attach various event sources to the newly created 
// workloop.
//
bool AppleATAPIIX::createWorkLoop(IOWorkLoop ** workLoop)
{
    ...
    timerEventSource = IOTimerEventSource::timerEventSource(
	                   this,
	                   (IOTimerEventSource::Action) &AppleATAPIIX::ataTimer);
...
//
bool AppleATAPIIX::programDma(IOATACommand * cmd)
{
...
//
bool AppleATAPIIX::startDma(IOATACommand * cmd)
{
	/*
...
// Stop the DMA engine.
//
bool AppleATAPIIX::stopDma(IOATACommand * cmd, UInt32 * transferCount)
{
	UInt8  bmisx;
...
//
//
void AppleATAPIIX::ataTimer( IOTimerEventSource * /* sender */ )
{
    ...
    if ( xferCmdTimer != 0 )
    {
        if ( --xferCmdTimer == 0 )
        {
            IOLog("AppleATAPIIX::%s() - Timeout occurred\n\r", __FUNCTION__ );
...
void AppleATAPIIX::writeATAReg(UInt32 regIndex, UInt32 regValue)
{
    if (regIndex == 0) {
		...
		outb(ioCtlRange + regIndex - ataRegDeviceControl + 2, 
		     (UInt8) regValue);
    }
}
...
UInt32 AppleATAPIIX::readATAReg( UInt32 regIndex )
{
	...
	return inb(ioCtlRange + regIndex - ataRegDeviceControl + 2);
}
