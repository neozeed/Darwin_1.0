...
#ifndef __ppc__
//
// Intel interrupt hack. This prevents a hang when the driver
// services the interrupt.
//
#define USE_FILTER_INTERRUPT_EVENT_SRC 1
#endif
...
public:
	...
	virtual IOService * Intel82557::probe(IOService * provider,
                                          SInt32 *    score);
	...
	bool setAdapterLevel(UInt32 newLevel);
	bool config();
	void disableAdapterInterrupts();
	void enableAdapterInterrupts();
	bool hwInit();
	bool iaSetup();
	bool mcSetup(enet_addr_t * addrs, UInt count, bool fromData = false);
	bool nop();
	...
	bool checkForInterrupt(IOFilterInterruptEventSource * src);
#endif
	...
	bool _phyGetLinkStatus(bool *         linkActive,
                           mediumType_t * activeMedium,
                           mediumType_t   selectedMedium);

	IONetworkMedium * _phyGetMediumWithCode(UInt32 code);
};
