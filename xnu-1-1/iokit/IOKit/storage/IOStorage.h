 ...
 * requests.
 * @constant kAccessNone
 * No access is requested; should not be passed to open().
 * @constant kAccessReader
 * Read-only access is requested.
 * @constant kAccessReaderWriter
 * Read and write access is requested.
 */

enum IOStorageAccess
{
    kAccessNone = 0, kAccessReader = 1, kAccessReaderWriter = 3
};
...
protected:
    ...
    inline void complete(IOStorageCompletion completion,
                         IOReturn            status,
...
inline bool IOStorage::open(IOService *     client,
                            IOOptionBits    options,
                            IOStorageAccess access)
{
    /*
    ...
    return IOService::open(client, options, (void *) access);
}
