...
//   the device decides it is idle and needs to reduce power. (changeStateTo)
//   the device decides it is not idle and needs to increase power. (changeStateTo)
//   the controlling driver requests a state change. (changeStateTo)
...
// into one, and there cannot be more than two parent-initiated change notes in the queue, because the parent does not
...
// and room for one more device change to get into the queue before collapsing it with its neighbor.  Or, we need
// room for two adjacent device changes (one in progress), a parent change, another device change, and room for
// one more device change to get into the queue before collapsing it with its neighbor.  Five entries in either case. 
// I'm not sure what irrationallity causes me to code for ten entries in the queue.
#define IOPMMaxChangeNotes 10
