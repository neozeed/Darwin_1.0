...
OSDefineMetaClass(IOMBufMemoryCursor, IOMemoryCursor)
OSDefineAbstractStructors(IOMBufMemoryCursor, IOMemoryCursor)
...
bool IOMBufMemoryCursor::initWithSpecification(OutputSegmentFunc inOutSeg,
                                               UInt inMaxSegmentSize,
                                               UInt inMaxNumSegments)
{
...
static const UInt kMBufDataCacheSize = 16;

static inline bool analyseSegments(
    struct mbuf *packet,        /* input packet mbuf */
    const UInt mbufsInCache,    /* number of entries in segsPerMBuf[] */
    const UInt segsPerMBuf[],   /* segments required per mbuf */
    SInt numSegs,               /* total number of segments */
    const UInt maxSegs)         /* max controller segments per mbuf */
{
    struct mbuf *newPacket;     // output mbuf chain.
    struct mbuf *out;           // current output mbuf link.
    SInt outSize;               // size of current output mbuf link.
    SInt outSegs;               // segments for current output mbuf link.
    SInt doneSegs;              // segments for output mbuf chain.
    SInt outLen;                // remaining length of input buffer.
    ...
    do {
        ...
        while (outLen > outSize) {
            ...
            if (doneSegs + 1 > (int) maxSegs) {
                IOLog("analyseSegments: maxSegs limit 1 reached! %d %d\n",
                    doneSegs, maxSegs);
                goto bombAnalysis;
            }
        ...
        if (doneSegs + outSegs > (int) maxSegs) {
            IOLog("analyseSegments: maxSegs limit 2 reached! %d %d %d\n",
...
UInt IOMBufMemoryCursor::genPhysicalSegments(struct mbuf *packet, void *vector,
                                       UInt maxSegs, bool doCoalesce)
{
    ...
    // packet represents.
    //
    UInt segsPerMBuf[kMBufDataCacheSize];
...
IOMBufBigMemoryCursor::withSpecification(UInt maxSegSize, UInt maxNumSegs)
{
...
IOMBufNaturalMemoryCursor::withSpecification(UInt maxSegSize, UInt maxNumSegs)
{
...
IOMBufLittleMemoryCursor::withSpecification(UInt maxSegSize, UInt maxNumSegs)
{
...
IOMBufDBDMAMemoryCursor::withSpecification(UInt maxSegSize, UInt maxNumSegs)
{
