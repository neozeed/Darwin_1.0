...
/* 16 */  { NULL, NULL, kIOUCScalarIStructI, 2, 0xffffffff },
    ...
    current_power_state = owner->registerInterestedDriver(this);
    owner->disallowPowerDown();
    return( true );
}
...
{
    return( owner->acknowledgePowerChange(this));
}

// If the frame buffer is being powered down, tell the window server.
IOReturn
IOFramebufferUserClient::powerStateWillChangeTo (IOPMPowerFlags newState, unsigned long, IOService* )
{
    kern_return_t r;
    mach_msg_header_t *msgh;

    if ( (current_power_state & IOPMDeviceUsable) && ! (newState & IOPMDeviceUsable) ) {
        msgh = (mach_msg_header_t *)notificationMsg;
        if( msgh && WSnotificationPort ) {
            msgh->msgh_id = 0;
            r = mach_msg_send_from_kernel( msgh, msgh->msgh_size);
        }
        current_power_state = newState;
        return 10000000;				// WS will ack within ten seconds
    }
    return IOPMAckImplied;
}


// If the frame buffer has been powered up, tell the window server.
IOReturn
IOFramebufferUserClient::powerStateDidChangeTo (IOPMPowerFlags newState, unsigned long, IOService* )
{
    kern_return_t r;
    mach_msg_header_t *msgh;
...
IOReturn IOFramebufferUserClient::clientClose( void )
{
    ...
    owner->deRegisterInterestedDriver(this);
    owner->disallowPowerDown();
    if( notificationMsg)
