...
#include <Drivers/platform/drvApplePMU/pmupriv.h>
...
ApplePMU *	PMUdriver;		// points to PMU driver
...
void AppleG3SeriesDisplay::initForPM ( IOService * provider )
{
    ...
    PMUname = OSSymbol::withCStringNoCopy(kPMUname);		// find the PMU driver
    service = waitForService(resourceMatching(PMUname));
    PMUdriver = OSDynamicCast(ApplePMU, service->getProperty(PMUname));

    rawTable = HooperTable;

    PMUdriver->sendMiscCommand(kPMUpower1Read,0, NULL, &unused,PMUreceiveBuffer);
    if ( PMUreceiveBuffer[0] & kScreenBit ) {							// is the screen currently on?
        PMUdriver->sendMiscCommand(kPMUReadBrightness,0, NULL, &unused,PMUreceiveBuffer);	// yes, figure out the brightness
    ...
    registerControllingDriver(this,ourPowerStates,kNumber_of_power_states);	// register with policy-maker (us)
}
    ...
    if ( powerStateOrdinal < kNumber_of_power_states ) {
        if ( powerStateOrdinal > pm_vars->myCurrentState ) {			// raising power
            if ( pm_vars->myCurrentState == 0 ) {					// is it currently off?
                PMUdriver->sendMiscCommand(kPMUpower1Cntl,1, &displayOn, &unused,NULL);
            }
        ...
        if ( powerStateOrdinal < pm_vars->myCurrentState ) {			// lowering power
            if (powerStateOrdinal == 0 ) {							// going all the way off?
                PMUdriver->sendMiscCommand(kPMUpower1Cntl,1, &displayOff, &unused,NULL);	// yes
                current_level = max_brightness_level[powerStateOrdinal];
            }
            else {
            ...
            case kBrightness_up:						// The brightness-up button has just been pressed
                ...
                if (current_level <  max_brightness_level[pm_vars->myCurrentState] ) {
                    ...
                    if ( pm_vars->myCurrentState < (kNumber_of_power_states-1) ) {
                        ...
                        if ( changeStateToPriv(pm_vars->myCurrentState + 1) != IOPMNoErr ) {		// request higher power
                ...
                if ( pm_vars->myCurrentState > 0 ) {				// don't lower if in lowest (off) state
                    if ( current_level > 0 ) {
                        ...
                        if (current_level <=  max_brightness_level[pm_vars->myCurrentState - 1] ) {	// if this takes us into the next lower state
                            changeStateToPriv(pm_vars->myCurrentState - 1);			// request lower power
                        }
                    }
                }
                break;
...
void AppleG3SeriesDisplay::setBrightness ( long brightness )
{
    ...
    PMUdriver->sendMiscCommand(kPMUSetBrightness,1, &setBrightnessBuffer, &unused,NULL);
}
