...
// FIX!!!
    ...
    frameBuffer = (volatile unsigned char *) vid_start;
}
#endif
		...
		int shmemVersion, int maxWidth, int maxHeight )
{
    ...
    sharedCursor = IOBufferMemoryDescriptor::withCapacity(
			round_page(size), kIODirectionNone, false );
    if( !sharedCursor)
        return( kIOReturnNoMemory );
    sharedCursor->setLength( size );
...
void IOFramebuffer::deferredMoveCursor( IOFramebuffer * inst, void * ref )
{
    StdFBShmem_t *	shmem = GetShmem(inst);
    IOReturn		err = kIOReturnSuccess;

    if( shmem)
        clock_get_uptime( &shmem->vblTime );
...
void IOFramebuffer::setBrightness( int /* level */ )
{
}
					...
					IOOptionBits options )
{
    ...
    shmem->hardwareCursorActive = (kIOReturnSuccess == err);
...
bool IOFramebuffer::start( IOService * provider )
{
    ...
    infoSerializer = OSSerializer::forTarget( (void *) this, &serializeInfoCB );
    if( !infoSerializer)
	return( false );
    ...
    changeStateToPriv(pm_vars->theNumberOfPowerStates - 1);
...
// disallowPowerDown
//
// Called by the User Client to disallow powering down the frame buffer.
// This condition exists between the instantiation of the User Client and
// the Window Server registering with it for power state notifications.
//
// As policy-maker, we request the highest power state, and this clamps
// the power on.
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

void IOFramebuffer::disallowPowerDown( void )
{
    changeStateToPriv(pm_vars->theNumberOfPowerStates - 1);
}
...
// allowPowerDown
//
// Called by the User Client to allow powering down the frame buffer.
// This condition exists after the Window Server registers with it for power
// state notifications and when the Window Server closes, regardless of
// whether or not it ever registered.
//
// As policy-maker, we request the lowest power state.  The power state
// is therefore controlled by the desires of our children (the displays).
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

void IOFramebuffer::allowPowerDown( void )
{
    changeStateToPriv(0);
}
...
//
// We are informed, as an Interested Driver, that our device is about to
// change power state.  If the new state is unusable, we warn the console
// to stop accessing the frame buffer for a while, since it will be powered off.
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

IOReturn IOFramebuffer::powerStateWillChangeTo( IOPMPowerFlags newState,
				unsigned long,  IOService* whatDevice )
{
    if ( ! (newState & IOPMDeviceUsable) ) {
        if( isConsoleDevice()) {
...
//
// We are informed, as an Interested Driver, that our device has changed
// power state.  If the new state is usable, we tell the console that it may
// access the frame buffer now, since it has powered on.
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

IOReturn IOFramebuffer::powerStateDidChangeTo( IOPMPowerFlags newState,
				unsigned long, IOService* whatDevice )
{
    if ( newState & IOPMDeviceUsable ) {
        if( isConsoleDevice()) {
...
IOReturn IOFramebuffer::open( void )
{
	...
	err = registerForInterruptType( kIOFBVBLInterruptType, 
			(IOFBInterruptProc) &deferredMoveCursor, 
