...
{   
    ...
    bzero( &targetParms, sizeof(SCSITargetParms) );
    ...
    if ( inquiryData->flags & kSCSIDevCapCmdQue )
    {
...
bool IOSCSIParallelDevice::setTargetParms( SCSITargetParms *targetParms )
{
    ...
    rc =  (target->negotiateResult == kIOReturnSuccess);
...
void IOSCSIParallelDevice::checkNegotiate( IOSCSIParallelCommand *scsiCmd )
{
    ...
    if ( target->negotiateState == kStateIssue )
    {
        if ( target->commandCount == 0 )
        {
            ...
            target->negotiateResult = kIOReturnSuccess;

            if ( tpCur->transferPeriodpS != tpNew->transferPeriodpS || tpCur->transferOffset != tpNew->transferOffset  )
            {
            ...
            if ( target->negotiateState != kStateActive )
            {
                IORWUnlock( target->targetSem ); 
...
void IOSCSIParallelDevice::executeCommandDone( IOSCSIParallelCommand *scsiCmd )
{
    ...
    if ( scsiCmd->scsiCmd.cdbFlags & (kCDBFlagsNegotiateSDTR | kCDBFlagsNegotiateWDTR | kCDBFlagsEnableTagQueuing) )
    {
        target->negotiateState  = kStateIdle;
        target->commandLimit    = target->commandLimitSave;

        if ( scsiCmd->results.adapterStatus != kSCSIAdapterStatusSuccess )
        {
            target->negotiateResult = kIOReturnIOError;

            target->targetParmsNew.transferWidth    = 1;
...
bool IOSCSIParallelDevice::matchPropertyTable(OSDictionary * table)
{
    ...
    if ( (match = compareProperty(table, kSCSIPropertyIOUnit)) )
