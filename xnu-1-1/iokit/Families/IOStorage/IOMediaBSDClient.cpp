...
protected:
    ...
    static IOReturn anchorChange(void *, void *, UInt32,
...
bool IOMediaBSDClient::createNodes(IOMedia * media)
{
    ...
    // it could be one of many other kinds of objects (eg. a RAID scheme).
    //
    ...
    media->setProperty(kIOBSDName,  _minors->getMinor(minorID)->name);
    media->setProperty(kIOBSDUnit,  anchorID, 32);              // ("BSD Unit" )
    media->setProperty(kIOBSDMajor, kMajor,   32);              // ("BSD Major")
    media->setProperty(kIOBSDMinor, minorID,  32);              // ("BSD Minor")
...
int dkopen(dev_t dev, int flags, int devtype, struct proc *)
{
    ...
    error = 0;
    level = (flags & FWRITE) ? kAccessReaderWriter : kAccessReader;
    ...
    {
        if ( minor->bdevWriter || minor->cdevWriter )  level = kAccessNone;
    }
    else                                                // (is client a reader?)
    {
        if ( minor->bdevOpen || minor->cdevOpen )  level = kAccessNone;
    }

    if ( error == 0 && level != kAccessNone )  // (issue the open (or upgrade)?)
    {
...
int dkclose(dev_t dev, int /* flags */, int devtype, struct proc *)
{
    ...
    else if ( !minor->bdevWriter && !minor->cdevWriter && wasWriter )
    {
        ...
        bool success;
        success = minor->media->open(gIOMediaBSDClient, 0, kAccessReader);
...
int dkioctl(dev_t dev, u_long cmd, caddr_t data, int, struct proc *)
{
    ...
    switch ( cmd )
    {
        ...
        {
            //
            // This ioctl asks that the media object be ejected from the drive.
            //

            IODrive *    drive;
            MinorTable * minors;

            drive  = OSDynamicCast(IODrive, minor->media->getProvider());
            minors = gIOMediaBSDClient->getMinors();

            // Determine whether this media has a drive object as its parent.

            if ( drive == 0 )  { error = ENOTTY; break; }
            ...
            else
            {
                ...
                // Close the media object, since IODrive semantics require that
                // no opens exist when the ejection request in made.

                minor->media->close(gIOMediaBSDClient);

                // Eject the media.

                error = gIOMediaBSDClient->errnoFromReturn(drive->ejectMedia());
            }
...
int dkreadwrite(dkr_t dkr, dkrtype_t dkrtype)
{
    ...
    //
    // We must reject non-block-aligned requests to conform to BSD semantics.
    //
    ...
    //
    // We must take in account that short reads and writes are not considered
    // errors under BSD semantics.  We're to transfer as many bytes as can be
    // read or written from the medium and return no error.  Also, reads that
    // start at (or perhaps past) the end-of-media are not considered errors,
    // even though no data is transferred, while writes at (or past) the end-
    // of-media do indeed return errors.  This differs from IOMedia semantics
    // which is to fail the entire request without transferring a single byte
    // should it include for something past the end-of-media.   We must adapt
    // the IOMedia semantics to look like BSD semantics here.
    // 
    ...
    // have retained the buffer themselves.  On the last release, the buffer is
    // complete()'d appropriately.
    //
                            ...
                            UInt64   actualByteCount )
{
    ...
    {
        IODrive * drive = dk_drive[anchorID];

        if ( drive )
        {
            dk_xfer[anchorID] = (long)
              ( drive->getStatistic(IODrive::kStatisticsReads ) +
                drive->getStatistic(IODrive::kStatisticsWrites) );
            dk_wds[anchorID] = (long) ( 8 *
              ( drive->getStatistic(IODrive::kStatisticsBytesRead   ) +
                drive->getStatistic(IODrive::kStatisticsBytesWritten) ) );
        }
    }
#endif IOMEDIABSDCLIENT_IOSTAT_SUPPORT
...
UInt32 AnchorTable::insert(IOService * anchor)
{
    ...
    if ( anchorID < DK_NDRIVE )
    {
        dk_drive[anchorID] = OSDynamicCast(IODrive, anchor);
...
IOReturn AnchorTable::anchorChange(    void *      /* target */,
                                       void *      /* parameter */,
                                       UInt32      messageType,
                                       IOService * anchor,
                                       ...
                                       vm_size_t   /* argSize */ )
{
    ...
    //
    // Notification handler for anchor termination (post-stop).
    //
