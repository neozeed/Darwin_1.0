...
IONDRV * IONDRV::instantiate( IOLogicalAddress container,
                                IOByteCount containerSize )
{
	...
	if( false == inst->init())
        ...
        if( err)
	    continue;
        err = PCodeInstantiate( inst->pcInst );
        if( err)
	    continue;
...
IONDRV * IONDRV::fromRegistryEntry( IORegistryEntry * regEntry )
{
    ...
    // -- only enable the boot display unless mmon flag is set
#if 0
    int	num;
    prop = (OSData *) regEntry->getProperty( "AAPL,boot-display" );
    if( (0 == prop) && (!PE_parse_boot_arg("mmon", &num)))
	return( 0 );
#endif
    // --
...
#define ins(i,d,a,simm) ((i<<26)+(d<<21)+(a<<16)+simm)
    ...
    if( pef) {
        kprintf( "pef = %08x, %08x\n", pef, propSize );
	inst = IONDRV::instantiate( pef, propSize );
	if( inst )
