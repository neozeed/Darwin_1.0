...
OSStatus _eExpMgrConfigReadLong( RegEntryID entryID, UInt8 offset, UInt32 * value )
{
    ...
    *value = ioDevice->configRead32( offset );
...
static SInt32 IONDRVStdInterruptHandler( InterruptSetMember setMember,
...
static bool IONDRVStdInterruptDisabler( InterruptSetMember setMember,
					void *refCon )
{
...
static void IONDRVStdInterruptEnabler( InterruptSetMember setMember,
					void *refCon )
{
    IONDRVInterruptSet *	set;
    IONDRVInterruptSource *	source;
...
static TVector tvIONDRVStdInterruptHandler  = { IONDRVStdInterruptHandler,  0 };
static TVector tvIONDRVStdInterruptEnabler  = { IONDRVStdInterruptEnabler,  0 };
static TVector tvIONDRVStdInterruptDisabler = { IONDRVStdInterruptDisabler, 0 };
					...
					IOService * provider, int index )
{
	...
	switch( result ) {
            ...
            case kIsrIsNotComplete:
		index++;
            case kIsrIsComplete:
		break;

            case kMemberNumberParent:
		assert( false );
		break;
...
//	IOService::waitForService( IOService::resourceMatching( "IONVRAM" ));
    ...
    data = OSData::withCapacity( kISTPropertyMemberCount
				 * sizeof( InterruptSetMember));

    InterruptSetMember 		setMember;
    IONDRVInterruptSet *	set;
    IONDRVInterruptSource *	source;

    set = IONDRVInterruptSet::with( provider, 0,
				kISTPropertyMemberCount );

    if( set) for( i = 1; i <= kISTPropertyMemberCount; i++ ) {
	...
	provider->registerInterrupt( i - 1, set,
				&IONDRVInterruptAction, (void *) 0x53 );
    } else
	data = 0;

    if( data) {
        provider->setProperty( kISTPropertyName, data );
        data->release();
        data = 0;
    }
