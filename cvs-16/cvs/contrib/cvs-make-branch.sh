...
usage ()
{
    ...
    echo "Options: -a automaticly update/checkout (don't ask)";
...
if ! args=$(getopt anvd:m:b: $*); then usage; fi;
set -- ${args};
for option; do
    case "${option}" in
      -d)
...
cvs ${q_Flag} "${Tag_Command}" ${Version_args} "${Branch}-base" ${Project_args};

echo "done.";

echo -n "Branching ${Branch} on ${Branch}-base in ${Project}...";

cvs ${q_Flag} "${Tag_Command}" -r "${Branch}-base" -b "${Branch}" ${Project_args};
