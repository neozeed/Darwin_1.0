...
static u_long		S_shadow_size_meg = SHADOW_SIZE_SAME;
#ifdef ppc
...
S_cfgProps_read()
{
    ...
    if (ni_pathsearch([ni_local handle], &confdir, BOOTSERV_CONFIG_NIDIR) 
	!= NI_OK) {
	...
	if (ni_read([ni_local handle], &confdir, &cfgProps) != NI_OK)
	    return (FALSE);
...
S_read_config()
{
    ...
    S_shadow_size_meg = SHADOW_SIZE_SAME;
    if ((nl_p = S_cfgProps_lookup(CFGPROP_SHADOW_SIZE_MEG)) 
	&& nl_p->ninl_len) {
...
#define NIPROP_GID		"gid"
    ...
    status = ni_pathsearch([ni_local handle], &dir, path);
    if (status != NI_OK) {
    ...
    status = ni_lookupprop([ni_local handle], &dir, NIPROP_GID, &nl);
    if (status == NI_OK) {
...
S_cfg_init()
{
	...
	for (i = 0; i < S_volumes.ninl_len; i++) {
		...
		syslog(LOG_INFO, "macNC: create_path(%s) failed, %m",  path);
		error = TRUE;
		break;
	    }
	    /*
...
{
    return (ni_sethostprop([ni_local handle], hostname, NIPROP_IPADDR, 
			   inet_ntoa(ip_addr)));
}
	      ...
	      int * host_number, boolean_t * err)
{
    ...
    *err = FALSE;
    ...
    if ([domain handle] != [ni_local handle]) {
	*err = TRUE;
	if (verbose)
	   ...
	   u_char * * bootfile, int * hostnum_p)
{
    ...
    *bootfile = ni_name_dup(S_default_bootfile);
	...
	/* create the entry */
	if (ni_createhost([ni_local handle], &pl, *hostname, 
...
#ifdef ppc
    else if (hfs_set_file_size(fd, size)) {
...
static boolean_t
S_remove_shadow(u_char * shadow_path, u_char * vol, u_char * dir)
{
    u_char path[PATH_MAX];
    ...
    S_set_uid_gid(shadow_path, 0, 0);
    unlink(shadow_path);
    S_get_volpath(path, vol, dir, NULL);
    /* and its directory */
    if (rmdir(path)) {
	u_char new_path[PATH_MAX];
	if (debug)
	    perror(path);
	S_get_volpath(new_path, vol, "Delete Me", NULL);
...
#define ONE_MEG		(1024UL * 1024UL)
	...
	if (nc_volume != NULL) {
		...
		timestamp_syslog("shadow file exists");
		if (debug)
		...
		if (sb_shadow.st_size != needspace) {
		    timestamp_syslog("shadow size is incorrect");
		    if (sb_shadow.st_size < needspace) {
			unsigned long long  	difference;
			unsigned long long	freespace = 0;
			
			timestamp_syslog("shadow file needs to be grown");
		    ...
		    else if (S_shadow_size_meg == SHADOW_SIZE_SAME) {
			set_file_size = TRUE;
		    }
		    else {
			timestamp_syslog("shadow size is OK");
		    }
		}
	    }
	    else {
	    ...
	    if (nc_volume) {
		if (set_file_size) {
		    timestamp_syslog("setting shadow file size");
		    ...
		    timestamp_syslog("setting shadow file perms/owner");
	   ...
	   struct timeval * tv_p)
{
    ...
    hp = hostbyaddr(S_ignore_hosts, rq->dp_htype, rq->dp_chaddr,
		    rq->dp_hlen);
    if (hp) {
    ...
    ea_p = (struct ether_addr *)rq->dp_chaddr;

    /* if gateway is set, allocate host from that subnet */
    if (rq->dp_giaddr.s_addr != 0) 
	server_ip = rq->dp_giaddr;
    else
	server_ip = intface->addr;

    if (subnetAddressAndMask(rq->dp_giaddr, intface, &subnetaddr, &subnetmask)
	== FALSE)
	return (FALSE);
    /* check if we have a host entry for this machine yet */
    if (S_lookup_host(rq->dp_htype, rq->dp_chaddr, rq->dp_hlen, 
	...
	reply->dp_yiaddr = ip_addr;
	bcopy(rfc_magic, reply->dp_options, sizeof(rfc_magic));

	/* add the bootfile */
	if (bootp_add_bootfile(rq->dp_file, hostname, S_default_bootdir,
			       bootfile, S_default_bootfile, 
			       NULL, reply->dp_file) == FALSE) {
	    goto err;
	}
	/* add the usual extensions/options */
	add_subnet_options(nil, NULL, ip_addr, intface, options, 
			   S_tags, S_tags_num);
	timestamp_syslog("subnet options added");
	...
	timestamp_syslog("getpwnam done");
	...
	timestamp_syslog("image options added");
...
#define MACNC_MAX_SERVICE_TIME	4 /* seconds */
	...
	}
	{
	    ...
	    reply->dp_siaddr = intface->addr;
	    strcpy(reply->dp_sname, intface->hostname);
	    if (sendreply(intface, (struct bootp *)reply, size, FALSE, NULL)) {
		if (!quiet)
  ...
  no_reply:
	...
	struct hosts * hp;
	hp = hostadd(&S_ignore_hosts, tv_p, rq->dp_htype, 
		     rq->dp_chaddr, rq->dp_hlen, NULL, NULL, NULL);
	if (verbose && hp) {
	    syslog(LOG_INFO, 
