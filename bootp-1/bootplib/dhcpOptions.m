...
- (boolean_t) addOption:(int)tag FromString:(unsigned char *)str
{
    ...
    len = [self freeSpace];
    if (len < (OPTION_OFFSET + 1)) {
	sprintf(errString, "no space for option %s (%d bytes left)", 
		tag_info->name, len);
	return (FALSE);
    }
    len -= (OPTION_OFFSET + 1); /* leave room for tag, len and end tag */
...
#define MAX_TAG (sizeof(dhcptag_info) / sizeof(dhcptag_info[0]))

+ (tag_info_t *) tagInfo:(int)tag
{
    if (tag >= MAX_TAG || dhcptag_info[tag].name == NULL)
	return (NULL);
...
+ (int) tagWithName:(unsigned char *) name
{
    int i;

    for (i = 0; i < MAX_TAG; i++) {
	if (dhcptag_info[i].name) {
	    if (strcmp(dhcptag_info[i].name, name) == 0)
		return (i);
	}
    }
    return (-1);
...
/*
 * Method: strFromTag:Option:Length:ErrorString:
 *
 * Purpose:
 *   Give a string representation to the given type.
 */
+ (boolean_t) str:(u_char *)tmp FromTag:(int) tag Option:(void *)opt 
	   Length:(int)len ErrorString:(u_char *)err
{
    tag_info_t * tag_info;
    type_info_t * type_info;

    tag_info = [self tagInfo:tag];
    if (tag_info == NULL)
	return (FALSE);
    type_info = [self typeInfo:tag_info->type];
    if (type_info->multiple_of == dhcptype_none_e)
	return [self str:tmp FromOption:opt Length:len 
		     Type:tag_info->type ErrorString:err];
    return [self str:tmp FromOption:opt Length:len 
		 Type:type_info->multiple_of ErrorString:err];
}
...
#if 0
@implementation dhcpPacketOptions
- initWithPacket:(struct dhcp *)pkt OptionSize:(int)size
{
    [super init];
    options = [[dhcpOptions alloc] 
	       initWithBuffer:pkt->dp_options + RFC_MAGIC_SIZE
	       Size:size - RFC_MAGIC_SIZE];
    if (options == nil)
	return [super free];
    return self;
}

- initAndParsePacket:(struct dhcp *)pkt OptionSize:(int)size
{
    if ([self initWithPacket:pkt OptionSize:size] == nil)
	return nil;
...
/*
 * The following function needs to be taught about parsing the
 * the bootfile/hostname options if need be too, and handling
 * any vendor-specific option areas as well.
 */
- (void *)findOptionWithTag:(int)tag Length:(int *)len_p
{
    return [options findOptionWithTag:tag Length:len_p];
}
