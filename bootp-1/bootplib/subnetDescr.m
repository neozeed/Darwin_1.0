...
- initRead:(NIDomain *)dom Dir:(ni_id)d Err:(u_char *)errString;
{
    ni_status 		status;
    ...
    status = ni_read([domain handle], &dir, &pl);
    if (status != NI_OK)
	return [self free];
...
- initFromDomains:domains Err:(u_char *)errString
{
    ni_id		dir;
    ni_entrylist 	el;
    int			i;
	...
	domain = [domains objectAt:i];
	if (domain == nil)
	    continue;
	status = ni_pathsearch([domain handle], &dir, SUBNETS_NIDIR);
	if (status != NI_OK) {
	    S_strcpy(errString, ni_error(status));
	    continue;
	}
	status = ni_list([domain handle], &dir, NIPROP_NET_RANGE, &el);
	if (status != NI_OK) {
	    S_strcpy(errString, ni_error(status));
	    continue;
	}
