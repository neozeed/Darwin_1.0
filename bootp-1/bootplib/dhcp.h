...
typedef u_int32_t		dhcp_time_secs_t; /* absolute time */
typedef u_int32_t		dhcp_lease_t;     /* relative time */
#define dhcp_time_hton		htonl
#define dhcp_time_ntoh		ntohl
#define dhcp_lease_hton		htonl
#define dhcp_lease_ntoh		ntohl	

#define DHCP_INFINITE_LEASE	((dhcp_lease_t)0xffffffff)
#define DHCP_INFINITE_SECS	((dhcp_time_secs_t)(-1))

#define DHCP_FLAGS_BROADCAST	((u_short)0x0001)

typedef enum {
    dhcp_cstate_none_e 	= 0,
    dhcp_cstate_declining_e,
