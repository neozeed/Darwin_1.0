	...
	switch (ifrp->ifr_addr.sa_family) {
	  case AF_INET: {
		  ...
		  if (ioctl(sockfd, SIOCGIFFLAGS, (caddr_t)&ifr) < 0) {
		      syslog(LOG_INFO, "%s: ioctl(SIOGIFFLAGS), %m",
			     name);
		  }
		  else
		  ...
		  syslog(LOG_INFO, "%s: ioctl(SIOGIFNETMASK), %m",
			 name);
	      }
	      else
		  entry->mask 
		      = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
	      if (entry->flags & IFF_BROADCAST) {
		  if (ioctl(sockfd, SIOCGIFBRDADDR, (caddr_t)&ifr)< 0) {
		      syslog(LOG_INFO, "%s: ioctl(SIOCGBRDADDR), %m",
			     name);
		  }
		  else
		      entry->broadcast 
		  ...
		  if (gethostname(entry->hostname, sizeof(entry->hostname))) {
		      syslog(LOG_INFO, 
			     "%s: gethostbyaddr(%s) and gethostname() failed",
			     name, inet_ntoa(entry->addr));
		      entry->hostname[0] = '\0';
		  }
	      }
	      else 
