...
#ifndef OLD_HFS_PROTO
    char			name[HFS_NAME_SIZE];
#endif OLD_HFS_PROTO
    long			n_vols;
    struct statfs * 		stat_p;
    int				stat_number;
...
#else OLD_HFS_PROTO
	if (GetVolumeInfo_VDI(vol_index, 0, NULL, 0, 0, 0, &volInfo,
			      kMacOSHFSFormat, name, sizeof(name)) != 0) {
	    break;
	}
#endif OLD_HFS_PROTO
