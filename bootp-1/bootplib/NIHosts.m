...
+ (NIDomain *)lookupKey:(ni_name)key Value:(ni_name)value 
 ...
 Matches:(int *)matches
 PropList:(ni_proplist *)proplist Dir:(ni_id *)dir_p
{
    ni_id	dir;
    int		d_index;
    ni_idlist   il;
	...
	id 	domain = [list objectAt:d_index];
	...
	if (ni_pathsearch([domain handle], &dir, NIDIR_MACHINES) == NI_OK
		...
		if (status != NI_OK) {
		    syslog(LOG_ERR, "NIHosts: ni_read failed %s\n", 
			   ni_error(status));
		    continue;
		}
		k++;
